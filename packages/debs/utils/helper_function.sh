...
setup_build(){
    ...
    cp -pr ${specs_path}/shieldnet-defend-${BUILD_TARGET}/debian ${sources_dir}/debian
...
get_package_and_checksum(){
    shieldnet_defend_version="$1"
    short_commit_hash="$2"
    base_name="shieldnet-defend-${BUILD_TARGET}_${shieldnet_defend_version}-${REVISION}"
    symbols_base_name="shieldnet-defend-${BUILD_TARGET}-dbg_${shieldnet_defend_version}-${REVISION}"
    ...
    if [[ "${checksum}" == "yes" ]]; then
        ...
        cd ${pkg_path} && sha512sum shieldnet-defend-${BUILD_TARGET}_*deb > /var/local/shieldnetdefend/${deb_file}.sha512
        sha512sum shieldnet-defend-${BUILD_TARGET}-*deb > /var/local/shieldnetdefend/${symbols_deb_file}.sha512
    fi

    find ${pkg_path} -type f -name "shieldnet-defend-${BUILD_TARGET}_*deb" -exec mv {} /var/local/shieldnetdefend/${deb_file} \;
    find ${pkg_path} -type f -name "shieldnet-defend-${BUILD_TARGET}-dbg_*deb" -exec mv {} /var/local/shieldnetdefend/${symbols_deb_file} \;
}
