...
# ShieldnetDefend Solaris 11 Package builder.

set -x
REPOSITORY="https://github.com/shieldnetdefend/shieldnetdefend"
shieldnet_defend_branch="main"
...
set_control_binary() {
    if [ -e ${SOURCE}/VERSION.json ]; then
        shieldnet_defend_version="v$(sed -n 's/.*"version"[ \t]*:[ \t]*"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)"
        number_version=`echo "${shieldnet_defend_version}" | cut -d v -f 2`
...
build_environment() {
    ...
    curl -LO http://packages-dev.shieldnetdefend.com/deps/solaris/precompiled-solaris-gcc-5.5.0.tar.gz
    ...
    curl -LO http://packages-dev.shieldnetdefend.com/deps/solaris/precompiled-solaris-cmake-3.18.3.tar.gz
...
compute_version_revision() {
    shieldnet_defend_version="$(sed -n 's/.*"version"[ \t]*:[ \t]*"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)"
    revision=$(sed -n 's/.*"stage": *"\([^"]*\)".*/\1/p' ${SOURCE}/VERSION.json)

    echo $shieldnet_defend_version > /tmp/VERSION
...
download_source() {
    ...
    if [[ "${shieldnet_defend_branch}" != "trunk" ]] || [[ "${shieldnet_defend_branch}" != "main" ]]; then
        cd $SOURCE
        git checkout $shieldnet_defend_branch
    fi
    cd ${current_path}
    compute_version_revision
}
...
#Compile and install shieldnet-defend-agent
compile() {
...
create_package() {
    ...
    if [ $(echo $VERSION | grep "v") ]; then
        ...
        sed "s/<VERSION>/$ver/" ${current_path}/shieldnet-defend-agent.mog-template > ${current_path}/shieldnet-defend-agent.mog-aux
    else
        sed "s/<VERSION>/$VERSION/" ${current_path}/shieldnet-defend-agent.mog-template > ${current_path}/shieldnet-defend-agent.mog-aux
    fi
    sed "s/<TAG>/$VERSION/" ${current_path}/shieldnet-defend-agent.mog-aux > ${current_path}/shieldnet-defend-agent.mog

    echo "Building the package shieldnet-defend-agent_$VERSION-sol11-${arch}.p5p"

    set_control_binary


    # Package generation process
    pkgsend generate ${install_path} | pkgfmt > shieldnet-defend-agent.p5m.1
    ...
    python solaris_fix.py -t SPECS/template_agent.json -p shieldnet-defend-agent.p5m.1 # Fix p5m.1 file
    mv shieldnet-defend-agent.p5m.1.aux.fixed shieldnet-defend-agent.p5m.1
    ...
    for file in etc/ossec.conf etc/local_internal_options.conf etc/client.keys; do
        sed "s:file $file.*:& preserve=install-only:"  shieldnet-defend-agent.p5m.1 > shieldnet-defend-agent.p5m.1.aux_sed
        mv shieldnet-defend-agent.p5m.1.aux_sed shieldnet-defend-agent.p5m.1
    done
    # Add service files
    echo "file smf_manifest.xml path=lib/svc/manifest/site/post-install.xml owner=root group=sys mode=0744 restart_fmri=svc:/system/manifest-import:default" >> shieldnet-defend-agent.p5m.1
    echo "dir  path=var/ossec/installation_scripts owner=root group=bin mode=0755" >> shieldnet-defend-agent.p5m.1
    echo "file postinstall.sh path=var/ossec/installation_scripts/postinstall.sh owner=root group=bin mode=0744" >> shieldnet-defend-agent.p5m.1
    echo "file shieldnet-defend-agent path=etc/init.d/shieldnet-defend-agent owner=root group=sys mode=0744" >> shieldnet-defend-agent.p5m.1
    echo "file S97shieldnet-defend-agent path=etc/rc2.d/S97shieldnet-defend-agent owner=root group=sys mode=0744" >> shieldnet-defend-agent.p5m.1
    echo "file S97shieldnet-defend-agent path=etc/rc3.d/S97shieldnet-defend-agent owner=root group=sys mode=0744" >> shieldnet-defend-agent.p5m.1

    # Add user and group shieldnetdefend
    echo "group groupname=shieldnetdefend" >> shieldnet-defend-agent.p5m.1
    echo "user username=shieldnetdefend group=shieldnetdefend" >> shieldnet-defend-agent.p5m.1

    # Necessary to upgrade from < 4.3 versions
    echo "group groupname=ossec" >> shieldnet-defend-agent.p5m.1
    echo "user username=ossec group=ossec" >> shieldnet-defend-agent.p5m.1

    pkgmogrify -DARCH=`uname -p` shieldnet-defend-agent.p5m.1 shieldnet-defend-agent.mog | pkgfmt > shieldnet-defend-agent.p5m.2
    pkgsend -s http://localhost:9001 publish -d ${install_path} -d /etc/init.d -d /etc/rc2.d -d /etc/rc3.d -d ${current_path} shieldnet-defend-agent.p5m.2 > pack
    package=`cat pack | grep shieldnetdefend | cut -c 13-` # This extracts the name of the package generated in the previous step
    rm -f *.p5p
    pkg_name="shieldnet-defend-agent_$VERSION-sol11-${arch}.p5p"
...
create_repo() {
    zfs create rpool/shieldnetdefend
    zfs set mountpoint=/shieldnetdefend rpool/shieldnetdefend
    pkgrepo create /shieldnetdefend
    ls /shieldnetdefend
    pkgrepo -s /shieldnetdefend set publisher/prefix=shieldnetdefend
    svccfg -s application/pkg/server setprop \ pkg/inst_root=/shieldnetdefend
...
uninstall() {
    ...
    rm -f `find /etc | grep shieldnetdefend`
    rm -f /etc/rc3.d/S97shieldnet-defend-agent
    rm -f /etc/rc2.d/S97shieldnet-defend-agent
}
...
clean() {
    ...
    umount /shieldnetdefend
    zfs destroy rpool/shieldnetdefend
    rm -rf /shieldnetdefend
    rm -rf $SOURCE/shieldnetdefend
    rm -f shieldnet-defend-agent.p5m*
    rm -f shieldnet-defend-agent.mog
    rm -f shieldnet-defend-agent.mog-aux
    rm -f pack
}
...
show_help() {
    ...
    echo -e "                Select Git branch or tag e.g. ${shieldnet_defend_branch}."
    echo -e ""
    echo -e "        -c, --checksum"
