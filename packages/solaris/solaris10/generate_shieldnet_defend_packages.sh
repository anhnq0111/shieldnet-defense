...
# ShieldnetDefend Solaris 10 Package builder.


# CONFIGURATION VARIABLES
shieldnet_defend_branch="$(echo "$2" | cut -d "/" -f2)"
...
if [ -z "${shieldnet_defend_branch}" ]; then
...
clone(){
    cd ${CURRENT_PATH}
    git clone --depth=1 $REPOSITORY -b $shieldnet_defend_branch ${SOURCE} || return 1
    cd $SOURCE
    compute_version_revision
...
package(){
    cd ${CURRENT_PATH}
    find ${install_path} | awk 'length > 0' > "shieldnet-defend-agent_$VERSION.list"
    ...
    echo "i pkginfo=pkginfo" > "shieldnet-defend-agent_$VERSION.proto"
    echo "i checkinstall=checkinstall.sh" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "i preinstall=preinstall.sh" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "i postinstall=postinstall.sh" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "i preremove=preremove.sh" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "i postremove=postremove.sh" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "f none /etc/init.d/shieldnet-defend-agent  0755 root root" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "s none /etc/rc2.d/S97shieldnet-defend-agent=/etc/init.d/shieldnet-defend-agent" >> "shieldnet-defend-agent_$VERSION.proto"
    echo "s none /etc/rc3.d/S97shieldnet-defend-agent=/etc/init.d/shieldnet-defend-agent" >> "shieldnet-defend-agent_$VERSION.proto"
    cat "shieldnet-defend-agent_$VERSION.list" | pkgproto >> "shieldnet-defend-agent_$VERSION.proto"

    echo $VERSION
    pkgmk -o -r / -d . -f "shieldnet-defend-agent_$VERSION.proto"
    pkg_name="shieldnet-defend-agent_$VERSION-sol10-$ARCH.pkg"
    pkgtrans -s ${CURRENT_PATH} "${pkg_name}" shieldnet-defend-agent
...
clean(){
    ...
    cd ${CURRENT_PATH}
    rm -rf ${SOURCE}
    rm -rf shieldnet-defend-agent shieldnetdefend *.list *proto
    rm -f *.new
...
build(){
    ...
    groupadd shieldnetdefend
    useradd -g shieldnetdefend shieldnetdefend
    installation
    package
}
