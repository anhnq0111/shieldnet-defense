#!/bin/sh
# preinstall script for shieldnet-defend-agent
...
getent group shieldnetdefend > /dev/null 2>&1
if [ "$?" -ne 0 ]; then
    groupadd ${GROUP}
fi

getent passwd shieldnetdefend > /dev/null 2>&1
if [ "$?" -ne 0 ]; then
...
case $type in
    ...
    if [ -f /etc/init.d/shieldnet-defend-agent ]; then
        rm /etc/init.d/shieldnet-defend-agent
    fi
