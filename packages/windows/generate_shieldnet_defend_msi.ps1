...
param (
    [string]$MSI_NAME = "shieldnet-defend-agent.msi",
    [string]$SIGN = "no",
...
if(($help.isPresent)) {
    "
    This tool can be used to generate the Windows ShieldnetDefend agent msi package.

    PARAMETERS TO BUILD SHIELDNET-DEFEND-AGENT MSI (OPTIONALS):
    ...
    USAGE:
        ...
        * SHIELDNETDEFEND:
          $ ./generate_shieldnet_defend_msi.ps1  -MSI_NAME {{ NAME }} -SIGN {{ yes|no }} -WIX_TOOLS_PATH {{ PATH }} -SIGN_TOOLS_PATH {{ PATH }}
            Build a devel msi:    $ ./generate_shieldnet_defend_msi.ps1 -MSI_NAME shieldnet-defend-agent_4.9.0-0_windows_0ceb378.msi -SIGN no
            Build a prod msi:     $ ./generate_shieldnet_defend_msi.ps1 -MSI_NAME shieldnet-defend-agent-4.9.0-1.msi -SIGN yes
    "
    Exit
}
...
function BuildShieldnetDefendMsi(){
    ...
    if($SIGN -eq "yes"){
        ...
        $filesToSign = @(
            ...
            "..\shieldnet_defend_modules\syscollector\build\bin\syscollector.dll",
    ...
    & $CANDLE_EXE -nologo .\shieldnet-defend-installer.wxs -out "shieldnet-defend-installer.wixobj" -ext WixUtilExtension -ext WixUiExtension
    & $LIGHT_EXE ".\shieldnet-defend-installer.wixobj" -out $MSI_NAME -ext WixUtilExtension -ext WixUiExtension
...
function ExtractDebugSymbols(){
	...
	$exeFiles +=  Get-ChildItem -Filter "shieldnet_defend_modules\syscollector\build\bin\syscollector.dll"
  ...
  #compress every pdb file in current folder
    ...
    $ZIP_NAME = $MSI_NAME -replace 'shieldnet-defend-agent', 'shieldnet-defend-agent-debug-symbols' -replace '\.msi$', '.zip'
