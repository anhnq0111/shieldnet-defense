#!/bin/bash
set -e
# Program to build and package OSX shieldnet-defend-agent
...
ARCH="intel64"
SHIELDNET_DEFEND_SOURCE_REPOSITORY="https://github.com/shieldnetdefend/shieldnetdefend"
SERVICE_PATH="/Library/LaunchDaemons/com.shieldnetdefend.agent.plist"
STARTUP_PATH="/Library/StartupItems/SHIELDNETDEFEND/StartupParameters.plist"
LAUNCHER_SCRIPT_PATH="/Library/StartupItems/SHIELDNETDEFEND/Shieldnet-Defend-launcher"
STARTUP_SCRIPT_PATH="/Library/StartupItems/SHIELDNETDEFEND/SHIELDNETDEFEND"
...
function clean_and_exit() {
    ...
    if [ -z "$BRANCH_TAG" ]; then
        make -C $SHIELDNET_DEFEND_PATH/src clean clean-deps
    fi
...
function sign_binaries() {
    if [ ! -z "${KEYCHAIN}" ] && [ ! -z "${CERT_APPLICATION_ID}" ] ; then
        ...
        # Sign every single binary in ShieldnetDefend's installation. This also includes library files.
...
function prepare_building_folder() {
    ...
    version="$1"
    pkg_final_name="$2"
    build_info_file="${SHIELDNET_DEFEND_PACKAGES_PATH}/specs/build-info.json"
    preinstall_script="${SHIELDNET_DEFEND_PACKAGES_PATH}/package_files/preinstall.sh"
    postinstall_script="${SHIELDNET_DEFEND_PACKAGES_PATH}/package_files/postinstall.sh"
    packaged_directory=$CURRENT_PATH/shieldnet-defend-agent/payload

    if [ -d "$CURRENT_PATH/shieldnet-defend-agent" ]; then

        echo "\nThe shieldnetdefend agent building directory is present on this machine."
        echo "Removing it from the system."

        rm -rf $CURRENT_PATH/shieldnet-defend-agent
    fi

    munkipkg --create --json $CURRENT_PATH/shieldnet-defend-agent

    cp -f $build_info_file $CURRENT_PATH/shieldnet-defend-agent/

    sed -i '' "s|VERSION|$version|g" $CURRENT_PATH/shieldnet-defend-agent/$(basename $build_info_file)
    sed -i '' "s|PACKAGE_NAME|$pkg_final_name|g" $CURRENT_PATH/shieldnet-defend-agent/$(basename $build_info_file)

    cp $preinstall_script $CURRENT_PATH/shieldnet-defend-agent/scripts/preinstall
    cp $postinstall_script $CURRENT_PATH/shieldnet-defend-agent/scripts/postinstall

    sed -i '' "s|PACKAGE_ARCH|$ARCH|g" $CURRENT_PATH/shieldnet-defend-agent/scripts/preinstall
...
function build_package() {
    ...
    if [ -n "$BRANCH_TAG" ]; then
        ...
        SHIELDNET_DEFEND_PATH="${SOURCES_DIRECTORY}/shieldnetdefend"
        git clone --depth=1 -b ${BRANCH_TAG} ${SHIELDNET_DEFEND_SOURCE_REPOSITORY} "${SHIELDNET_DEFEND_PATH}"
    else
        SHIELDNET_DEFEND_PATH="${CURRENT_PATH}/../.."
    fi
    short_commit_hash="$(cd "${SHIELDNET_DEFEND_PATH}" && git rev-parse --short=7 HEAD)"

    export CONFIG="${SHIELDNET_DEFEND_PATH}/etc/preloaded-vars.conf"
    SHIELDNET_DEFEND_PACKAGES_PATH="${SHIELDNET_DEFEND_PATH}/packages/macos"
    ENTITLEMENTS_PATH="${SHIELDNET_DEFEND_PACKAGES_PATH}/entitlements.plist"

    VERSION="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $SHIELDNET_DEFEND_PATH/VERSION.json)"

    # Define output package name
    if [ $IS_STAGE == "no" ]; then
        pkg_name="shieldnet-defend-agent_${VERSION}-${REVISION}_${ARCH}_${short_commit_hash}"
    else
        pkg_name="shieldnet-defend-agent-${VERSION}-${REVISION}.${ARCH}"
    fi

    if [ -d "${INSTALLATION_PATH}" ]; then

        echo "\nThe shieldnetdefend agent is already installed on this machine."
        echo "Removing it from the system."

        ${CURRENT_PATH}/uninstall.sh
    fi

    ${SHIELDNET_DEFEND_PACKAGES_PATH}/package_files/build.sh "${INSTALLATION_PATH}" "${SHIELDNET_DEFEND_PATH}" ${JOBS} ${DEBUG} ${MAKE_COMPILATION}
    ...
    # create package
    if munkipkg $CURRENT_PATH/shieldnet-defend-agent ; then
        echo "The shieldnetdefend agent package for macOS has been successfully built."
        mv $CURRENT_PATH/shieldnet-defend-agent/build/* $DESTINATION/
        symbols_pkg_name="shieldnet-defend-agent-debug-symbols-${VERSION}-${REVISION}.${ARCH}-macos"
        cp -R "${SHIELDNET_DEFEND_PATH}/src/symbols"  "${DESTINATION}"
