...
# Copyright (C) 2018 ShieldnetDefend Inc.
# This program is a free software; you can redistribute
# it and/or modify it under the terms of GPLv2
# ShieldnetDefend HP-UX Package builder.
...
source_directory=/shieldnet-defend-sources
configuration_file="${source_directory}/etc/preloaded-vars.conf"
target_dir="${current_path}/output/"
shieldnet_defend_version=""
shieldnet_defend_revision=""
depot_path=""
control_binary=""
compute_checksums="no"

# Needed variables to build ShieldnetDefend with custom GCC and cmake
...
compute_version_revision() {
    shieldnet_defend_version="$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $source_directory/VERSION.json)"

    if [ -z "$shieldnet_defend_revision" ]; then
        stage_value=$(awk -F'"' '/"stage"[ \t]*:/ {print $4}' $source_directory/VERSION.json)
        shieldnet_defend_revision=$(echo "$stage_value" | sed 's/[^0-9]*\([0-9][0-9]*\).*/\1/')
    fi

    # Add commit hash to the VERSION.json file
    short_commit_hash=$(/usr/local/bin/curl -ks "https://api.github.com/repos/shieldnetdefend/shieldnetdefend/commits/${shieldnet_defend_branch}" | awk -F '"' '/"sha":/ {print substr($4, 1, 7); exit}')
    ...
    echo ${shieldnet_defend_revision} > /tmp/REVISION
...
download_source() {
    ...
    /usr/local/bin/curl -k -L -o "/shieldnetdefend.zip" "https://github.com/shieldnetdefend/shieldnetdefend/archive/${shieldnet_defend_branch}.zip"
    /usr/local/bin/unzip /shieldnetdefend.zip
    mv shieldnet-defend-* ${source_directory}
    compute_version_revision
}

check_version() {
    shieldnet_defend_version="v$(awk -F'"' '/"version"[ \t]*:/ {print $4}' $source_directory/VERSION.json)"
...
compile() {
    echo "Compiling code"
    # Compile and install shieldnetdefend
...
create_package() {
    ...
    shieldnet_defend_version=`echo "${shieldnet_defend_version}" | cut -d v -f 2`
    pkg_tar_file="shieldnet-defend-agent-${shieldnet_defend_version}-${shieldnet_defend_revision}-hpux-11v3-ia64.tar"
    tar cvpf ${target_dir}/${pkg_tar_file} ${install_path} /sbin/init.d/shieldnet-defend-agent /sbin/rc2.d/S97shieldnet-defend-agent /sbin/rc3.d/S97shieldnet-defend-agent
...
clean() {
    ...
    find /sbin -name "*shieldnet-defend-agent*" -exec rm {} \;
...
show_help() {
    ...
    echo "    -r, --revision Revision to naming ShieldnetDefend agent package."
