...
This integration enrichs any alert generated by ShieldnetDefend via the Maltiverse API,
...
And restart ShieldnetDefend Manager:

    /etc/init.d/shieldnet-defend-manager restart
...
#    5 - Invalid URL
#    6 - Unable to connect to ShieldnetDefend
#    7 - Unexpected error sending message to ShieldnetDefend
...
def process_args(args: list):
    ...
    # ShieldnetDefend Manager in case of positive match
...
def send_event(msg: str, agent: dict = None):
    """Send an event to the ShieldnetDefend Manager.
    ...
    try:
        ...
        if e.errno == 111:
            print('ERROR: ShieldnetDefend is not running.')
            sys.exit(6)
        elif e.errno == 90:
            print('ERROR: Message too long to send to ShieldnetDefend.')
            sys.exit(7)
