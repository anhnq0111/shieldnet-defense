...
brief: The 'shieldnet-defend-agentd' program is the client-side daemon that communicates with the server.
       These tests will check if the content of the 'shieldnet-defend-agentd' daemon statistics file is valid.
       The statistics files are documents that show real-time information about the ShieldnetDefend environment.
...
references:
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/statistics-files/shieldnet-defend-agentd-state.html
...
from shieldnet_defend_testing.modules.agentd.utils import parse_state_file
...
cases_path = Path(TEST_CASES_PATH, 'shieldnet_defend_state_config_tests.yaml')
...
def test_agentd_state(test_configuration, test_metadata, set_shieldnet_defend_configuration, remove_state_file, configure_local_internal_options,
                      truncate_monitored_files, clean_keys, add_keys, daemons_handler):
    '''
    description: Check that the statistics file 'shieldnet-defend-agentd.state' is created automatically
    ...
    parameters:
        ...
        - remove_state_file:
            type: fixture
            brief: Removes the shieldnet-defend-agentd.state file
    ...
    assertions:
        - Verify that the 'shieldnet-defend-agentd.state' statistics file has been created.
        - Verify that the information stored in the 'shieldnet-defend-agentd.state' statistics file
          is consistent with the connection status to the 'shieldnet-defend-remoted' daemon.

    input_description: An external YAML file (shieldnet_defend_conf.yaml) includes configuration settings for the agent.
                       Different test cases that are contained in an external YAML file (shieldnet_defend_state_tests.yaml)
