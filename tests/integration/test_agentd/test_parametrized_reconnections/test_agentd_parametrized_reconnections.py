...
brief: The 'shieldnet-defend-agentd' program is the client-side daemon that communicates with the server.
       The objective is to check how the 'shieldnet-defend-agentd' daemon behaves when there are delays
       between connection attempts to the 'shieldnet-defend-remoted' daemon using TCP and UDP protocols.
...
from shieldnet_defend_testing.modules.agentd.patterns import AGENTD_TRYING_CONNECT, AGENTD_CONNECTED_TO_SERVER
...
def test_agentd_parametrized_reconnections(test_metadata, set_shieldnet_defend_configuration, configure_local_internal_options,
    ...
    assertions:
        - Verify that when the 'shieldnet-defend-agentd' daemon initializes, it connects to
          the 'shieldnet-defend-remoted' daemon of the manager before reaching the maximum number of attempts.
    ...
    shieldnet_defend_log_monitor.start(accumulations = test_metadata['MAX_RETRIES'], callback=callbacks.generate_callback(AGENTD_TRYING_CONNECT,{'IP':'','PORT':''}))
    assert (shieldnet_defend_log_monitor.callback_result != None), f'Trying to connect to server message not found expected times'

    #Check number of connected message is the expected
    if test_metadata['ENROLL'] == 'yes':
        shieldnet_defend_log_monitor.start(callback=callbacks.generate_callback(AGENTD_CONNECTED_TO_SERVER))
