...
references:
    ...
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/localfile.html
...
from shieldnet_defend_testing.utils.file import write_json_file
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                reset_ofe_status, remove_all_localfiles_shieldnet_defend_config, set_shieldnet_defend_configuration, pre_send_journal_logs,
                                daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'shieldnet-defend-logcollector' daemon starts properly when the 'journald' tag is used 
                 ...
                 Finally, the test will verify that the Shieldnet-Defend-logcollector read the logs, and the ShieldnetDefend API returns the correct values
                 for the 'localfile' section.

    shieldnet_defend_min_version: 4.9.0
    ...
    parameters:
        ...
        - remove_all_localfiles_shieldnet_defend_config:
            type: fixture
            brief: Remove all 'localfile' sections from the ShieldnetDefend configuration.
        - reset_ofe_status:
            type: fixture
    ...
    assertions:
        - Verify that the ShieldnetDefend component (agent or manager) can start when the 'jouranld' tag is used.
        - Verify that the ShieldnetDefend component (agent or manager) can read the logs from the 'journald' log format and the logs older than the current logcollector start time.

    input_description: A configuration file with journal block settings and the expected log messages.
                       Those include configuration settings for `journal` configuration in 'shieldnet-defend-logcollector'.
