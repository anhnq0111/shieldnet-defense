...
from shieldnet_defend_testing.constants.daemons import LOGCOLLECTOR_DAEMON
from shieldnet_defend_testing.modules.logcollector.patterns import LOGCOLLECTOR_MODULE_START
...
from shieldnet_defend_testing.utils.file import truncate_file, replace_regex_in_file, write_json_file

# Logcollector internal paths
LOGCOLLECTOR_OFE_PATH = path_join(SHIELDNET_DEFEND_PATH, 'queue', 'logcollector', 'file_status.json')

@pytest.fixture()
def stop_logcollector(request):
    """Stop shieldnet-defend-logcollector and truncate logs file."""
...
@pytest.fixture()
def remove_all_localfiles_shieldnet_defend_config(request):
    ...
    replace_regex_in_file(list_tags, [''] * len(list_tags), SHIELDNET_DEFEND_CONF_PATH, True)
