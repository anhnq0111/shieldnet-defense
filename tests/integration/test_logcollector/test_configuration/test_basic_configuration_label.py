...
brief: The 'shieldnet-defend-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       the 'label' tag is set in the configuration, and the ShieldnetDefend API returns the same values for
...
references:
    ...
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/localfile.html#label
...
config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_basic_configuration_label.yaml')
...
def test_configuration_label(test_configuration, test_metadata, set_shieldnet_defend_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'shieldnet-defend-logcollector' daemon can monitor log files configured to use labels.
                 ...
                 Finally, the test will verify that the ShieldnetDefend API returns the same values for
    ...
    shieldnet_defend_log_monitor.start(callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE, {
