...
brief: The 'shieldnet-defend-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the ShieldnetDefend component (agent or manager) starts when
       the 'location' tag is set in the configuration, and the ShieldnetDefend API returns the same values for
...
from shieldnet_defend_testing.modules.agentd import configuration as agentd_configuration
from shieldnet_defend_testing.modules.logcollector import patterns
from shieldnet_defend_testing.modules.logcollector import utils
...
# Configuration
default_config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_basic_configuration_log_format_location.yaml')
...
def test_configuration_location(test_configuration, test_metadata, truncate_monitored_files, configure_local_internal_options,
                                set_shieldnet_defend_configuration, daemons_handler, wait_for_logcollector_start):
    '''
    description: Check if the 'shieldnet-defend-logcollector' daemon starts properly when the 'location' tag is used.
                 ...
                 ShieldnetDefend component is started by checking its process, and the ShieldnetDefend API returns the same
    ...
    assertions:
        - Verify that the ShieldnetDefend component (agent or manager) can start when the 'location' tag is used.
        - Verify that the ShieldnetDefend API returns the same value for the 'localfile' section as the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_location) is contained in an external
                       YAML file (shieldnet_defend_basic_configuration.yaml). That template is combined with different
                       test cases defined in the module. Those include configuration settings for
                       the 'shieldnet-defend-logcollector' daemon.

    expected_output:
        - Boolean values to indicate the state of the ShieldnetDefend component.
    ...
    if sys.platform == WINDOWS:
        if test_metadata['location'] == 'invalidchannel':
            ...
            shieldnet_defend_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT, callback=callback)
            assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_EVENTCHANNEL
    else:
