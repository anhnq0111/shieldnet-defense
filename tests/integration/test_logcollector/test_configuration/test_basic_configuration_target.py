...
references:
    ...
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/localfile.html#out-format
...
config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_basic_configuration_target.yaml')
...
def test_configuration_target(test_configuration, test_metadata, set_shieldnet_defend_configuration, configure_local_internal_options, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'shieldnet-defend-logcollector' daemon detects invalid configurations for the 'target' attribute
                 ...
                 the ShieldnetDefend API returns the same values for the 'localfile' section that the configured one.
    ...
    input_description: A configuration template (test_basic_configuration_target) is contained in an external
                       ...
                       for the 'shieldnet-defend-logcollector' daemon.
    ...
    shieldnet_defend_log_monitor.start(callback, timeout=10)
    assert shieldnet_defend_log_monitor.callback_result, assert_error
