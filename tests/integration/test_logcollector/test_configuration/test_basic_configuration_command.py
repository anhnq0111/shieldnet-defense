...
brief: The 'shieldnet-defend-logcollector' daemon monitors configured files and commands for new log messages.
       ...
       correctly executed by the logcollector, and the ShieldnetDefend API returns the same values for
...
references:
    ...
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/localfile.html#command
...
config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_basic_configuration_command.yaml')
...
def test_configuration_command(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if the 'shieldnet-defend-logcollector' daemon can monitor commands that use multiple parameters.
    ...
    shieldnet_defend_log_monitor.start(callback=callback)
