...
brief: The 'shieldnet-defend-logcollector' daemon monitors configured files and commands for new log messages.
       Specifically, these tests will check if the logcollector detects invalid values for
       the 'log_format' tag and the ShieldnetDefend API returns the same values for the configured
...
references:
    - https://documentation.shieldnetdefend.com/current/user-manual/capabilities/log-data-collection/index.html
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/localfile.html#log-format
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/localfile.html#location
...
from shieldnet_defend_testing.constants.paths.logs import SHIELDNET_DEFEND_LOG_PATH, MACOS_LOG_COMMAND_PATH
...
from shieldnet_defend_testing.modules.logcollector import configuration as logcollector_configuration
from shieldnet_defend_testing.modules.logcollector import patterns, PREFIX
...
default_config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_basic_configuration_log_format.yaml')
default_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format.yaml')

macos_duplicated_config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_duplicated_macos_configuration.yaml')
macos_duplicated_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_duplicated.yaml')

macos_no_defined_config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_no_defined_location_macos_configuration.yaml')
macos_no_defined_cases_path = Path(TEST_CASES_PATH, 'cases_basic_configuration_log_format_macos_no_defined.yaml')

win_config_path = Path(CONFIGURATIONS_PATH, 'shieldnet_defend_basic_configuration_log_format_location.yaml')
...
def check_log_format_valid(test_configuration, test_metadata):
    """Check if ShieldnetDefend run correctly with the specified log formats.
    ...
    if test_metadata['log_format'] not in log_format_not_print_analyzing_info:
        shieldnet_defend_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_ANALYZING_FILE,
                                                   {'file': test_metadata['location']}))
        assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_FILE
        ...
        if test_metadata['log_format'] == 'full_command':
            ...
            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MONITORING_COMMAND, {
                                'command': test_metadata['command']})
        shieldnet_defend_log_monitor.start(timeout=5, callback=callback)
        assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_COMMAND_MONITORING
        ...
        assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_DJB_MULTILOG_NOT_PRODUCED
        ...
        if 'location' in test_metadata and test_metadata['location'] != 'macos':
            ...
            assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_INVALID_MACOS_VALUE
        if 'location' not in test_metadata:
            shieldnet_defend_log_monitor.start(timeout=5,
                                    callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MISSING_LOCATION))
            assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_MISSING_LOCATION_VALUE

        shieldnet_defend_log_monitor.start(timeout=5,
                                callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                   {'command_path': MACOS_LOG_COMMAND_PATH}))
        assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_MACOS_LOG_NOT_PRODUCED


def check_log_format_invalid(test_metadata):
    """Check if ShieldnetDefend fails because a invalid frequency configuration value.
    ...
    shieldnet_defend_log_monitor.start(timeout=5, callback=log_callback)
    assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_GENERIC_MESSAGE
...
def check_log_file_duplicated():
    """Check if ShieldnetDefend shows a warning message when the configuration is duplicated."""
    shieldnet_defend_log_monitor = file_monitor.FileMonitor(SHIELDNET_DEFEND_LOG_PATH)
    shieldnet_defend_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_LOG_FILE_DUPLICATED))
    assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_LOG_FILE_DUPLICATED
    shieldnet_defend_log_monitor.start(timeout=LOG_COLLECTOR_GLOBAL_TIMEOUT,
                            callback=callbacks.generate_callback(patterns.LOGCOLLECTOR_MACOS_MONITORING_LOGS,
                                                                 {'command_path': MACOS_LOG_COMMAND_PATH}))
    assert (shieldnet_defend_log_monitor.callback_result != None), patterns.ERROR_ANALYZING_MACOS
...
def test_log_format(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
                    set_shieldnet_defend_configuration, daemons_handler_module, stop_logcollector):
    '''
    description: Check if the 'shieldnet-defend-logcollector' daemon detects invalid configurations for the 'log_format' tag.
                 ...
                 Finally, the test will verify that the ShieldnetDefend API returns the same values for the 'localfile' section
    ...
    assertions:
        ...
        - Verify that the ShieldnetDefend API returns the same values for the 'localfile' section as the configured one.


    input_description: A configuration templates (test_basic_configuration_log_format) are contained in externals
                       YAML files (shieldnet_defend_basic_configuration.yaml, shieldnet_defend_duplicated_macos_configuration.yaml, and
                       shieldnet_defend_no_defined_location_macos_configuration.yaml). Those templates are combined with
                       different test cases defined in the module. Those include configuration settings for
                       the 'shieldnet-defend-logcollector' daemon.
    ...
    if test_metadata['valid_value']:
        ...
        if sys.platform == WINDOWS:
            pytest.xfail("Windows agent allows invalid localfile configuration:\
                          https://github.com/shieldnetdefend/shieldnetdefend/issues/10890")
