...
from shieldnet_defend_testing.constants.paths.logs import ACTIVE_RESPONSE_LOG_PATH, SHIELDNET_DEFEND_LOG_PATH
from shieldnet_defend_testing.constants.platforms import WINDOWS
from shieldnet_defend_testing.modules.execd.active_response.patterns import ACTIVE_RESPONSE_RESTART_SHIELDNET_DEFEND
...
from shieldnet_defend_testing.modules.execd.patterns import EXECD_SHUTDOWN_RECEIVED
...
ar_conf = 'restart-shieldnet-defend0 - restart-shieldnet-defend - 0\nrestart-shieldnet-defend0 - restart-shieldnet-defend.exe - 0'
...
def test_execd_restart(test_configuration, test_metadata, configure_local_internal_options, truncate_monitored_files,
    ...
    '''
    description: Check if 'restart-shieldnet-defend' command of 'active response' is executed correctly.
                 For this purpose, a simulated server is used, from which the active response is sent.
                 This response includes the order to restart the ShieldnetDefend agent, which must restart after
    ...
    assertions:
        ...
        - Check the restart-shieldnet-defend program is used.
    input_description:
    ...
    if error_message := test_metadata.get('expected_error'):
        ...
        assert ar_monitor.callback_result, 'AR `shieldnet-defend-restart` did not fail.'
        return

    shieldnet_defend_log_monitor.start(callback=generate_callback(EXECD_SHUTDOWN_RECEIVED))
    assert shieldnet_defend_log_monitor.callback_result, 'Execd `shutdown` log not raised.'

    ar_monitor.start(callback=generate_callback(ACTIVE_RESPONSE_RESTART_SHIELDNET_DEFEND))
    assert ar_monitor.callback_result, 'AR `restart-shieldnet-defend` program not used.'
