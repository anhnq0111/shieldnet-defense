...
def test_configuration_file(test_configuration, test_metadata, set_shieldnet_defend_configuration, daemons_handler):
    '''
    description: Checks if `shieldnet-defend-logtest` works as expected under different predefined configurations that cause
                 `shieldnet-defend-logtest` to start correctly, to be disabled, or to register an error. To do this, it checks
                 some values in these configurations from 'shieldnet_defend_conf.yaml' file.
    ...
    shieldnet_defend_log_monitor.start(callback=generate_callback(callback), timeout=session_parameters.default_timeout)
    assert shieldnet_defend_log_monitor.callback_result, 'Event not found'
