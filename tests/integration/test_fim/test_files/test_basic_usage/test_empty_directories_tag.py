...
from shieldnet_defend_testing.modules.fim.patterns import EMPTY_DIRECTORIES_TAG
...
def test_empty_directories_tag(test_configuration, test_metadata, set_shieldnet_defend_configuration, configure_local_internal_options,
                               truncate_monitored_files, daemons_handler):
    '''
    description: Check if the 'shieldnet-defend-syscheckd' daemon shows a debug message when an empty 'directories' tag is found.
    ...
    assertions:
        - Verify that the 'shieldnet-defend-syscheckd' daemon generates a debug log when
    ...
    # Assert
    shieldnet_defend_log_monitor.start(generate_callback(EMPTY_DIRECTORIES_TAG))
