...
references:
    ...
    - https://documentation.shieldnetdefend.com/current/user-manual/reference/ossec-conf/syscheck.html#ignore
...
import pytest
from shieldnet_defend_testing import global_parameters
...
from shieldnet_defend_testing.modules.fim.patterns import EVENT_TYPE_ADDED, IGNORING_DUE_TO_SREGEX, IGNORING_DUE_TO_PATTERN
...
# Skipped on Windows due Issue https://github.com/shieldnetdefend/shieldnetdefend/issues/9298"
...
def test_ignore_subdirectory(test_configuration, test_metadata, configure_local_internal_options,
                             ...
                             file_to_monitor):
    '''
    description: Check if the 'shieldnet-defend-syscheckd' daemon ignores the files that are in a monitored subdirectory
    ...
    input_description: Different test cases are contained in external YAML files
                       ...
                       for the 'shieldnet-defend-syscheckd' daemon and testing directories to monitor.
    ...
    if test_metadata.setdefault('xfail', False):
        pytest.xfail(reason="Expected fail - Issue https://github.com/shieldnetdefend/shieldnetdefend/issues/22186.")

    shieldnet_defend_log_monitor = FileMonitor(SHIELDNET_DEFEND_LOG_PATH)

    if test_metadata.setdefault('triggers_event', False) == True:
        shieldnet_defend_log_monitor.start(timeout=60, callback=generate_callback(EVENT_TYPE_ADDED))
        callback_result = shieldnet_defend_log_monitor.callback_result
        ...
        if test_metadata['is_pattern'] == False:
            shieldnet_defend_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_SREGEX))
            assert shieldnet_defend_log_monitor.callback_result
        else:
            shieldnet_defend_log_monitor.start(callback=generate_callback(IGNORING_DUE_TO_PATTERN))
            assert shieldnet_defend_log_monitor.callback_result
