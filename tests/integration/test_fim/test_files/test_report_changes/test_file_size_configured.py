...
from shieldnet_defend_testing.modules.fim.patterns import DIFF_MAXIMUM_FILE_SIZE, ERROR_MSG_MAXIMUM_FILE_SIZE_EVENT, ERROR_MSG_WRONG_VALUE_MAXIMUM_FILE_SIZE
...
def test_diff_size_limit(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'shieldnet-defend-syscheckd' daemon limits the size of 'diff' information to generate from
    ...
    shieldnet_defend_log_monitor.start(generate_callback(DIFF_MAXIMUM_FILE_SIZE), timeout=30)
    callback_result = shieldnet_defend_log_monitor.callback_result
    assert callback_result, ERROR_MSG_MAXIMUM_FILE_SIZE_EVENT
    assert str(shieldnet_defend_log_monitor.callback_result[0]) == test_metadata.get('diff_size_limit_kb'), ERROR_MSG_WRONG_VALUE_MAXIMUM_FILE_SIZE
