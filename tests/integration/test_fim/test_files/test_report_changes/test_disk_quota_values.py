...
from shieldnet_defend_testing.utils.random import get_random_string
...
def test_disk_quota_values(test_configuration, test_metadata, configure_local_internal_options,
    ...
    '''
    description: Check if the 'shieldnet-defend-syscheckd' daemon limits the size of the diff folder to generate
    ...
    shieldnet_defend_log_monitor.start(callback=generate_callback(DISK_QUOTA_LIMIT_REACHED), timeout=30)
    ...
    assert shieldnet_defend_log_monitor.callback_result, ERROR_MSG_REPORT_CHANGES_EVENT_NOT_DETECTED
    assert 'Unable to calculate diff due to \'disk_quota\' limit has been reached.' in shieldnet_defend_log_monitor.callback_result, 'Wrong content_changes field'
