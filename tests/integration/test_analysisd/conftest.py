...
from shieldnet_defend_testing.constants.paths.configurations import CUSTOM_RULES_PATH, CUSTOM_RULES_FILE, SHIELDNET_DEFEND_CONF_PATH
from shieldnet_defend_testing.constants.paths.logs import ALERTS_JSON_PATH, SHIELDNET_DEFEND_LOG_PATH
from shieldnet_defend_testing.constants.users import SHIELDNET_DEFEND_UNIX_GROUP, SHIELDNET_DEFEND_UNIX_USER
...
from shieldnet_defend_testing.utils import callbacks, file


@pytest.fixture()
def prepare_custom_rules_file(request, test_metadata):
    """Configure a syscollector custom rules for testing.
    Restarting shieldnet-defend-analysisd is required to apply this changes.
    """
    ...
    shutil.chown(target_rule, SHIELDNET_DEFEND_UNIX_USER, SHIELDNET_DEFEND_UNIX_GROUP)
...
def configure_local_rules(request, test_configuration):
    """Configure a custom rule for testing. Restart ShieldnetDefend is needed for applying the configuration. """
...
def configure_remove_tags(request, test_metadata):
    """Configure a custom settting for testing. Restart ShieldnetDefend is needed for applying the configuration. """
    # Remove test case tags from ossec.conf
    file.replace_regex_in_file(test_metadata['remove_tags'], [''] * len(test_metadata['remove_tags']), SHIELDNET_DEFEND_CONF_PATH)
