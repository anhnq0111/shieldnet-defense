...
This module uses a stateless design to process incoming messages and index them in the ShieldnetDefend Indexer. It integrates several design patterns (Facade, Factory Method, and Chain of Responsibility) to modularize responsibilities and simplify maintenance. Below is an overview of the main components and their roles.

## Main Components

- **`src/shieldnet_defend_modules/inventory_harvester/src/inventoryHarvester.cpp`**
...
- **`src/shieldnet_defend_modules/inventory_harvester/src/inventoryHarvesterFacade.cpp`**
...
- **`src/shieldnet_defend_modules/inventory_harvester/src/common/`**
  A folder containing common operations used by the `InventoryHarvester` module:

  - **`clearAgent`**: Removes all data related to an agent (when the agent is removed from the manager) by sending a `DELETED_BY_QUERY` message to the ShieldnetDefend Indexer.
  ...
  - **`indexSync`**: Synchronizes indices with the ShieldnetDefend Indexer.
...
- **`src/shieldnet_defend_modules/inventory_harvester/src/systemInventory/`**
...
- **`src/shieldnet_defend_modules/inventory_harvester/src/wcsModel/`**
  Contains schema definitions for the **ShieldnetDefend Common Schema (WCS)** models. By adhering to WCS, the InventoryHarvester ensures consistency and compatibility across ShieldnetDefend modules. The models capture essential information (system inventory) and use a **JSON reflection mechanism** to convert internal data structures into JSON for the ShieldnetDefend Indexer.
...
subgraph ShieldnetDefendManager["ShieldnetDefend Manager"]
  Remoted["Remoted"]
  subgraph ShieldnetDefendModulesM["ShieldnetDefend Modules"]
...
IndexerConnector -- indexes JSON document --> ShieldnetDefendIndexer
subgraph ShieldnetDefendAgent["ShieldnetDefend Agent"]
  ...
  subgraph ShieldnetDefendModulesA["shieldnet-defend-modulesd"]
    SystemInventory["System inventory"]
  end
  subgraph Syscheckd["shieldnet-defend-syscheckd"]
...
ShieldnetDefendIndexer["ShieldnetDefend Indexer"]
ShieldnetDefendDashboard["ShieldnetDefend Dashboard"]
ShieldnetDefendDashboard -- /_search/dedicated_index --> ShieldnetDefendIndexer
```
