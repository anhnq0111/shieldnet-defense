...
cp -rf /tmp_volume/config/* /var/ossec/ && chown -R shieldnetdefend:shieldnetdefend /var/ossec/api
...
if [ $4 == "standalone" ]; then
  ...
  sed -i "s:validate_responses=False:validate_responses=True:g" /var/ossec/api/scripts/shieldnet_defend_apid.py
fi
...
chown -R shieldnetdefend:shieldnetdefend /var/ossec/queue/db
chown -R shieldnetdefend:shieldnetdefend /var/ossec/etc/shared
chmod --reference=/var/ossec/etc/shared/default /var/ossec/etc/shared/group*
cd /var/ossec/etc/shared && find -name merged.mg -exec chown shieldnetdefend:shieldnetdefend {} \; && cd /
chown root:shieldnetdefend /var/ossec/etc/shared/ar.conf
...
/var/ossec/bin/shieldnet-defend-control start
...
if [ "$3" == "master" ]; then
  ...
  # Wait until ShieldnetDefend API is ready
  elapsed_time=0
