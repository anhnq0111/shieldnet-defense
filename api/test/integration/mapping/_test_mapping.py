...
framework = path.join(base, 'framework', 'shieldnetdefend')
...
# ShieldnetDefend modules
shieldnet_defend_modules = [
    api,  # API
...
def extract_module_from_path(file_path):
    ...
    shieldnet_defend_modules = parts[-1]
    return shieldnet_defend_modules
...
if __name__ == '__main__':
    ...
    if len(sys.argv) == 1:
        ...
        mapping_list = list()
        for module in shieldnet_defend_modules:
            chdir(module)
    ...
    # Calculate mappings for a given file
        ...
        if not path.exists(path.join(base, file_rel_path)):
            print('The relative path does not exist. Example: "framework/shieldnetdefend/agent.py"')
            exit(0)
