...
SOCKET_PATH_DAEMONS_MAPPING = {'/var/ossec/queue/sockets/remote': 'shieldnet-defend-remoted',
                               '/var/ossec/queue/sockets/analysis': 'shieldnet-defend-analysisd'}
DAEMON_SOCKET_PATHS_MAPPING = {'shieldnet-defend-remoted': '/var/ossec/queue/sockets/remote',
                               'shieldnet-defend-analysisd': '/var/ossec/queue/sockets/analysis'}
...
def test_totals():
    ...
    with patch('shieldnetdefend.stats.totals_', return_value=({})):
        response = stats.totals(date(2019, 8, 13))
        assert response.total_affected_items == len(response.affected_items)
        assert isinstance(response, AffectedItemsShieldnetDefendResult), 'The result is not ShieldnetDefendResult type'
...
@patch('shieldnetdefend.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('shieldnetdefend.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('shieldnetdefend.core.common.WDB_SOCKET', '/var/ossec/queue/db/wdb')
@patch('shieldnetdefend.stats.get_daemons_stats_socket')
def test_get_daemons_stats(mock_get_daemons_stats_socket):
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['shieldnet-defend-remoted', 'shieldnet-defend-analysisd', 'shieldnet-defend-db'])
    ...
    assert isinstance(response, AffectedItemsShieldnetDefendResult), 'The result is not AffectedItemsShieldnetDefendResult type'
    assert response.total_affected_items == len(response.affected_items)


@patch('shieldnetdefend.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/wrong_socket_name')
def test_get_daemons_stats_ko():
    """Makes sure get_daemons_stats() fit with the expected."""
    response = stats.get_daemons_stats(['shieldnet-defend-remoted'])
...
@pytest.mark.parametrize('daemons_list, expected_daemons_list', [
    ([], ['shieldnet-defend-remoted', 'shieldnet-defend-analysisd']),
    (['shieldnet-defend-remoted'], ['shieldnet-defend-remoted']),
    (['shieldnet-defend-remoted', 'shieldnet-defend-analysisd'], ['shieldnet-defend-remoted', 'shieldnet-defend-analysisd'])
])
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
@patch('shieldnetdefend.stats.get_agents_info', return_value={'000', '001', '002', '003', '004', '005'})
@patch('shieldnetdefend.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('shieldnetdefend.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('shieldnetdefend.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats)
def test_get_daemons_stats_agents(mock_get_daemons_stats_socket, mock_get_agents_info, mock_socket_connect,
    ...
    assert isinstance(result, AffectedItemsShieldnetDefendResult), 'The result is not an AffectedItemsShieldnetDefendResult object'
...
@patch('shieldnetdefend.core.common.REMOTED_SOCKET', '/var/ossec/queue/sockets/remote')
@patch('shieldnetdefend.core.common.ANALYSISD_SOCKET', '/var/ossec/queue/sockets/analysis')
@patch('shieldnetdefend.stats.get_daemons_stats_socket', side_effect=side_effect_test_get_daemons_stats_all)
...
@patch('shieldnetdefend.stats.get_daemons_stats_', return_value=[{"events_decoded": 1.0}])
...
@patch('shieldnetdefend.core.agent.Agent.get_stats')
@patch('shieldnetdefend.stats.get_agents_info', return_value=['000', '001'])
