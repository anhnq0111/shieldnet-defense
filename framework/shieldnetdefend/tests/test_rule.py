...
from shieldnetdefend.core.common import USER_RULES_PATH
import pytest
...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend import rule
...
def mock_shieldnet_defend_paths():
    with patch('shieldnetdefend.core.common.RULES_PATH', new=os.path.join(parent_directory, core_data_path)):
        with patch('shieldnetdefend.core.common.USER_RULES_PATH', new=os.path.join(parent_directory, tests_data_path)):
            with patch('shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH', new=parent_directory):
                with patch('shieldnetdefend.rule.to_relative_path', side_effect=lambda x: os.path.relpath(x, parent_directory)):
                    yield
...
@patch("shieldnetdefend.rule.configuration.get_ossec_conf", return_value=rule_ossec_conf)
def test_get_rules_files_status_include(mock_ossec, status, func):
    ...
    if status == 'random':
        ...
        with pytest.raises(ShieldnetDefendError, match='.* 1202 .*'):
            func(status=status)
    else:
...
@patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=rule_ossec_conf)
...
@patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=None)
def test_failed_get_rules_file(mock_config):
    """
    Test failed get_rules_file function when ossec.conf don't have ruleset section
    """
    with pytest.raises(ShieldnetDefendError, match=".* 1200 .*"):
        rule.get_rules_files()
...
@patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=other_rule_ossec_conf)
...
def test_failed_get_rules():
    ...
    with pytest.raises(ShieldnetDefendError, match=".* 1203 .*"):
...
def test_get_rule_file_path(filename, relative_dirname, result, mock_shieldnet_defend_paths):
    """Test get_rule_file_path function."""
    with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        res = rule.get_rule_file_path(filename=filename,
                                            relative_dirname=relative_dirname)
        assert res == os.path.join(shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH, result) if result else not res
...
def test_get_rule_file(filename, raw, relative_dirname, contains):
    ...
    with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        if raw:
            ...
            assert isinstance(d_files, AffectedItemsShieldnetDefendResult)
...
@patch('shieldnetdefend.rule.delete_rule_file')
@patch('shieldnetdefend.rule.full_copy')
@patch('shieldnetdefend.rule.upload_file')
@patch('shieldnetdefend.rule.remove')
@patch('shieldnetdefend.rule.safe_move')
@patch('shieldnetdefend.rule.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_xml, mock_full_copy,
    ...
    with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('shieldnetdefend.rule.validate_upload_delete_dir', return_value=ret_validation):
            with patch('shieldnetdefend.rule.exists', return_value=overwrite):
                ...
                if overwrite:
                    full_path = os.path.join(shieldnetdefend.common.SHIELDNET_DEFEND_PATH, rule_path)
...
@patch('shieldnetdefend.rule.delete_rule_file', side_effect=ShieldnetDefendError(1019))
@patch('shieldnetdefend.rule.upload_file')
@patch('shieldnetdefend.rule.safe_move')
...
def test_delete_rule_file(file, relative_dirname):
    ...
    with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        with patch('shieldnetdefend.rule.exists', return_value=True):
            with patch('shieldnetdefend.rule.remove'):
...
def test_delete_rule_file_ko():
    ...
    with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=get_rule_file_ossec_conf):
        ...
        with patch('shieldnetdefend.rule.remove', side_effect=IOError()):
