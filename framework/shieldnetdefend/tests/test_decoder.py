...
import pytest
from shieldnetdefend.core.common import USER_DECODERS_PATH


with patch('shieldnetdefend.core.common.getgrnam'):
    with patch('shieldnetdefend.core.common.getpwnam'):
        ...
        from shieldnetdefend import decoder
...
def mock_shieldnet_defend_paths():
    """Mock shieldnetdefend paths."""
    with patch('shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH', new=test_data_path):
        with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=decoder_ossec_conf):
            with patch('shieldnetdefend.core.common.DECODERS_PATH',
                       new=os.path.join(test_data_path, "tests", "data", "decoders")):
                with patch('shieldnetdefend.core.common.USER_DECODERS_PATH',
                           new=os.path.join(test_data_path, "tests", "data", "etc", "decoders")):
                    with patch('shieldnetdefend.decoder.to_relative_path',
                               side_effect=lambda x: os.path.relpath(x, shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH)):
                        yield
...
@pytest.mark.parametrize('names, status, filename, relative_dirname, parents, expected_names, expected_total_failed', [
    (None, None, None, None, False, {'agent-buffer', 'json', 'agent-upgrade', 'shieldnetdefend', 'agent-restart'}, 0),
    (['agent-buffer'], None, None, None, False, {'agent-buffer'}, 0),
    (['agent-buffer', 'non_existing'], None, None, None, False, {'agent-buffer'}, 1),
    (None, 'enabled', None, None, False, {'agent-buffer', 'agent-upgrade', 'shieldnetdefend', 'agent-restart'}, 0),
    (None, 'disabled', None, None, False, {'json'}, 0),
    (['agent-upgrade', 'non_existing', 'json'], 'enabled', None, None, False, {'agent-upgrade'}, 1),
    (None, None, 'test1_decoders.xml', None, False, {'agent-buffer', 'agent-upgrade', 'shieldnetdefend', 'agent-restart'}, 0),
    (None, None, 'test2_decoders.xml', 'tests/data/decoders', False, {'json'}, 0),
    (None, 'all', None, 'tests/data/decoders', True, {'shieldnetdefend', 'json'}, 0),
...
@pytest.mark.parametrize('conf, exception', [
    ...
    ({'ruleset': None}, ShieldnetDefendInternalError(1500))
])
def test_get_decoders_files(conf, exception):
    """Test get_decoders_files function"""
    with patch('shieldnetdefend.core.configuration.get_ossec_conf', return_value=conf):
        try:
            # UUT call
        ...
        except ShieldnetDefendInternalError as exc:
...
def test_get_decoder_file_path(filename, relative_dirname, result, mock_shieldnet_defend_paths):
    """Test get_decoder_file_path function."""
    res = decoder.get_decoder_file_path(filename=filename,
                                         relative_dirname=relative_dirname)
    assert res == os.path.join(shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH, result) if result else not res and isinstance(res, str)
...
def test_get_decoder_file(filename, raw, relative_dirname, contains, mock_shieldnet_defend_paths):
    ...
    if raw:
        ...
        # Assert the result is an AffectedItemsShieldnetDefendResult
        assert isinstance(result, AffectedItemsShieldnetDefendResult)
...
@patch('shieldnetdefend.decoder.delete_decoder_file')
@patch('shieldnetdefend.decoder.full_copy')
@patch('shieldnetdefend.decoder.validate_shieldnet_defend_xml')
@patch('shieldnetdefend.decoder.upload_file')
@patch('shieldnetdefend.decoder.remove')
@patch('shieldnetdefend.decoder.safe_move')
@patch('shieldnetdefend.decoder.validate_dummy_logtest')
def test_upload_file(mock_logtest, mock_safe_move, mock_remove, mock_upload_file,
                     mock_xml, mock_full_copy, mock_delete, mock_shieldnet_defend_paths,
    ...
    with patch('shieldnetdefend.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('shieldnetdefend.decoder.exists', return_value=overwrite):
            with patch('shieldnetdefend.decoder.to_relative_path',
                       side_effect=lambda x: os.path.relpath(x, shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH)):
            ...
            if overwrite:
                full_path = os.path.join(shieldnetdefend.common.SHIELDNET_DEFEND_PATH, decoder_path)
...
@patch('shieldnetdefend.decoder.delete_decoder_file', side_effect=ShieldnetDefendError(1019))
@patch('shieldnetdefend.decoder.upload_file')
@patch('shieldnetdefend.decoder.safe_move')
...
def test_upload_file_ko(*_):
    ...
    with patch('shieldnetdefend.decoder.validate_upload_delete_dir', return_value=ret_validation):
        with patch('shieldnetdefend.decoder.exists'):
    ...
    # clean backup files
    search_pattern = os.path.join(shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH, "**", "*.backup")
...
def test_delete_decoder_file(filename, relative_dirname):
    ...
    with patch('shieldnetdefend.decoder.exists', return_value=True):
        # Assert returned type is AffectedItemsShieldnetDefendResult when everything is correct
        with patch('shieldnetdefend.decoder.remove'):
...
def test_delete_decoder_file_ko():
    ...
    with patch('shieldnetdefend.decoder.exists', return_value=True):
        with patch('shieldnetdefend.decoder.remove', side_effect=IOError()):
