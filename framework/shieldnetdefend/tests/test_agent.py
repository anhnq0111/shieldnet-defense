...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.agent import add_agent, assign_agents_to_group, create_group, delete_agents, delete_groups, \
        ...
        from shieldnetdefend.core.agent import Agent
        ...
        from shieldnetdefend.core.tests.test_agent import InitAgent
        from api.util import remove_nones_to_dict
        from shieldnetdefend.core.exception import ShieldnetDefendResourceNotFound
        from shieldnetdefend.core.wdb_http import AgentsSummary
...
@pytest.mark.parametrize('fields, expected_items', [
    ...
    (
            ['version'],
            [{'version': 'ShieldnetDefend v3.9.0', 'count': 1}, {'version': 'ShieldnetDefend v3.8.2', 'count': 2},
             {'version': 'ShieldnetDefend v3.6.2', 'count': 1}, {'version': 'N/A', 'count': 2}]
    ),
    (
...
@patch('shieldnetdefend.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
...
def test_agent_get_distinct_agents(socket_mock, send_mock, fields, expected_items):
    ...
    assert isinstance(distinct, AffectedItemsShieldnetDefendResult), 'The returned object is not an "AffectedItemsShieldnetDefendResult".'
...
def test_agent_sort_order(socket_mock, send_mock, fields, order, expected_items):
    ...
    assert isinstance(sorted_agents, AffectedItemsShieldnetDefendResult), 'The returned object is not an ' \
...
def test_agent_get_agents_summary_status(socket_mock, send_mock):
    ...
    assert isinstance(summary, ShieldnetDefendResult), 'The returned object is not an "ShieldnetDefendResult" instance.'
...
def test_agent_get_agents_summary_os(connect_mock, send_mock):
    ...
    assert isinstance(summary, AffectedItemsShieldnetDefendResult), 'The returned object is not an "ShieldnetDefendResult" instance.'
...
@patch('shieldnetdefend.core.agent.Agent.reconnect')
@patch('shieldnetdefend.agent.get_agents_info', return_value=short_agent_list)
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_agents(socket_mock, send_mock, agents_info_mock, reconnect_mock, agent_list, expected_items,
    ...
    assert isinstance(result, AffectedItemsShieldnetDefendResult), 'The returned object is not an "AffectedItemsShieldnetDefendResult".'
...
@patch('shieldnetdefend.agent.send_restart_command')
@patch('shieldnetdefend.agent.get_agents_info', return_value=set(short_agent_list))
@patch('shieldnetdefend.core.wdb_http.ShieldnetDefendDBHTTPClient._post', side_effect=send_msg_to_wdb_http_post_restartinfo)
@patch('socket.socket.connect')
...
@patch('shieldnetdefend.agent.get_agents')
@patch('shieldnetdefend.agent.get_groups')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('group, q, expected_q', [
    ('default', '(name~shieldnetdefend,status~active)', 'group=default;((name~shieldnetdefend,status~active))'),
    ('default', 'name~shieldnetdefend,status~active', 'group=default;(name~shieldnetdefend,status~active)')
])
@patch('shieldnetdefend.agent.get_agents')
@patch('shieldnetdefend.agent.get_groups')
...
@pytest.mark.parametrize('agent_list, filters, q, error_code, expected_items', [
    ...
    (full_agent_list[1:], {'status': 'all', 'older_than': '1s', 'version': 'ShieldnetDefend v3.6.2'}, None, 1731, ['002']),
    ...
    (['001', '002'], {'status': 'all', 'older_than': '1s'}, None, ShieldnetDefendError(1726), None),
])
@patch('shieldnetdefend.agent.Agent.remove')
@patch('shieldnetdefend.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_delete_agents(socket_mock, send_mock, mock_remove, agent_list, filters, q, error_code, expected_items):
    ...
    """
    if not isinstance(error_code, ShieldnetDefendException):
...
@patch('shieldnetdefend.core.agent.get_manager_status', return_value={'shieldnet-defend-authd': 'running'})
...
@patch('shieldnetdefend.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
def test_agent_get_agent_groups_exceptions(socket_mock, send_mock, mock_get_groups, db_global, system_groups,
    ...
    with patch('shieldnetdefend.core.common.DATABASE_PATH_GLOBAL', new=db_global):
        try:
...
@patch('shieldnetdefend.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('shieldnetdefend.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
...
@pytest.mark.parametrize('shared_path, group_list, group_exists, side_effect, expected_exception', [
    (test_shared_path, ['none'], False, None, ShieldnetDefendResourceNotFound(1710)),
    ('invalid-path', ['default'], True, None, ShieldnetDefendError(1006)),
    (test_shared_path, ['default'], True, ShieldnetDefendError(1405), ShieldnetDefendError(1405)),
    (test_shared_path, ['default'], True, ShieldnetDefendException(1400), ShieldnetDefendInternalError(1727))
])
@patch('shieldnetdefend.agent.process_array')
@patch('shieldnetdefend.core.agent.Agent.group_exists')
def test_agent_get_group_files_exceptions(mock_group_exists, mock_process_array, shared_path, group_list, group_exists,
    ...
    """
    with patch('shieldnetdefend.core.common.SHARED_PATH', new=shared_path):
        ...
        except (ShieldnetDefendError, ShieldnetDefendInternalError) as e:
...
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
@patch('shieldnetdefend.core.common.shieldnet_defend_gid', return_value=getgrnam('root'))
@patch('shieldnetdefend.core.common.shieldnet_defend_uid', return_value=getpwnam('root'))
@patch('shieldnetdefend.agent.chown_r')
def test_create_group(chown_mock, uid_mock, gid_mock, group_id):
    ...
    try:
        ...
        assert isinstance(result, ShieldnetDefendResult), 'The returned object is not an "ShieldnetDefendResult" instance.'
...
@pytest.mark.parametrize('group_id, exception, exception_code', [
    ('default', ShieldnetDefendError, 1711),
    ('group-1', ShieldnetDefendError, 1711),
    ('invalid!', ShieldnetDefendError, 1722),
    ('delete-me', ShieldnetDefendInternalError, 1005),
    ('ar.conf', ShieldnetDefendError, 1713),
    ('agent-template.conf', ShieldnetDefendError, 1713)
])
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
def test_create_group_exceptions(group_id, exception, exception_code):
    ...
    exception_code : int
        Expected error code for the ShieldnetDefend Exception object raised by `get_group_files` with the given parameters.
    """
    try:
...
@patch('shieldnetdefend.agent.get_groups')
@patch('shieldnetdefend.agent.Agent.delete_single_group')
...
@pytest.mark.parametrize('group_list, expected_errors', [
    (['none-1'], [ShieldnetDefendResourceNotFound(1710)]),
    (['default'], [ShieldnetDefendError(1712)]),
    (['none-1', 'none-2'], [ShieldnetDefendResourceNotFound(1710)]),
    (['default', 'none-1'], [ShieldnetDefendError(1712), ShieldnetDefendResourceNotFound(1710)]),
])
@patch('shieldnetdefend.agent.get_groups')
def test_agent_delete_groups_other_exceptions(mock_get_groups, group_list, expected_errors):
    ...
    group_list : List of str
        List of groups to be deleted.
    expected_errors : list of ShieldnetDefendError
        List of expected ShieldnetDefendError to be raised by delete_groups if a group is not valid. An exception will be returned
...
@patch('shieldnetdefend.agent.Agent.add_group_to_agent')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['none-1'], ['001'], ShieldnetDefendResourceNotFound(1710), True),
    (['group-1'], ['100'], ShieldnetDefendResourceNotFound(1701), False),
    (['default'], ['000'], ShieldnetDefendError(1703), False)
])
@patch('shieldnetdefend.agent.Agent.group_exists')
@patch('shieldnetdefend.agent.Agent.add_group_to_agent')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
async def test_agent_assign_agents_to_group_exceptions(socket_mock, send_mock, mock_add_group, mock_group_exists, group_list,
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : ShieldnetDefendError
    ...
    catch_exception : bool
        ...
        `AffectedItemsShieldnetDefendResult` containing the exceptions in its 'failed_items'.
    """
    ...
    except ShieldnetDefendException as ex:
...
@patch('shieldnetdefend.core.common.DATABASE_PATH_GLOBAL', new=test_global_bd_path)
@patch('shieldnetdefend.core.agent.Agent.unset_single_group_agent')
@patch('shieldnetdefend.agent.get_groups')
@patch('shieldnetdefend.agent.get_agents_info')
...
@pytest.mark.parametrize('group_id, agent_id, expected_error', [
    ('any-group', '100', ShieldnetDefendResourceNotFound(1701)),
    ('any-group', '000', ShieldnetDefendError(1703)),
    ('group-1', '005', ShieldnetDefendResourceNotFound(1710)),
])
@patch('shieldnetdefend.agent.get_agents_info', return_value=short_agent_list)
@patch('shieldnetdefend.agent.get_groups', side_effect={'default'})
async def test_agent_remove_agent_from_group_exceptions(group_mock, agents_info_mock, group_id, agent_id, expected_error):
    ...
    expected_error : ShieldnetDefendError
        The ShieldnetDefendError object expected to be raised by remove_agent_from_group with the given parameters.
    """
    try:
    ...
    except (ShieldnetDefendError, ShieldnetDefendResourceNotFound) as error:
...
@patch('shieldnetdefend.core.agent.Agent.unset_single_group_agent')
@patch('shieldnetdefend.agent.get_agents_info', return_value=short_agent_list)
@patch('shieldnetdefend.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['any-group'], ['100'], ShieldnetDefendResourceNotFound(1701), True),
    (['any-group'], ['000'], ShieldnetDefendError(1703), True),
    (['any-group'], ['005'], ShieldnetDefendResourceNotFound(1710), False),
])
@patch('shieldnetdefend.core.agent.Agent.unset_single_group_agent')
@patch('shieldnetdefend.agent.get_agents_info', return_value=short_agent_list)
@patch('shieldnetdefend.agent.get_groups', return_value={'group-1'})
async def test_agent_remove_agent_from_groups_exceptions(mock_get_groups, mock_get_agents, mock_unset, group_list, agent_list,
    ...
    try:
        ...
        assert not catch_exception, \
            'An "ShieldnetDefendError" exception was expected but was not raised.'
        # Check Typing
        assert isinstance(result, AffectedItemsShieldnetDefendResult), 'The returned object is not an "AffectedItemsShieldnetDefendResult".'
        assert isinstance(result.failed_items, dict), \
        ...
        assert catch_exception, \
            'No exception should be raised at this point. An AffectedItemsShieldnetDefendResult object with at least one ' \
...
@pytest.mark.parametrize('group_list, agent_list, expected_error, catch_exception', [
    (['non-group'], ['000'], ShieldnetDefendResourceNotFound(1710), True),
    (['group-1'], ['000'], ShieldnetDefendError(1703), False),
    (['group-1'], ['100'], ShieldnetDefendResourceNotFound(1701), False),
])
@patch('shieldnetdefend.agent.get_agents_info', return_value=short_agent_list)
@patch('shieldnetdefend.agent.get_groups', return_value={'group-1'})
...
@pytest.mark.parametrize('agent_set, expected_errors_and_items, result_from_socket, filters, raise_error', [
    ...
    (
            ...
            {'error': 1,
             'data': [{'error': 18,
                       'message': 'Error from socket indicating ShieldnetDefendInternalError',
                       'agent': 1}
                      ],
             'message': 'Error'},
            None,
...
@patch('shieldnetdefend.agent.get_agents_info', return_value=set(full_agent_list))
@patch('shieldnetdefend.core.common.CLIENT_KEYS', new=os.path.join(test_agent_path, 'client.keys'))
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_upgrade_agents(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    """
    with patch('shieldnetdefend.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        core_upgrade_agents_mock.return_value = result_from_socket

        if raise_error:
            # Upgrade expecting a ShieldnetDefend Exception
            for error in expected_errors_and_items.keys():
                if int(error) in ERROR_CODES_UPGRADE_SOCKET_BAD_REQUEST:
                    with pytest.raises(ShieldnetDefendError, match=f".* {error} .*"):
                    ...
                    with pytest.raises(ShieldnetDefendInternalError, match=f".* {error} .*"):
                        upgrade_agents(agent_list=list(agent_set), filters=filters)
        else:
...
def test_agent_get_upgrade_result(mock_socket, mock_wdb, mock_client_keys, agent_set, expected_errors_and_items,
    ...
    with patch('shieldnetdefend.core.agent.core_upgrade_agents') as core_upgrade_agents_mock:
        ...
        if raise_error:
            # Get upgrade result expecting a ShieldnetDefend Exception
            for error in expected_errors_and_items.keys():
                with pytest.raises(ShieldnetDefendInternalError, match=f".* {error} .*"):
...
def test_agent_get_agent_config(mock_exists, socket_mock, send_mock, shieldnet_defend_socket_mock, agent_list, component, configuration):
    ...
    """
    shieldnet_defend_socket_mock.return_value.receive.return_value = b'ok {"test": "conf"}'
...
def test_agent_get_agent_config_exceptions(socket_mock, send_mock, agent_list):
    ...
    except ShieldnetDefendError as error:
        assert error == ShieldnetDefendError(1740)
...
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
@patch('shieldnetdefend.core.common.MULTI_GROUPS_PATH', new=test_multigroup_path)
@patch('shieldnetdefend.agent.get_agents_info', return_value=full_agent_list)
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@pytest.mark.parametrize('agent_list, expected_error', [
    (['000'], ShieldnetDefendError(1703)),
    (['100'], ShieldnetDefendResourceNotFound(1701))
])
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_agents_sync_group_exceptions(socket_mock, send_mock, agent_list, expected_error):
    ...
    agent_list : List of str
        List of agent ID's.
    expected_error : ShieldnetDefendError
        Expected ShieldnetDefendError to be returned (not raised) by the function.
    """
    result = get_agents_sync_group(agent_list=agent_list)
    assert isinstance(result, AffectedItemsShieldnetDefendResult), 'The returned object is not an "AffectedItemsShieldnetDefendResult".'
    # Check failed items
...
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
@patch('shieldnetdefend.core.configuration.upload_group_configuration')
...
@patch('shieldnetdefend.core.common.SHARED_PATH', new=test_shared_path)
@patch('shieldnetdefend.agent.get_distinct_agents')
@patch('shieldnetdefend.agent.get_agent_groups')
@patch('shieldnetdefend.agent.get_agents_summary_status')
@patch('shieldnetdefend.agent.get_agents')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
...
@patch('shieldnetdefend.agent.get_agents_info', return_value=['test', 'test2'])
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_get_agents_big_env(mock_conn, mock_send, mock_get_agents, insert_agents_db, agent_list, params, expected_ids):
    ...
    with patch('shieldnetdefend.agent.get_agents_info', return_value=set(agent_ids_format(range(100000)))):
...
@patch('shieldnetdefend.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('shieldnetdefend.core.agent.Agent.group_exists', return_value=False)
...
async def test_unset_single_group_agent_ko(agent_basic_mock, group_exists_mock, get_groups_mock, agent_id, group_id,
    ...
    expected_exc: int
        Expected ShieldnetDefendException code error.
    """
    get_groups_mock.return_value = ['default']
    with pytest.raises(ShieldnetDefendException, match=f".* {expected_exc} .*"):
...
def test_check_uninstall_permission():
    ...
    expected = ShieldnetDefendResult({'message': 'User has permission to uninstall agents'})
