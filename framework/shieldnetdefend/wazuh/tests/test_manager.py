...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.manager import *
        from shieldnetdefend.core.manager import LoggingFormat
        from shieldnetdefend.core.tests.test_manager import get_logs
        from shieldnetdefend import ShieldnetDefendInternalError
...
def mock_shieldnet_defend_path():
    with patch('shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH', new=test_data_path):
        yield
...
manager_status = {'shieldnet-defend-agentlessd': 'running', 'shieldnet-defend-analysisd': 'running', 'shieldnet-defend-authd': 'running',
 'shieldnet-defend-csyslogd': 'running', 'shieldnet-defend-dbd': 'running', 'shieldnet-defend-monitord': 'running',
 'shieldnet-defend-execd': 'running', 'shieldnet-defend-integratord': 'running', 'shieldnet-defend-logcollector': 'running',
 'shieldnet-defend-maild': 'running', 'shieldnet-defend-remoted': 'running', 'shieldnet-defend-reportd': 'running',
 'shieldnet-defend-syscheckd': 'running', 'shieldnet-defend-clusterd': 'running', 'shieldnet-defend-modulesd': 'running',
 'shieldnet-defend-db': 'running', 'shieldnet-defend-apid': 'running'}


@patch('shieldnetdefend.core.manager.status', return_value=manager_status)
def test_get_status(mock_status):
    """Tests get_status() function works"""
    result = get_status()

    # Assert there are no errors and type returned
    assert isinstance(result, AffectedItemsShieldnetDefendResult), 'No expected result type'
...
@pytest.mark.parametrize('tag, level, total_items, sort_by, sort_ascending', [
    ...
    ('shieldnet-defend-modulesd:database', None, 2, None, None),
    ('shieldnet-defend-modulesd:syscollector', None, 2, None, None),
    ('shieldnet-defend-modulesd:syscollector', None, 2, None, None),
    ('shieldnet-defend-modulesd:aws-s3', None, 5, None, None),
    ('shieldnet-defend-execd', None, 1, None, None),
    ('shieldnet-defend-csyslogd', None, 2, None, None),
...
@patch("shieldnetdefend.core.manager.get_shieldnet_defend_active_logging_format", return_value=LoggingFormat.plain)
@patch("shieldnetdefend.core.manager.exists", return_value=True)
def test_ossec_log(mock_exists, mock_active_logging_format, tag, level, total_items, sort_by, sort_ascending):
    ...
    tag : str
        Filters by log category (i.e. shieldnet-defend-remoted).
    total_items : int
    ...
    """
    with patch('shieldnetdefend.core.manager.tail') as tail_patch:
        ...
        if tag is not None and level != 'shieldnet-defend-modulesd:syscollector':
...
def test_ossec_log_summary(mock_exists, mock_active_logging_format):
    ...
    expected_result = {
        'shieldnet-defend-csyslogd': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'shieldnet-defend-execd': {'all': 1, 'info': 0, 'error': 1, 'critical': 0, 'warning': 0, 'debug': 0},
        'shieldnet-defend-modulesd:aws-s3': {'all': 5, 'info': 2, 'error': 1, 'critical': 0, 'warning': 2, 'debug': 0},
        'shieldnet-defend-modulesd:database': {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 2},
        'shieldnet-defend-modulesd:syscollector': {'all': 2, 'info': 2, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0},
        'shieldnet-defend-rootcheck': {'all': 1, 'info': 1, 'error': 0, 'critical': 0, 'warning': 0, 'debug': 0}
    }

    logs = get_logs().splitlines()
    with patch('shieldnetdefend.core.manager.tail', return_value=logs):
...
@patch('socket.socket')
@patch('shieldnetdefend.core.cluster.utils.fcntl')
@patch('shieldnetdefend.core.cluster.utils.open')
...
def test_restart_ko_socket(mock_exists, mock_fcntl, mock_open):
    ...
    with pytest.raises(ShieldnetDefendInternalError, match='.* 1901 .*'):
        restart()

    # Socket error
    with patch("os.path.exists", return_value=True):
        with patch('socket.socket', side_effect=socket.error):
            with pytest.raises(ShieldnetDefendInternalError, match='.* 1902 .*'):
                restart()

        with patch('socket.socket.connect'):
            with patch('socket.socket.send', side_effect=socket.error):
                with pytest.raises(ShieldnetDefendInternalError, match='.* 1014 .*'):
                    restart()


@pytest.mark.parametrize('error_flag, error_msg', [
    (0, ""),
    (1, "2019/02/27 11:30:07 shieldnet-defend-clusterd: ERROR: [Cluster] [Main] Error 3004 - Error in cluster configuration: "
        "Unspecified key"),
    (1, "2019/02/27 11:30:24 shieldnet-defend-authd: ERROR: (1230): Invalid element in the configuration: "
        "'use_source_i'.\n2019/02/27 11:30:24 shieldnet-defend-authd: ERROR: (1202): Configuration error at "
...
def test_validation(mock_exists, error_flag, error_msg):
    ...
    """
    with patch('shieldnetdefend.core.manager.ShieldnetDefendSocket') as sock:
        # Mock sock response
...
@pytest.mark.parametrize('exception', [
    ...
    ShieldnetDefendError(1013)
])
@patch('shieldnetdefend.manager.validate_ossec_conf')
def test_validation_ko(mock_validate, exception):
    mock_validate.side_effect = exception

    if isinstance(exception, ShieldnetDefendInternalError):
        with pytest.raises(ShieldnetDefendInternalError, match='.* 1013 .*'):
            validation()
    else:
        result = validation()
...
@patch('shieldnetdefend.core.configuration.get_active_configuration')
...
@patch('shieldnetdefend.manager.validate_ossec_conf', return_value={'status': 'OK'})
@patch('shieldnetdefend.manager.write_ossec_conf')
@patch('shieldnetdefend.manager.validate_shieldnet_defend_xml')
@patch('shieldnetdefend.manager.full_copy')
@patch('shieldnetdefend.manager.exists', return_value=True)
@patch('shieldnetdefend.manager.remove')
@patch('shieldnetdefend.manager.safe_move')
