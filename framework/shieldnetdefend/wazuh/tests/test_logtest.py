...
with patch('shieldnetdefend.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.logtest import run_logtest, end_logtest_session


def send_logtest_msg_mock(**kwargs):
    socket_response = create_shieldnet_defend_socket_message(command=kwargs['command'], parameters=kwargs['parameters'])
...
def test_get_logtest_output(logtest_param_values):
    ...
    with patch('shieldnetdefend.logtest.send_logtest_msg') as send_mock:
