...
with patch('shieldnetdefend.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.tests.util import get_fake_database_data, RBAC_bypasser, InitWDBSocketMock

        shieldnetdefend.rbac.decorators.expose_resources = RBAC_bypasser
        from shieldnetdefend import task, ShieldnetDefendError
...
@patch('shieldnetdefend.core.utils.ShieldnetDefendDBConnection', return_value=InitWDBSocketMock(sql_schema_file='schema_tasks_test.sql'))
@patch.object(ShieldnetDefendDBQueryTask, '_final_query', fake_final_query)
