...
with patch('shieldnetdefend.core.common.getgrnam'):
    with patch('shieldnetdefend.core.common.getpwnam'):
        ...
        from shieldnetdefend.cdb_list import get_lists, get_path_lists, iterate_lists, get_list_file, upload_list_file,\
            delete_list_file
...
RELATIVE_PATH = os.path.join("framework", "shieldnetdefend", "tests", "data", "test_cdb_list")
NAME_FILE_1 = "test_lists_1"
NAME_FILE_2 = "test_lists_2"
NAME_FILES = [NAME_FILE_1, NAME_FILE_2]

RESULT_GET_LIST_FILE_1 = [{'items': [{'key': 'test-shieldnet-defend-w', 'value': 'write'},
                                     {'key': 'test-shieldnet-defend-r', 'value': 'read'},
                                     {'key': 'test-shieldnet-defend-a', 'value': 'attribute'},
                                     {'key': 'test-shieldnet-defend-x', 'value': 'execute'},
                                     {'key': 'test-shieldnet-defend-c', 'value': 'command'}
                                     ],
...
@patch('shieldnetdefend.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
@pytest.mark.parametrize("search_text, complementary_search, search_in_fields, paths, expected_result", [
    ...
    ("test-shieldnet-defend-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-ossec-w", False, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-shieldnet-defend-w", False, "items", [NAME_FILE_2], []),
    ...
    ("test-shieldnet-defend-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_2),
    ("test-ossec-w", True, "items", NAME_FILES, RESULT_GET_LIST_FILE_1),
    ("test-shieldnet-defend-w", True, "items", [NAME_FILE_2], RESULT_GET_LIST_FILE_2),
])
@patch('shieldnetdefend.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
...
def test_get_lists_sort():
    ...
    assert isinstance(result_a, AffectedItemsShieldnetDefendResult)
    assert isinstance(result_b, AffectedItemsShieldnetDefendResult)
...
@patch('shieldnetdefend.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH)
@patch('shieldnetdefend.cdb_list.iterate_lists', side_effect=lists_path_mock)
...
def test_get_list_file(filename, raw, expected_result, total_failed_items):
    ...
    """
    with patch('shieldnetdefend.cdb_list.get_filenames_paths', return_value=[os.path.join(DATA_PATH, 'test_lists_2')]):
        result = get_list_file([filename], raw)
        if raw:
            assert result == expected_result
        else:
            isinstance(result, AffectedItemsShieldnetDefendResult)
...
@patch('shieldnetdefend.cdb_list.safe_move')
@patch('shieldnetdefend.cdb_list.delete_file_with_backup')
@patch('shieldnetdefend.cdb_list.upload_file')
@patch('shieldnetdefend.cdb_list.delete_list_file')
@patch('shieldnetdefend.cdb_list.remove')
@patch('shieldnetdefend.cdb_list.exists', return_value=True)
...
@patch('shieldnetdefend.cdb_list.common.USER_LISTS_PATH', return_value='/test/path')
@patch('shieldnetdefend.cdb_list.remove')
def test_upload_list_file_ko(mock_remove, mock_lists_path):
    ...
    with patch('shieldnetdefend.cdb_list.safe_move') as mock_safe_move:
        with patch('shieldnetdefend.cdb_list.exists', return_value=True):
            ...
            with patch('shieldnetdefend.cdb_list.get_filenames_paths', return_value=['/test']):
        ...
        with patch('builtins.open'):
            with patch('shieldnetdefend.cdb_list.exists', return_value=False):
                with patch('shieldnetdefend.core.configuration.tempfile.mkstemp', return_value=['mock_handle', 'mock_tmp_file']):
                    with pytest.raises(ShieldnetDefendInternalError, match=r'\b1005\b'):
                        upload_list_file(filename='test', content='test:content', overwrite=False)


@patch('shieldnetdefend.core.cdb_list.delete_shieldnet_defend_file')
def test_delete_list_file(mock_delete_file):
    ...
    try:
        ...
        with patch('shieldnetdefend.cdb_list.common.USER_LISTS_PATH', new=DATA_PATH):
            result = delete_list_file(['test_file'])
            assert result.render()['data']['affected_items'][0] ==\
                   'framework/shieldnetdefend/tests/data/test_cdb_list/test_file'
    finally:
        try:
            os.remove(test_file)
        except Exception:
