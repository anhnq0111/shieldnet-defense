...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.ciscat import get_ciscat_results
...
@patch('shieldnetdefend.ciscat.get_agents_info', return_value=['001'])
...
def test_get_ciscat_results_select_ko(agents_info_mock, socket_mock, exists_mock):
    ...
    with patch('shieldnetdefend.core.utils.ShieldnetDefendDBConnection') as mock_wdb:
        mock_wdb.return_value = InitWDBSocketMock(sql_schema_file=db_file)
        with pytest.raises(ShieldnetDefendError, match=r'\b1724\b'):
