...
from shieldnetdefend.core.common import shieldnet_defend_uid, shieldnet_defend_gid, DEFAULT_RBAC_RESOURCES
from shieldnetdefend.core.utils import get_utc_now, safe_move
from shieldnetdefend.rbac.utils import clear_tokens_cache

logger = logging.getLogger("shieldnet-defend-api")

# Max reserved ID value
SHIELDNET_DEFEND_USER_ID = 1
SHIELDNET_DEFEND_WUI_USER_ID = 2
MAX_ID_RESERVED = 99
    ...
    def migrate_data(self, source: str, target: str, from_id: int = None, to_id: int = None):
        ...
        with AuthenticationManager(self.sessions[target]) as auth_manager:
            for user in old_users:
                if user.id in (SHIELDNET_DEFEND_USER_ID, SHIELDNET_DEFEND_WUI_USER_ID):
        ...
        if from_id == SHIELDNET_DEFEND_USER_ID and to_id == SHIELDNET_DEFEND_WUI_USER_ID:
            return
...
def check_database_integrity():
    ...
    def _set_permissions_and_ownership(database: str):
        """Set ShieldnetDefend ownership and permissions.
        ...
        """
        chown(database, shieldnet_defend_uid(), shieldnet_defend_gid())
    ...
    try:
        ...
        if os.path.exists(DB_FILE):
            ...
            if current_version < expected_version:
                ...
                db_manager.migrate_data(source=DB_FILE, target=DB_FILE_TMP, from_id=SHIELDNET_DEFEND_USER_ID,
                                        to_id=SHIELDNET_DEFEND_WUI_USER_ID)
                ...
                safe_move(DB_FILE_TMP, DB_FILE,
                          ownership=(shieldnet_defend_uid(), shieldnet_defend_gid()),
                          permissions=0o640)
        ...
        logger.error("Error retrieving the current ShieldnetDefend RBAC database version. Aborting database integrity check")
        db_manager.close_sessions()
        raise e
