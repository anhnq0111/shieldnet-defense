...
from shieldnetdefend.core import common, shieldnet_defend_socket
from shieldnetdefend.core.exception import ShieldnetDefendError, ShieldnetDefendInternalError, ShieldnetDefendResourceNotFound
from shieldnetdefend.core.utils import cut_array, load_shieldnet_defend_xml, safe_move
...
DEFAULT_CTI_URL = 'https://cti.shieldnetdefend.com'
...
def _replace_custom_values(opt_value: Union[list, dict, str]) -> Union[list, dict, str]:
    """Replace custom values introduced by 'load_shieldnet_defend_xml' with their real values.
...
def _rcl2json(filepath: str) -> dict:
    ...
    try:
        ...
        raise ShieldnetDefendError(1101, str(e))
...
def get_ossec_conf(section: str = None, field: str = None, conf_file: str = common.OSSEC_CONF,
    ...
    ShieldnetDefendError(1101)
        Requested component does not exist.
    ShieldnetDefendError(1102)
        Invalid section.
    ShieldnetDefendError(1103)
        Invalid field in section.
    ShieldnetDefendError(1106)
    ...
    try:
        # Read XML
        xml_data = load_shieldnet_defend_xml(conf_file)

        # Parse XML to JSON
        data = _ossecconf2json(xml_data)
    except Exception as e:
        if not from_import:
            raise ShieldnetDefendError(1101, extra_message=str(e))
        else:
            print(f"shieldnet-defend-apid: There is an error in the ossec.conf file: {str(e)}")
            sys.exit(0)
    ...
    if section:
        try:
            ...
            if section not in CONF_SECTIONS.keys():
                raise ShieldnetDefendError(1102, extra_message=e.args[0])
            else:
                raise ShieldnetDefendError(1106, extra_message=e.args[0])
    ...
    if section and field:
        try:
            ...
            raise ShieldnetDefendError(1103)
...
def get_agent_conf(group_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if not os_path.exists(os_path.join(common.SHARED_PATH, group_id)):
        raise ShieldnetDefendResourceNotFound(1710, group_id)
    ...
    if not os_path.exists(agent_conf):
        raise ShieldnetDefendError(1006, agent_conf)
    ...
    try:
        ...
        # Parse XML to JSON
            ...
            # Read XML
            xml_data = load_shieldnet_defend_xml(agent_conf)
...
def get_agent_conf_multigroup(multigroup_id: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
    ...
    if multigroup_id and not os_path.exists(os_path.join(common.MULTI_GROUPS_PATH, multigroup_id)) or not multigroup_id:
        raise ShieldnetDefendResourceNotFound(1710, extra_message=multigroup_id if multigroup_id else "No multigroup provided")
    ...
    if not os_path.exists(agent_conf):
        raise ShieldnetDefendError(1006, extra_message=os_path.join("SHIELDNET_DEFEND_PATH", "var", "multigroups", agent_conf))
    ...
    try:
        ...
        raise ShieldnetDefendError(1101)
...
def get_file_conf(filename: str, group_id: str = None, type_conf: str = None, raw: bool = False) -> dict | str:
    ...
    if not os_path.exists(file_path):
        raise ShieldnetDefendError(1006, file_path)
    ...
    if type_conf:
        if type_conf in types:
            ...
            raise ShieldnetDefendError(1104, f'{type_conf}. Valid types: {types.keys()}')
    else:
...
def parse_internal_options(high_name: str, low_name: str) -> str:
    ...
    Raises
    ------
    ShieldnetDefendInternalError(1107)
        Internal options file not found.
    ShieldnetDefendInternalError(1108)
    ...
    if not os_path.exists(common.INTERNAL_OPTIONS_CONF):
        raise ShieldnetDefendInternalError(1107)
    ...
    try:
        ...
        raise ShieldnetDefendInternalError(1108, e.args[0])
...
def get_internal_options_value(high_name: str, low_name: str, max_: int, min_: int) -> int:
    ...
    Raises
    ------
    ShieldnetDefendError(1109)
        Option must be a digit.
    ShieldnetDefendError(1110)
    ...
    if not option.isdigit():
        raise ShieldnetDefendError(1109, f'Option: {high_name}.{low_name}. Value: {option}')

    option = int(option)
    if option < min_ or option > max_:
        raise ShieldnetDefendError(1110, f'Max value: {max_}. Min value: {min_}. Found: {option}.')
...
def upload_group_configuration(group_id: str, file_content: str) -> str:
    ...
    ShieldnetDefendError(1113)
        XML syntax error.
    ShieldnetDefendError(1114)
        ShieldnetDefend syntax error.
    ShieldnetDefendError(1115)
        Error executing verify-agent-conf.
    ShieldnetDefendInternalError(1743)
        Error running ShieldnetDefend syntax validator.
    ...
    try:
        # check ShieldnetDefend xml format
        try:
            subprocess.check_output([os_path.join(common.SHIELDNET_DEFEND_PATH, "bin", "verify-agent-conf"), '-f', tmp_file_path],
            ...
            if output_regex:
                raise ShieldnetDefendError(1114, ' '.join(output_regex))
            else:
                raise ShieldnetDefendError(1115, e.output.decode())
        except Exception as e:
            raise ShieldnetDefendInternalError(1743, str(e))

        # move temporary file to group folder
        try:
            new_conf_path = os_path.join(common.SHARED_PATH, group_id, "agent.conf")
            safe_move(tmp_file_path, new_conf_path, ownership=(common.shieldnet_defend_uid(), common.shieldnet_defend_gid()),
                      permissions=0o660)
        except Exception as e:
            raise ShieldnetDefendInternalError(1016, extra_message=str(e))
...
def upload_group_file(group_id: str, file_data: str, file_name: str = 'agent.conf') -> str:
    ...
    ShieldnetDefendResourceNotFound(1710)
        Group was not found.
    ShieldnetDefendError(1112)
        Empty files are not supported.
    ShieldnetDefendError(1111)
    ...
    if file_name == 'agent.conf':
        ...
        raise ShieldnetDefendError(1111)
...
def get_active_configuration(agent_id: str, component: str, configuration: str) -> dict:
    ...
    component_socket_mapping = {'agent': 'analysis', 'agentless': 'agentless', 'analysis': 'analysis', 'auth': 'auth',
                                ...
                                'request': 'remote', 'syscheck': 'syscheck', 'shieldnet-defend-db': 'wdb', 'wmodules': 'wmodules'}
    component_socket_dir_mapping = {'agent': 'sockets', 'agentless': 'sockets', 'analysis': 'sockets',
                                    ...
                                    'request': 'sockets', 'syscheck': 'sockets', 'shieldnet-defend-db': 'db',
                                    'wmodules': 'sockets'}

    if not component or not configuration:
        raise ShieldnetDefendError(1307)

    # Check if the component is correct
    components = component_socket_mapping.keys()
    if component not in components:
        raise ShieldnetDefendError(1101, f'Valid components: {", ".join(components)}')
    ...
    def get_active_configuration_manager():
        ...
        dest_socket = os_path.join(common.SHIELDNET_DEFEND_PATH, "queue", component_socket_dir_mapping[component],
                                   component_socket_mapping[component])

        # Verify component configuration
        if not os.path.exists(dest_socket):
            raise ShieldnetDefendError(1121,
        ...
        if component_socket_mapping[component] not in sockets_json_protocol:
            ...
            # Socket connection
            try:
                s = shieldnet_defend_socket.ShieldnetDefendSocket(dest_socket)
            except ShieldnetDefendInternalError:
                raise
            except Exception as unhandled_exc:
                raise ShieldnetDefendInternalError(1121, extra_message=str(unhandled_exc))
            ...
            try:
                ...
                raise ShieldnetDefendInternalError(1118, extra_message="Data could not be received")
            finally:
                s.close()
        ...
        # JSON socket message
            ...
            msg = shieldnet_defend_socket.create_shieldnet_defend_socket_message(origin={'module': common.origin_module.get()},
                                                           command=GETCONFIG_COMMAND,
                                                           parameters={'section': configuration})

            # Socket connection
            try:
                s = shieldnet_defend_socket.ShieldnetDefendSocketJSON(dest_socket)
            except ShieldnetDefendInternalError:
                raise
            except Exception as unhandled_exc:
                raise ShieldnetDefendInternalError(1121, extra_message=str(unhandled_exc))
    ...
    if rec_error == 'ok' or rec_error == 0:
        ...
        if data.get('auth', {}).get('use_password') == 'yes':
            try:
                with open(os_path.join(common.SHIELDNET_DEFEND_PATH, "etc", "authd.pass"), 'r') as f:
        ...
        raise ShieldnetDefendError(1117 if "No such file or directory" in rec_data or "Cannot send request" in rec_data else 1116,
                         extra_message=f'{component}:{configuration}')


def write_ossec_conf(new_conf: str):
    """Replace the current shieldnetdefend configuration (ossec.conf) with the provided configuration.
    ...
    try:
        ...
        raise ShieldnetDefendError(1126, extra_message=str(e))
