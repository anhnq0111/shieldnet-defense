...
import shieldnetdefend.core.results as results
        ...
        if match := re.match(regex, pid_file):
            try:
                ...
                if daemon.replace('-', '_') in command:
                    ...
                    print(f"{daemon}: Process {pid} does not belong to {daemon}, removing from {common.SHIELDNET_DEFEND_PATH}/var/run...")

            except (OSError, psutil.NoSuchProcess):
                print(f'{daemon}: Non existent process {pid}, removing from {common.SHIELDNET_DEFEND_PATH}/var/run...')
            finally:
...
def process_array(array: list, search_text: str = None, complementary_search: bool = False,
    ...
    """Process a ShieldnetDefend framework data array.
...
def cut_array(array: list, offset: int = 0, limit: int = common.DATABASE_LIMIT) -> list:
    ...
    Raises
    ------
    ShieldnetDefendError(1400)
        Invalid offset.
    ShieldnetDefendError(1401)
        Invalid limit.
    ShieldnetDefendError(1405)
        Limit exceeding the maximum permitted.
    ShieldnetDefendError(1406)
        Invalid limit (0).
    ...
    if limit is not None:
        if limit > common.MAXIMUM_DATABASE_LIMIT:
            raise ShieldnetDefendError(1405, extra_message=str(limit))
        elif limit == 0:
            raise ShieldnetDefendError(1406)
    ...
    if offset < 0:
        raise ShieldnetDefendError(1400)
    elif limit < 1:
        raise ShieldnetDefendError(1401)
    else:
...
def sort_array(array: list, sort_by: list = None, sort_ascending: bool = True,
    ...
    Raises
    ------
    ShieldnetDefendError(1403)
        Not a valid sort field.
    ShieldnetDefendError(1402)
    ...
    def check_sort_fields(allowed_sort_fields, sort_by):
        ...
        if not sort_by.issubset(allowed_sort_fields):
            ...
            raise ShieldnetDefendError(1403, extra_remediation='Allowed sort fields: {0}. '
    ...
    if not isinstance(sort_ascending, bool):
        raise ShieldnetDefendError(1402)
    ...
    if sort_by:  # array should be a dictionary or a Class
        ...
        if type(array) is set or (type(array[0]) is not dict and 'class \'shieldnetdefend' not in str(type(array[0]))):
...
def select_array(array: list, select: list = None, required_fields: set = None,
    ...
    if allowed_select_fields and not select_no_nested.issubset(allowed_select_fields):
        raise ShieldnetDefendError(1724, "{}".format(', '.join(select_no_nested)))
    ...
    for item in array:
        ...
        if array and not allowed_select_fields and not selected_fields:
            raise ShieldnetDefendError(1724, "{}".format(', '.join(select)))
...
def delete_shieldnet_defend_file(full_path: str) -> bool:
    """Delete a ShieldnetDefend file.
    ...
    Raises
    ------
    ShieldnetDefendError(1906)
        File does not exist.
    ShieldnetDefendError(1907)
    ...
    bool
        True if success.
    """
    if not full_path.startswith(common.SHIELDNET_DEFEND_PATH) or '..' in full_path:
        raise ShieldnetDefendError(1907)
...
def _get_hashing_algorithm(hash_algorithm):
    ...
    if hash_algorithm not in algorithm_list:
        raise ShieldnetDefendError(1723, "Available algorithms are {0}.".format(', '.join(algorithm_list)))
        ...
        try:
            for line in command_regex.findall(data)[0].split(split_section):
                ...
                if command_matches and \
                    ...
                    raise ShieldnetDefendError(1124)
        except IndexError:
            pass
...
def check_shieldnet_defend_limits_unchanged(new_conf, original_conf):
    """Check if ShieldnetDefend limits remain unchanged.
    ...
    for disabled_limit in [conf for conf, allowed in limits_configuration.items() if not allowed['allow']]:
        ...
        if len(new_limits) != len(original_limits) or any(x != y for x, y in zip(new_limits, original_limits)):
            raise ShieldnetDefendError(1127, extra_message=f"global > limits > {disabled_limit}")
        ...
        try:
            for line in agents_regex.findall(data)[0].split(split_section):
                ...
                if tag_matches and (tag_matches.group(1) == 'yes'):
                    raise ShieldnetDefendError(1129)
        except IndexError:
            pass
...
def check_indexer(new_conf: str, original_conf: str):
    ...
    if not upload_configuration['indexer']['allow']:
        ...
        if len(new_indexer) != len(original_indexer) or any(x != y for x, y in zip(new_indexer, original_indexer)):
            raise ShieldnetDefendError(1127, extra_message='indexer')
    ...
    if not blocked_configurations['public_key']['allow']:
        ...
        for api_key in api_keys:
            ...
            try:
                ...
                raise ShieldnetDefendError(1131, extra_message=f'{extra_msg}')
            if response_minimum_quota == minimum_quota:
                raise ShieldnetDefendError(1130, extra_message='integrations > virustotal')


def load_shieldnet_defend_xml(xml_path, data=None):
    if not data:
        with open(xml_path) as f:
            try:
                data = f.read()
            except Exception as e:
                raise ShieldnetDefendError(1113, extra_message=str(e))
...
class ShieldnetDefendVersion:

    def __init__(self, version):

        pattern = r"(?:ShieldnetDefend )?v?(\d+)\.(\d+)\.(\d+)\-?(alpha|beta|rc)?(\d*)"
...
def get_timeframe_in_seconds(timeframe: str) -> int:
    ...
    if not timeframe.isdigit():
        if 'h' not in timeframe and 'd' not in timeframe and 'm' not in timeframe and 's' not in timeframe:
            raise ShieldnetDefendError(1411, timeframe)
        ...
        for or_clause in or_clauses:
            ...
            for and_clause in and_clauses:
                ...
                try:
                    ...
                    raise ShieldnetDefendError(1407, extra_message=f"Parameter 'q' is not valid: '{and_clause}'")
...
class ShieldnetDefendDBBackend(AbstractDatabaseBackend):
    """
    This class describes a shieldnetdefend db backend that executes database queries.
    """
    ...
    def __init__(self, agent_id=None, query_format='agent', request_slice=500):
        if query_format == 'agent' and not path.exists(path.join(common.WDB_PATH, f"{agent_id}.db")):
            raise ShieldnetDefendError(2007, extra_message=f"There is no database for agent {agent_id}. "
    ...
    def connect_to_db(self):
        return ShieldnetDefendDBConnection(request_slice=self.request_slice)
    ...
    def execute(self, query, request, count=False):
        """Execute SQL query through ShieldnetDefendDB socket."""
        query = self._substitute_params(query, request)
        return self.conn.execute(query=self._render_query(query), count=count)


class ShieldnetDefendDBQuery(object):
    """This class describes a database query for shieldnetdefend."""
    ...
    def __init__(self, offset: int, limit: int, table: str, sort: dict, search: dict, select: list, query: str,
        ...
        """ShieldnetDefend DB Query constructor.
        ...
        # For example, the following regex -> (name!=shieldnetdefend;id>5),group=webserver <- would return 3 different matches:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        isinstance(self.backend, ShieldnetDefendDBBackend) and self.backend.close_connection()
    ...
    def _add_limit_to_query(self):
        if self.limit:
            if self.limit > common.MAXIMUM_DATABASE_LIMIT:
                raise ShieldnetDefendError(1405, extra_message=str(self.limit))
        ...
        if self.sort:
            if self.sort['fields']:
                ...
                if not set(sort_fields).issubset(allowed_sort_fields):
                    raise ShieldnetDefendError(1403, "Allowed sort fields: {}. Fields: {}".format(
    ...
    def _parse_select_filter(self, select_fields):
        if select_fields:
            ...
            if not select_fields_set or not select_fields_set.issubset(allowed_select_fields):
                raise ShieldnetDefendError(1724, "Allowed select fields: {0}. Fields: {1}". \
    ...
    def _parse_query(self):
        ...
        if not self.query_regex.match(self.q):
            raise ShieldnetDefendError(1407, self.q)
        ...
        for open_level, field, operator, value, close_level, separator in self.query_regex.findall(self.q):
            if field not in allowed_query_fields:
                raise ShieldnetDefendError(1408, "Available fields: {}. Field: {}".format(
                    ', '.join(sorted(allowed_query_fields)), field)
                )
            if operator not in self.query_operators:
                raise ShieldnetDefendError(1409,
    ...
    def _filter_date(self, date_filter, filter_db_name):
        ...
        if date_filter['value'].isdigit() or re.match(r'\d+[dhms]', date_filter['value']):
            ...
            raise ShieldnetDefendError(1412, date_filter['value'])
    ...
    def oversized_run(self) -> dict:
        ...
        Raises
        ------
        ShieldnetDefendInternalError(1123)
        ...
        final_ids = list()
        resources = list()
        if self.__class__.__name__ == 'ShieldnetDefendDBQueryAgents':
            resource = 'id'
        elif self.__class__.__name__ == 'ShieldnetDefendDBQueryGroups':
            resource = 'name'
        else:
            raise ShieldnetDefendInternalError(1123)
...
class ShieldnetDefendDBQueryDistinct(ShieldnetDefendDBQuery):
    ...
    def _add_filters_to_query(self):
        ShieldnetDefendDBQuery._add_filters_to_query(self)
        self.query += ' WHERE ' if not self.q and 'WHERE' not in self.query else ' AND '
        self.query += ' AND '.join(
            ["{0} IS NOT null AND {0} != ''".format(self.fields[field]) for field in self.select])
    ...
    def _add_select_to_query(self):
        if len(self.select) > 1:
            raise ShieldnetDefendError(1410)

        ShieldnetDefendDBQuery._add_select_to_query(self)
...
def validate_shieldnet_defend_xml(content: str, config_file: bool = False):
    """Validate ShieldnetDefend XML files (rules, decoders and ossec.conf)
    ...
    try:
        ...
        if config_file:
            ...
            check_shieldnet_defend_limits_unchanged(final_xml, current_xml)
        # Check xml format
        load_shieldnet_defend_xml(xml_path='', data=final_xml)
    except ExpatError:
        raise ShieldnetDefendError(1113)
    except ShieldnetDefendError as e:
        raise e
    except Exception as e:
        raise ShieldnetDefendError(1113, str(e))
...
def upload_file(content: str, file_path: str, check_xml_formula_values: bool = True):
    ...
    ShieldnetDefendInternalError(1005)
        Error reading file.
    ShieldnetDefendInternalError(1016)
        Error moving file.
    ShieldnetDefendError(1006)
    ...
    try:
        ...
        raise ShieldnetDefendInternalError(1005) from exc

    # Move temporary file to group folder
    try:
        new_conf_path = path.join(common.SHIELDNET_DEFEND_PATH, file_path)
        safe_move(tmp_file_path, new_conf_path, ownership=(common.shieldnet_defend_uid(), common.shieldnet_defend_gid()), permissions=0o660)
    except PermissionError as exc:
        raise ShieldnetDefendError(1006) from exc
    except Error as exc:
        raise ShieldnetDefendInternalError(1016) from exc

    return results.ShieldnetDefendResult({'message': 'File was successfully updated'})
...
def to_relative_path(full_path: str, prefix: str = common.SHIELDNET_DEFEND_PATH) -> str:
    """Return a relative path from the ShieldnetDefend base directory.
    ...
    full_path : str
        Absolute path.
    prefix : str, opt
        Prefix to strip from the absolute path. Default `common.SHIELDNET_DEFEND_PATH`
...
def check_if_shieldnet_defend_agent_version(version_str: str) -> bool:
    """Check if the string has the expected shieldnetdefend agent version format.

    Parameters
    ----------
    version_str : str
        The shieldnetdefend version string.

    Returns
    -------
    bool
        True if the string has the expected shieldnetdefend version format.

    """
    if not isinstance(version_str, str):
        return False

    return bool(re.match(r'^ShieldnetDefend v(\d+)\.(\d+)\.(\d+)', version_str))


def parse_shieldnet_defend_agent_version(version_str: str) -> tuple:
    ...
    -------
    tuple
        The tuple of the shieldnetdefend version string.
    """
