...
class TimeBasedFileRotatingHandler(logging.handlers.TimedRotatingFileHandler):
    """
    ShieldnetDefend log rotation. It rotates the log at midnight and sets the appropriate permissions to the new log file.
    """
    ...
    def doRollover(self):
        ...
        # Save rotated file in {SHIELDNET_DEFEND_PATH}/logs/api directory
...
class SizeBasedFileRotatingHandler(logging.handlers.RotatingFileHandler):
    """ShieldnetDefend log rotation. It rotates when the logging file size exceeds the maximum number of bytes configured."""
...
class ShieldnetDefendLogger:
    """
    Define attributes of a Python ShieldnetDefend daemon's logger.
    """
    def __init__(self, foreground_mode: bool, log_path: str, debug_level: [int, str], logger_name: str = 'shieldnetdefend',
        ...
        log_path : str
            Filepath of the file to send logs to. Relative to the ShieldnetDefend installation path.
        ...
        """
        self.log_path = os.path.join(common.SHIELDNET_DEFEND_PATH, log_path)
        self.logger = None
