...
from shieldnetdefend.core.cluster import client, cluster, server
from shieldnetdefend.core.cluster import common as c_common
from shieldnetdefend.core.cluster.dapi import dapi
from shieldnetdefend.core.cluster.utils import context_tag
...
class LocalServer(server.AbstractServer):
    ...
    async def start(self):
        ...
        socket_path = os.path.join(common.SHIELDNET_DEFEND_PATH, 'queue', 'cluster', 'c-internal.sock')
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            ...
            if node_name in self.server.node.clients:
                ...
                raise ShieldnetDefendClusterError(3022)
        else:
            return super().process_request(command, data)
    ...
    def process_request(self, command: bytes, data: bytes):
        ...
        if command == b'dapi':
            if self.server.node.client is None:
                raise ShieldnetDefendClusterError(3023)
            asyncio.create_task(self.log_exceptions(
