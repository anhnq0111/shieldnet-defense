...
import shieldnetdefend.core.cluster.cluster
import shieldnetdefend.core.cluster.utils
import shieldnetdefend.core.manager
import shieldnetdefend.core.results as wresults
from shieldnetdefend import agent
from shieldnetdefend.cluster import get_node_wrapper, get_nodes_info
from shieldnetdefend.core import common, exception
from shieldnetdefend.core.cluster import local_client, common as c_common
from shieldnetdefend.core.cluster.cluster import check_cluster_status
from shieldnetdefend.core.exception import ShieldnetDefendException, ShieldnetDefendClusterError, ShieldnetDefendError
from shieldnetdefend.core.pyDaemonModule import spawn_process_pool_worker, API_AUTHENTICATION_PROCESS
from shieldnetdefend.core.shieldnet_defend_socket import shieldnet_defend_sendsync


authentication_funcs = {'check_token', 'check_user_master', 'get_permissions', 'get_security_conf'}
events_funcs = {'send_event_to_analysisd'}

node_info = shieldnetdefend.core.cluster.cluster.get_node()
pools = common.mp_pools.get()
if node_info['type'] == 'master':
    # Suppress exception when the user running ShieldnetDefend cannot access /dev/shm.
...
class DistributedAPI:
    ...
    def __init__(self, f: Callable, logger: logging.getLogger, f_kwargs: Dict = None, node: c_common.Handler = None,
        ...
        self.cluster_items = shieldnetdefend.core.cluster.utils.get_cluster_items() if node is None else node.cluster_items
        self.debug = debug
        ...
        if not basic_services:
            self.basic_services = ('shieldnet-defend-modulesd', 'shieldnet-defend-analysisd', 'shieldnet-defend-execd', 'shieldnet-defend-db', 'shieldnet-defend-remoted')
        else:
    ...
    def debug_log(self, message):
        ...
        message : str
            Full log message.
        """
        if self.logger.name == 'shieldnet-defend-api':
            self.logger.debug2(message)
        else:
            self.logger.debug(message)

    async def distribute_function(self) -> [Dict, exception.ShieldnetDefendException]:
        """
        Distribute an API call.

        Returns
        -------
        dict or ShieldnetDefendException
            Dictionary with API response or ShieldnetDefendException in case of error.
        """
        try:
            ...
            try:
                response = json.loads(response, object_hook=c_common.as_shieldnet_defend_object) \
                    if isinstance(response, str) else response
            except json.decoder.JSONDecodeError:
                response = {'message': response}

            return response if isinstance(response, (wresults.AbstractShieldnetDefendResult, exception.ShieldnetDefendException)) \
                else wresults.ShieldnetDefendResult(response)

        except json.decoder.JSONDecodeError:
            e = exception.ShieldnetDefendInternalError(3036)
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}")
            return e
        except exception.ShieldnetDefendInternalError as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            self.logger.error(f"{e.message}", exc_info=not isinstance(e, exception.ShieldnetDefendClusterError))
            return e
        except exception.ShieldnetDefendError as e:
            e.dapi_errors = self.get_error_info(e)
            return e
            ...
            return exception.ShieldnetDefendInternalError(1000,
                                                dapi_errors=self.get_error_info(e))

    def check_shieldnet_defend_status(self):
        """
        There are some services that are required for shieldnetdefend to correctly process API requests. If any of those services
        is not running, the API must raise an exception indicating that:
            ...
            * There's an error in any of those services that must be addressed before using the API if any service is
              in failed status.
            * ShieldnetDefend must be started before using the API is the services are stopped.

        The basic services shieldnetdefend needs to be running are: shieldnet-defend-modulesd, shieldnet-defend-remoted, shieldnet-defend-analysisd, shieldnet-defend-execd
        and shieldnet-defend-db
        """
        if self.f == shieldnetdefend.core.manager.status:
            return

        status = shieldnetdefend.core.manager.status()
        ...
        if not_ready_daemons:
            ...
            raise exception.ShieldnetDefendInternalError(1017, extra_message=extra_info)
    ...
    async def execute_local_request(self) -> str:
        ...
        try:
            ...
            before = time.time()
            self.check_shieldnet_defend_status()
            ...
            try:
                ...
                try:
                    ...
                    raise exception.ShieldnetDefendInternalError(3021)
                except OperationalError as exc:
                    raise exception.ShieldnetDefendInternalError(2008, extra_message=str(exc.orig))
                except process.BrokenProcessPool:
                    raise exception.ShieldnetDefendInternalError(901)
            except json.decoder.JSONDecodeError:
                raise exception.ShieldnetDefendInternalError(3036)
            except process.BrokenProcessPool:
                raise exception.ShieldnetDefendInternalError(900)
            ...
            # ShieldnetDefendClusterError
            self.logger.error(f"{e.message}",
                              exc_info=e.code not in {3021, 3036, 1913, 1017} and not isinstance(e,
                                                                                           exception.ShieldnetDefendClusterError))
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.ShieldnetDefendJSONEncoder)
        except (exception.ShieldnetDefendError, exception.ShieldnetDefendResourceNotFound) as e:
            e.dapi_errors = self.get_error_info(e)
            if self.debug:
                raise
            return json.dumps(e, cls=c_common.ShieldnetDefendJSONEncoder)
            ...
            if self.debug:
                raise
            return json.dumps(exception.ShieldnetDefendInternalError(1000,
                                                           dapi_errors=self.get_error_info(e)),
                              cls=c_common.ShieldnetDefendJSONEncoder)
    ...
    def get_error_info(self, e) -> Dict:
        ...
        except exception.ShieldnetDefendException as rbac_exception:
        ...
        error_message = e.message if isinstance(e, exception.ShieldnetDefendException) else exception.GENERIC_ERROR_MSG
        result = {node: {'error': error_message}
                  }

        # Give log path only in case of ShieldnetDefendInternalError
        if isinstance(e, exception.ShieldnetDefendInternalError):
            log_filename = None
            for h in self.logger.handlers or self.logger.parent.handlers:
                if hasattr(h, 'baseFilename'):
                    log_filename = os.path.join('SHIELDNET_DEFEND_HOME', os.path.relpath(h.baseFilename, start=common.SHIELDNET_DEFEND_PATH))
    ...
    async def send_tmp_file(self, node_name=None):
        ...
        res = json.loads(await client.send_file(os.path.join(common.SHIELDNET_DEFEND_PATH,
                                                             self.f_kwargs['tmp_file']),
                                                node_name),
                         object_hook=c_common.as_shieldnet_defend_object)
        os.remove(os.path.join(common.SHIELDNET_DEFEND_PATH, self.f_kwargs['tmp_file']))
    ...
    async def execute_remote_request(self) -> Dict:
        ...
        node_response = await client.execute(command=b'dapi',
                                             data=json.dumps(self.to_dict(),
                                                             cls=c_common.ShieldnetDefendJSONEncoder).encode())
        return json.loads(node_response,
                          object_hook=c_common.as_shieldnet_defend_object)

    async def forward_request(self) -> [wresults.AbstractShieldnetDefendResult, exception.ShieldnetDefendException]:
        ...
        Returns
        -------
        wresults.AbstractShieldnetDefendResult or exception.ShieldnetDefendException
        """

        async def forward(node_name: Tuple) -> [wresults.AbstractShieldnetDefendResult, exception.ShieldnetDefendException]:
            ...
            if node_name == self.node_info['node']:
                ...
                try:
                    ...
                    if e.code == 3022:
                        ...
                        raise e
                # Convert a non existing node into a ShieldnetDefendError exception
                if isinstance(result, ShieldnetDefendClusterError) and result.code == 3022:
                    common.rbac.set(self.rbac_permissions)
                    try:
                    ...
                    dikt['ids'] = {node_name}
                    result = ShieldnetDefendError.from_dict(dikt)

            return result if isinstance(result, (wresults.AbstractShieldnetDefendResult, exception.ShieldnetDefendException)) \
                else wresults.ShieldnetDefendResult(result)
        ...
        if 'node_id' in self.f_kwargs or 'node_list' in self.f_kwargs:
            ...
            allowed_nodes = wresults.AffectedItemsShieldnetDefendResult()
        ...
        if allowed_nodes.total_affected_items > 1:
            ...
            if isinstance(response, wresults.AbstractShieldnetDefendResult):
        ...
        # It might be a ShieldnetDefendError after reducing
        if isinstance(response, wresults.AffectedItemsShieldnetDefendResult):
...
class ShieldnetDefendRequestQueue:
    """Represents a queue of ShieldnetDefend requests"""
...
class APIRequestQueue(ShieldnetDefendRequestQueue):
    """
    ...
    def __init__(self, server):
        ...
        self.logger = logging.getLogger('shieldnetdefend').getChild('dapi')
        self.logger.addFilter(shieldnetdefend.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='D API'))
    ...
    async def run(self):
        while True:
            ...
            try:
                request = json.loads(request, object_hook=c_common.as_shieldnet_defend_object)
                ...
                task_id = await node.send_string(json.dumps(result, cls=c_common.ShieldnetDefendJSONEncoder).encode())
            except Exception as e:
...
class SendSyncRequestQueue(ShieldnetDefendRequestQueue):
    """
    ...
    def __init__(self, server):
        super().__init__(server)
        self.logger = logging.getLogger('shieldnetdefend').getChild('sendsync')
        self.logger.addFilter(shieldnetdefend.core.cluster.utils.ClusterFilter(tag='Cluster', subtag='SendSync'))
    ...
    async def run(self):
        while True:
            ...
            try:
                ...
                result = await shieldnet_defend_sendsync(**request)
