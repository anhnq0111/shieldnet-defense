...
with patch('shieldnetdefend.common.getgrnam'):
    with patch('shieldnetdefend.common.getpwnam'):
        with patch('shieldnetdefend.common.shieldnet_defend_uid'):
            with patch('shieldnetdefend.common.shieldnet_defend_gid'):
                ...
                from shieldnetdefend.core.cluster import control
                from shieldnetdefend.core.cluster.local_client import LocalClient
                from shieldnetdefend import ShieldnetDefendInternalError, ShieldnetDefendError
...
async def test_get_nodes():
    ...
    with patch('shieldnetdefend.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
    ...
    with patch('shieldnetdefend.core.cluster.local_client.LocalClient.execute', side_effect=[ShieldnetDefendClusterError(3020), 'error']):
        with pytest.raises(ShieldnetDefendClusterError):
            await control.get_nodes(lc=local_client)
...
async def test_get_system_nodes():
    ...
    with patch('shieldnetdefend.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client):
        ...
        for expected in expected_result:
            with patch('shieldnetdefend.core.cluster.control.get_nodes', return_value=expected):
                result = await control.get_system_nodes()
                assert result == [expected['items'][0]['name']]

        with patch('shieldnetdefend.core.cluster.control.get_nodes', side_effect=ShieldnetDefendInternalError(3012)):
            result = await control.get_system_nodes()
            assert result == ShieldnetDefendError(3013)
...
async def test_get_node_ruleset_integrity():
    ...
    local_client = LocalClient()
    with patch('shieldnetdefend.core.cluster.local_client.LocalClient.execute', side_effect=async_local_client) as execute_mock:
...
@pytest.mark.parametrize('side_effect, json_loads_return_value, expected_err',
    [
        ...
        (ShieldnetDefendClusterError(3020), None, ShieldnetDefendClusterError),
...
async def test_set_reload_ruleset_flag_nok(side_effect, json_loads_return_value, expected_err):
    ...
    local_client = LocalClient()
    with patch('shieldnetdefend.core.cluster.local_client.LocalClient.execute', side_effect=side_effect):
