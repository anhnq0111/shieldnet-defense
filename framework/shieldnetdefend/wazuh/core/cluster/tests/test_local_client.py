...
with patch('shieldnetdefend.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.common.shieldnet_defend_gid'):
        from shieldnetdefend.core.cluster.local_client import *
        from shieldnetdefend.core.cluster.common import InBuffer
...
@patch("shieldnetdefend.core.cluster.client.asyncio.get_running_loop")
...
async def test_localclient_start():
    ...
    with patch("uvloop.Loop.create_unix_connection", side_effect=create_unix_connection) as mock_create_unix_connection:
        ...
        with patch("os.path.join", return_value="path/test"):
            with patch("shieldnetdefend.core.cluster.client.asyncio.get_running_loop", return_value=mocked_loop):
                lc = LocalClient()
                await lc.start()
...
async def test_localclient_start_ko(mock_get_running_loop):
    ...
    with pytest.raises(ShieldnetDefendInternalError, match=r'.* 3009 .*'):
        await LocalClient().start()

    with patch("shieldnetdefend.core.cluster.local_client.os.path.join", side_effect=MemoryError):
        with pytest.raises(ShieldnetDefendInternalError, match=r'.* 1119 .*'):
            await LocalClient().start()

    with patch("shieldnetdefend.core.cluster.local_client.os.path.join", side_effect=FileNotFoundError):
        with pytest.raises(ShieldnetDefendInternalError, match=r'.* 3012 .*'):
            await LocalClient().start()

    with patch("shieldnetdefend.core.cluster.local_client.os.path.join", side_effect=ConnectionRefusedError):
        with pytest.raises(ShieldnetDefendInternalError, match=r'.* 3012 .*'):
            await LocalClient().start()
...
async def test_wait_for_response():
    ...
    lc.protocol.send_request.side_effect = [b"None", exception.ShieldnetDefendClusterError(3018)]

    with patch("asyncio.Event.wait", side_effect=asyncio.TimeoutError):
        with pytest.raises(ShieldnetDefendInternalError, match=rf".* 3020 .*"):
...
async def test_localclient_execute():
    ...
    with patch("shieldnetdefend.core.cluster.local_client.LocalClient.start"):
        with patch("shieldnetdefend.core.cluster.local_client.LocalClient.send_api_request", return_value="Test"):
...
async def test_localclient_send_file():
    ...
    with patch("shieldnetdefend.core.cluster.local_client.LocalClient.start"):
        with patch("shieldnetdefend.core.cluster.local_client.LocalClient.send_api_request", return_value=b"shieldnetdefend/test python"):
            lc = LocalClient()
            assert await lc.send_file(path="shieldnetdefend/test", node_name="python") == b"shieldnetdefend/test python"
