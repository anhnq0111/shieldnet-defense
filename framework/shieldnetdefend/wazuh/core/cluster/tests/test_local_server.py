...
with patch('shieldnetdefend.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.core.cluster.local_server import *
...
async def test_LocalServerHandler_connection_made(event_loop):
    ...
    with patch.object(logger, "debug") as logger_debug_mock:
        with patch("shieldnetdefend.core.cluster.local_server.context_tag", ContextVar("tag", default="")) as mock_contextvar:
...
@pytest.mark.asyncio
@patch("shieldnetdefend.core.cluster.local_server.server.AbstractServerHandler.process_request")
...
async def test_LocalServerHandler_get_ruleset_hashes(event_loop):
    ...
    lsh = LocalServerHandler(server=ServerMock(), loop=event_loop, fernet_key=None, cluster_items={})
    with patch("shieldnetdefend.core.cluster.local_server.cluster.get_ruleset_status",
...
@pytest.mark.asyncio
@patch("shieldnetdefend.core.cluster.local_server.LocalServerHandler.send_request", return_value='changed')
...
async def test_LocalServerHandlerMaster_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(ShieldnetDefendClusterError, match=".* 3022 .*"):
...
@pytest.mark.asyncio
@patch("shieldnetdefend.core.cluster.local_server.LocalServerHandler.process_request")
async def test_LocalServerHandlerWorker_process_request(process_request_mock, event_loop):
    ...
    with pytest.raises(ShieldnetDefendClusterError, match=".* 3023 .*"):
...
async def test_AsyncReloadRulesetFlag_initial_state():
    ...
    from shieldnetdefend.core.cluster.worker import AsyncReloadRulesetFlag
