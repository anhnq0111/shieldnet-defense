...
from shieldnetdefend.core.exception import ShieldnetDefendError, ShieldnetDefendException, ShieldnetDefendInternalError, ShieldnetDefendHAPHelperError
from shieldnetdefend.core.results import ShieldnetDefendResult
from shieldnetdefend.core.utils import temporary_cache
from shieldnetdefend.core.shieldnet_defend_socket import create_shieldnet_defend_socket_message
from shieldnetdefend.core.wlogging import ShieldnetDefendLogger
...
execq_lockfile = os.path.join(common.SHIELDNET_DEFEND_PATH, "var", "run", ".api_execq_lock")

HELPER_DEFAULTS = {
    HAPROXY_PORT: 5555,
    HAPROXY_PROTOCOL: 'http',
    HAPROXY_BACKEND: 'shieldnet_defend_reporting',
    ...
    for field in [
        ...
        if helper_config.get(field):
            try:
                ...
                raise ShieldnetDefendError(3004, extra_message=f"HAProxy Helper {field} must be an integer.")
    ...
    for field in [IMBALANCE_TOLERANCE]:
        if helper_config.get(field):
            try:
                ...
                raise ShieldnetDefendError(3004, extra_message=f"HAProxy Helper {field} must be a float.")
    return helper_config
...
def parse_haproxy_helper_config(helper_config: dict) -> dict:
    ...
    ShieldnetDefendError (3004)
        If some value has an invalid type.
    ShieldnetDefendHAPHelperError (3042)
    ...
    if helper_config[HAPROXY_PROTOCOL].lower() == 'https' and type(helper_config[HAPROXY_CERT]) == bool:
        raise ShieldnetDefendHAPHelperError(3042, extra_message='HAProxy certificate file required in the haproxy_cert parameter')
...
def read_cluster_config(config_file=common.OSSEC_CONF, from_import=False) -> typing.Dict:
    ...
    try:
        ...
        if e.code == 1106:
            ...
            raise ShieldnetDefendError(3006, extra_message=e.message)
    except Exception as e:
        raise ShieldnetDefendError(3006, extra_message=str(e))
    ...
    if isinstance(config_cluster['port'], str) and not config_cluster['port'].isdigit():
        raise ShieldnetDefendError(3004, extra_message="Cluster port must be an integer.")
    ...
    if config_cluster[DISABLED] == NO:
        ...
        raise ShieldnetDefendError(3004,
...
def get_manager_status(cache=False) -> typing.Dict:
    ...
    Raises
    ------
    ShieldnetDefendInternalError(1913)
    ...
    try:
        ...
        raise ShieldnetDefendInternalError(1913, extra_message=str(e))

    processes = ['shieldnet-defend-agentlessd', 'shieldnet-defend-analysisd', 'shieldnet-defend-authd', 'shieldnet-defend-csyslogd', 'shieldnet-defend-dbd', 'shieldnet-defend-monitord',
                 'shieldnet-defend-execd', 'shieldnet-defend-integratord', 'shieldnet-defend-logcollector', 'shieldnet-defend-maild', 'shieldnet-defend-remoted',
                 'shieldnet-defend-reportd', 'shieldnet-defend-syscheckd', 'shieldnet-defend-clusterd', 'shieldnet-defend-modulesd', 'shieldnet-defend-db', 'shieldnet-defend-apid']

    data, pidfile_regex, run_dir = {}, re.compile(r'.+\-(\d+)\.pid$'), os.path.join(common.SHIELDNET_DEFEND_PATH, "var", "run")
...
def get_cluster_status() -> typing.Dict:
    ...
    try:
        cluster_status |= {"running": "yes" if get_manager_status()['shieldnet-defend-clusterd'] == 'running' else "no"}
...
def manager_restart() -> ShieldnetDefendResult:
    """Restart ShieldnetDefend manager.

    Send JSON message with the 'restart-shieldnet-defend' command to common.EXECQ_SOCKET socket.

    Raises
    ------
    ShieldnetDefendInternalError(1901)
        If the socket path doesn't exist.
    ShieldnetDefendInternalError(1902)
    ...
    try:
        ...
        # json msg for restarting ShieldnetDefend manager
        msg = json.dumps(create_shieldnet_defend_socket_message(origin={'module': common.origin_module.get()},
                                                     command=common.RESTART_SHIELDNET_DEFEND_COMMAND,
        ...
        if os.path.exists(socket_path):
            try:
                ...
                raise ShieldnetDefendInternalError(1902)
        else:
    ...
    return ShieldnetDefendResult({'message': 'Restart request sent'})
...
def get_cluster_items():
    ...
    try:
        ...
        with open(os.path.join(common.SHIELDNET_DEFEND_PATH, here, 'cluster.json')) as f:
        ...
        raise ShieldnetDefendError(3005, str(e))
...
class ClusterLogger(ShieldnetDefendLogger):
    """
    Define the logger used by shieldnet-defend-clusterd.
    """
    ...
    def setup_logger(self):
        ...
        Set ups cluster logger. In addition to super().setup_logger() this method adds:
            ...
            * Sets log level based on the "debug_level" parameter received from shieldnet-defend-clusterd binary.
        """
        super().setup_logger()
...
def process_spawn_sleep(child):
    ...
    """
    pid = os.getpid()
    pyDaemonModule.create_pid(f'shieldnet-defend-clusterd_child_{child}', pid)
...
async def forward_function(func: callable, f_kwargs: dict = None, request_type: str = 'local_master',
    ...
    Returns
    -------
    Return either a dict or `ShieldnetDefendResult` instance in case the execution did not fail. Return an exception otherwise.
    """
