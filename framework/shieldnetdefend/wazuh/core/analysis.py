...
from shieldnetdefend.core.shieldnet_defend_socket import create_shieldnet_defend_socket_message, ShieldnetDefendSocket
...
def is_ruleset_file(filename: str) -> bool:
    ...
    If `filename` is already an absolute path and includes `common.SHIELDNET_DEFEND_PATH`, it will be normalized and used as is.
    Otherwise, it will be joined with `common.SHIELDNET_DEFEND_PATH`.
    ...
    if os.path.isabs(filename):
        ...
        full_path = os.path.normpath(os.path.join(common.SHIELDNET_DEFEND_PATH, filename))
...
def send_reload_ruleset_msg(origin: dict[str, str]) -> RulesetReloadResponse:
    ...
    """
    msg = create_shieldnet_defend_socket_message(origin=origin, command=RELOAD_RULESET_COMMAND)

    socket = ShieldnetDefendSocket(common.ANALYSISD_SOCKET)
...
def send_reload_ruleset_and_get_results(node_id: str, results: AffectedItemsShieldnetDefendResult) -> AffectedItemsShieldnetDefendResult:
    """
    ...
    results : AffectedItemsShieldnetDefendResult
    ...
    if socket_response.is_ok():
        ...
        results.add_failed_item(id_=node_id,
                                error=ShieldnetDefendError(code=1914, extra_message=', '.join(socket_response.errors)))
