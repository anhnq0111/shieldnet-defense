...
def load_decoders_from_file(decoder_file: str, decoder_path: str, decoder_status: str) -> list:
    ...
    Raises
    ------
    ShieldnetDefendError(1502)
        Error reading decoders file (permissions).
    ShieldnetDefendInternalError(1501)
    ...
    try:
        decoders = list()
        position = 0
        root = load_shieldnet_defend_xml(os.path.join(common.SHIELDNET_DEFEND_PATH, decoder_path, decoder_file))
        ...
        raise ShieldnetDefendError(1502, extra_message=os.path.join('SHIELDNET_DEFEND_HOME', decoder_path, decoder_file))
    except Exception:
        raise ShieldnetDefendInternalError(1501, extra_message=os.path.join('SHIELDNET_DEFEND_HOME', decoder_path, decoder_file))
