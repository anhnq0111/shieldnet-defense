...
import shieldnetdefend.core.exception as wexception
...
class AbstractShieldnetDefendResult(collections.abc.MutableMapping):
    """
    ...
    def __init__(self, dct: Union[dict, object]):
        ...
        Raises
        ------
        wexception.ShieldnetDefendInternalError(1000)
            If dct is of a wrong type.
        """
        if isinstance(dct, dict):
            self.dikt = dct
        elif isinstance(dct, AbstractShieldnetDefendResult):
            self.dikt = dct.dikt
        else:
            raise wexception.ShieldnetDefendInternalError(1000, extra_message=f"dct param must be a dict or "
                                                                    f"an AbstractShieldnetDefendResult subclass, "
                                                                    f"not a {type(dct)}")
    ...
    def __or__(self, other):
        """ | operator used to merge two AbstractShieldnetDefendResult objects. When merged with a ShieldnetDefendException, the result is
        always a ShieldnetDefendException

        Parameters
        ----------
        other : AbstractShieldnetDefendResult or wexception.ShieldnetDefendException

        Raises
        ------
        ShieldnetDefendInternalError(1000)
            ShieldnetDefend internal error when two object could not be merged.

        Returns
        -------
        AbstractShieldnetDefendResult or wexception.ShieldnetDefendException
            Resultant object.
        """
        if isinstance(other, wexception.ShieldnetDefendException):
            return other
        elif not isinstance(other, (dict, AbstractShieldnetDefendResult)):
            raise wexception.ShieldnetDefendInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def _merge_dict(self, self_field: dict, other_field: dict, key: str = None) -> dict:
        ...
        AbstractShieldnetDefendResult subclass. This method may be redefined in subclasses.
    ...
    def limit(self, limit: int = DATABASE_LIMIT, offset: int = 0):
        """Should take the first `limit` results starting from `offset`. To be redefined in ShieldnetDefendResult subclasses.

        Parameters
        ----------
        limit : int
            Default: the value specified in shieldnetdefend.core.common.DATABASE_LIMIT
        offset : int
            Default: 0.

        Returns
        -------
        AbstractShieldnetDefendResult
            Filtered AbstractShieldnetDefendResult
        """
        result = deepcopy(self)
    ...
    def sort(self, fields: list = None, order: str = 'asc'):
        """Sort according to `fields` in order `order`. To be redefined in ShieldnetDefendResult subclasses.
        ...
        -------
        AbstractShieldnetDefendResult
            Sorted AbstractShieldnetDefendResult
        """
...
class ShieldnetDefendResult(AbstractShieldnetDefendResult):
    """
...
class AffectedItemsShieldnetDefendResult(AbstractShieldnetDefendResult):
    ...
    def add_failed_item(self, id_: str = None, error: wexception.ShieldnetDefendException = None):
        ...
        error : wexception.ShieldnetDefendException
    ...
    def add_failed_items_from(self, other):
        ...
        Parameters
        ----------
        other : AffectedItemsShieldnetDefendResult
        ...
        wexception.ShieldnetDefendInternalError(1000)
            If dct is of a wrong type.
        """
        if not isinstance(other, AffectedItemsShieldnetDefendResult):
            raise wexception.ShieldnetDefendInternalError(1000,
    ...
    def __or__(self, other):
        """Merge a AffectedItemsShieldnetDefendResult with self.

        Parameters
        ----------
        other : AffectedItemsShieldnetDefendResult
            AffectedItemsShieldnetDefendResult object to merge.
        ...
        if isinstance(other, wexception.ShieldnetDefendError):
            if len(other.ids) > 0:
                ...
                return other
        elif isinstance(result, wexception.ShieldnetDefendException):
            return result
        elif not isinstance(other, AffectedItemsShieldnetDefendResult):
            raise wexception.ShieldnetDefendInternalError(1000, extra_message=f"Cannot be merged with {type(other)} object")
    ...
    def to_dict(self) -> dict:
        """Return the AffectedItemsShieldnetDefendResult as a dict.
    ...
    def decode_json(cls, obj: dict):
        ...
        -------
        AbstractShieldnetDefendResult
            Instance of AbstractShieldnetDefendResult.
        """
        result = cls()
    ...
    def render(self) -> dict:
        """Render AffectedItemsShieldnetDefendResult object.

        Returns
        -------
        dict
            Return AffectedItemsShieldnetDefendResult as a dictionary with fields data, message and error.
        """
