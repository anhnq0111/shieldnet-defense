...
from shieldnetdefend.core import common, configuration, stats
...
from shieldnetdefend.core.common import AGENT_COMPONENT_STATS_REQUIRED_VERSION, DATE_FORMAT
from shieldnetdefend.core.exception import ShieldnetDefendException, ShieldnetDefendError, ShieldnetDefendInternalError, ShieldnetDefendResourceNotFound
from shieldnetdefend.core.utils import ShieldnetDefendVersion, plain_dict_to_nested_dict, get_fields_to_nest, ShieldnetDefendDBQuery, \
    ShieldnetDefendDBQueryDistinct, ShieldnetDefendDBQueryGroupBy, ShieldnetDefendDBBackend, get_utc_now, get_utc_strptime, \
    get_date_from_timestamp
from shieldnetdefend.core.shieldnet_defend_queue import ShieldnetDefendQueue
from shieldnetdefend.core.shieldnet_defend_socket import ShieldnetDefendSocket, ShieldnetDefendSocketJSON, create_shieldnet_defend_socket_message
...
from shieldnetdefend.rbac.utils import resource_cache
...
class ShieldnetDefendDBQueryAgents(ShieldnetDefendDBQuery):
    """Class used to query ShieldnetDefend agents."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        if filters is None:
            filters = {}
        unify_shieldnet_defend_version_format(filters)
        if min_select_fields is None:
            min_select_fields = {'id'}
        backend = ShieldnetDefendDBBackend(query_format='global')
        ShieldnetDefendDBQuery.__init__(self, offset=offset, limit=limit, table='agent', sort=sort, search=search, select=select,
    ...
    def _filter_date(self, date_filter: dict, filter_db_name: str):
        """Add date filter to the ShieldnetDefend query."""
        ShieldnetDefendDBQuery._filter_date(self, date_filter, filter_db_name)
        self.query += ' AND id != 0'
    ...
    def _sort_query(self, field: str) -> str:
        ...
        return ShieldnetDefendDBQuery._sort_query(self, field)

    def _add_search_to_query(self):
        """Add search to the ShieldnetDefend query with the specific implications.

        Since IDs are stored in the database as integers, id searches must be turned into integers to work as expected.
        """
        if self.search:
            del self.fields['id']
            ShieldnetDefendDBQuery._add_search_to_query(self)
            self.fields['id'] = 'id'
    ...
    def _process_filter(self, field_name: str, field_filter: str, q_filter: dict):
        ...
        if field_name == 'group' and q_filter['value'] is not None:
            ...
            if q_filter['operator'] == '=':
                ...
                raise ShieldnetDefendError(1409, f"Valid operators for 'group' field: {', '.join(valid_group_operators)}. "
                                       f"Used operator: {q_filter['operator']}")
        else:
            ShieldnetDefendDBQuery._process_filter(self, field_name, field_filter, q_filter)


class ShieldnetDefendDBQueryGroup(ShieldnetDefendDBQuery):
    """Class used to query ShieldnetDefend groups."""
    ...
    def __init__(self, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: dict = None,
        ...
        backend = ShieldnetDefendDBBackend(query_format='global')
        ShieldnetDefendDBQuery.__init__(self, offset=offset, limit=limit, table='`group`', sort=sort, search=search,
                              select=select,
...
class ShieldnetDefendDBQueryDistinctAgents(ShieldnetDefendDBQueryDistinct, ShieldnetDefendDBQueryAgents):
    pass


class ShieldnetDefendDBQueryGroupByAgents(ShieldnetDefendDBQueryGroupBy, ShieldnetDefendDBQueryAgents):
    ...
    def __init__(self, filter_fields: dict, *args: dict, **kwargs: dict):
        ...
        filter_fields : dict
            Fields to filter by.
        """
        ShieldnetDefendDBQueryAgents.__init__(self, *args, **kwargs)
        ShieldnetDefendDBQueryGroupBy.__init__(self, *args, table=self.table, fields=self.fields, filter_fields=filter_fields,
    ...
    def _format_data_into_dictionary(self) -> str:
        ...
        return ShieldnetDefendDBQuery._format_data_into_dictionary(self)


class ShieldnetDefendDBQueryMultigroups(ShieldnetDefendDBQueryAgents):
    ...
    def __init__(self, group_id: str, query: str = '', *args: dict, **kwargs: dict):
        ...
        ShieldnetDefendDBQueryAgents.__init__(self, query=query, *args, **kwargs)
...
class Agent:
    ...
    def load_info_from_db(self, select: list = None):
        ...
        """
        with ShieldnetDefendDBQueryAgents(offset=0, limit=None, sort=None, search=None, select=select,
    ...
    def reconnect(self, wq: ShieldnetDefendQueue) -> str:
        """Force reconnect to the manager.

        Parameters
        ----------
        wq : ShieldnetDefendQueue
            ShieldnetDefendQueue used for the active response message.

        Raises
        ------
        ShieldnetDefendError(1707)
            If the agent to be reconnected is not active.

        Returns
        -------
        str
            Message generated by ShieldnetDefend.
        """
        # Check if agent is active
        self.get_basic_information()
        if self.status.lower() != 'active':
            raise ShieldnetDefendError(1707)

        # Send force reconnect message to the ShieldnetDefendQueue
        ret_msg = wq.send_msg_to_agent(ShieldnetDefendQueue.HC_FORCE_RECONNECT, self.id)
    ...
    def remove(self, purge: bool = False) -> str:
        ...
        ShieldnetDefendInternalError(1757)
            Unhandled exception.
        ...
        str
            Message generated by ShieldnetDefend.
        """
        # Check that shieldnet-defend-authd is running
        try:
            manager_status = get_manager_status(cache=True)
        except ShieldnetDefendInternalError as e:
            # shieldnet-defend-authd is not running due to a problem with /proc availability
            raise ShieldnetDefendError(1726, extra_message=str(e))

        if manager_status.get('shieldnet-defend-authd') != 'running':
            # shieldnet-defend-authd is not running
            raise ShieldnetDefendError(1726)
        ...
        try:
            ...
            raise ShieldnetDefendInternalError(1757, extra_message=str(e))
    ...
    def _remove_authd(self, purge: bool = False) -> dict:
        ...
        authd_socket = ShieldnetDefendSocketJSON(common.AUTHD_SOCKET)
    ...
    def _add(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to ShieldnetDefend.
        2 uses:
        ...
        Raises
        ------
        ShieldnetDefendError(1706)
            If there is an agent with the same IP or the IP is invalid.
        ShieldnetDefendInternalError(1725)
        ...
        if ip != 'any':
            if ip.find('/') > 0:
                try:
                    ...
                    raise ShieldnetDefendError(1706, extra_message=ip)
            else:
                try:
        ...
        try:
            ...
            raise ShieldnetDefendInternalError(1725, extra_message=str(e))

    def _add_authd(self, name: str, ip: str, id: str = None, key: str = None, force: bool = None):
        """Add an agent to ShieldnetDefend using authd.
        2 uses:
            - name and ip [force]: Add an agent like manage_agents (generate id and key).
            - name, ip, id, key [force]: Insert an agent with an existing id and key.
        ...
        if key and len(key) < 64:
            raise ShieldnetDefendError(1709)
        ...
        try:
            ...
            if e.code == 9008:
                raise ShieldnetDefendError(1705, extra_message=name)
            elif e.code == 9007:
                raise ShieldnetDefendError(1706, extra_message=ip)
            elif e.code == 9012:
                raise ShieldnetDefendError(1708, extra_message=id)
            raise e
    ...
    def get_agent_os_name(self) -> str:
        ...
        query = ShieldnetDefendDBQueryAgents(select=['os.name'], filters={'id': [self.id]})
    ...
    def get_agents_overview(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort: dict = None, search: str = None,
        ...
        db_query = ShieldnetDefendDBQueryAgents(offset=offset, limit=limit, sort=sort, search=search, select=select,
    ...
    async def add_group_to_agent(group_id: str, agent_id: str, replace: bool = False, replace_list: list = None) -> str:
        ...
        Raises
        ------
        ShieldnetDefendInternalError(2007)
            Body is empty.
        ShieldnetDefendError(1752)
            Could not force single group for the agent.
        ShieldnetDefendError(1737)
    ...
    def set_agent_group_relationship(agent_id: str, group_id: str, remove: bool = False, override: bool = False):
        ...
        wdb = ShieldnetDefendDBConnection()
        try:
    ...
    async def unset_single_group_agent(agent_id: str, group_id: str, force: bool = False) -> str:
        ...
        ShieldnetDefendError(1734)
            Error removing agent from group.
        ShieldnetDefendError(1745)
        ...
        if group_id not in group_list:
            raise ShieldnetDefendError(1734)
        elif len(group_list) == 1:
            if group_id == 'default':
                raise ShieldnetDefendError(1745)
            else:
                set_default = True
    ...
    def get_config(self, component: str = '', config: str = '', agent_version: str = '') -> dict:
        ...
        agent_version : str
            Agent version to compare with the required version. The format is vX.Y.Z or ShieldnetDefend vX.Y.Z.

        Raises
        ------
        ShieldnetDefendError(1735)
        ...
        """
        if ShieldnetDefendVersion(agent_version) < ShieldnetDefendVersion(common.ACTIVE_CONFIG_VERSION):
            raise ShieldnetDefendInternalError(1735, extra_message=f"Minimum required version is {common.ACTIVE_CONFIG_VERSION}")
    ...
    def get_stats(self, component: str) -> dict:
        ...
        Raises
        ------
        ShieldnetDefendInternalError(1015)
            Agent version is null.
        ShieldnetDefendInternalError(1735)
        ...
        if self.version is None:
            raise ShieldnetDefendInternalError(1015)
        agent_version = ShieldnetDefendVersion(self.version.split(" ")[1])
        required_version = ShieldnetDefendVersion(AGENT_COMPONENT_STATS_REQUIRED_VERSION.get(component))
        if agent_version < required_version:
            raise ShieldnetDefendInternalError(1735, extra_message="Minimum required version is " + str(required_version))

        return stats.get_daemons_stats_from_socket(self.id, component)


def unify_shieldnet_defend_upgrade_version_format(upgrade_version: str) -> str:
...
def unify_shieldnet_defend_version_format(filters: dict):
    """Verify and format the specified shieldnetdefend version into the 'shieldnetdefend vX.Y.Z' standard.
    ...
    if wv is not None:
        if re.match(r'^v?\d+\.\d+\.\d+$', wv, re.IGNORECASE):
            filters['version'] = f"shieldnetdefend {'v' if 'v' not in wv else ''}{wv}"
        elif re.match(r'^shieldnetdefend \d+\.\d+\.\d+$', wv, re.IGNORECASE):
...
def send_restart_command(agent_id: str = '', agent_version: str = '', wq: ShieldnetDefendQueue = None) -> str:
    ...
    wq : ShieldnetDefendQueue
        ShieldnetDefendQueue used for the active response messages.
    ...
    str
        Message generated by ShieldnetDefend.
    """
    # If the ShieldnetDefend agent version is newer or equal to the AR legacy version,
    # the message sent will have JSON format
    if ShieldnetDefendVersion(agent_version) >= ShieldnetDefendVersion(common.AR_LEGACY_VERSION):
        ret_msg = wq.send_msg_to_agent(ShieldnetDefendQueue.RESTART_AGENTS_JSON, agent_id)
    else:
        ret_msg = wq.send_msg_to_agent(ShieldnetDefendQueue.RESTART_AGENTS, agent_id)
...
def core_upgrade_agents(agents_chunk: list, command: str = 'upgrade_result', wpk_repo: str = None, version: str = None,
    ...
    """
    msg = create_shieldnet_defend_socket_message(origin={'module': 'api'},
                                      command=command,
                                      parameters={
                                          'agents': agents_chunk,
                                          'version': unify_shieldnet_defend_upgrade_version_format(version),
                                          'force_upgrade': force,
                                          'use_http': use_http,
    ...
    s = ShieldnetDefendSocket(common.UPGRADE_SOCKET)
