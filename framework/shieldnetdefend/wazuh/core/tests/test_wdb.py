...
from shieldnetdefend.core.common import MAX_SOCKET_BUFFER_SIZE
...
def test_async_init():
    """Verify that AsyncShieldnetDefendDBConnection attributes are correct."""
    async_wdb = AsyncShieldnetDefendDBConnection('test')
...
async def test_async_open_connection(open_unix_connection_mock):
    ...
    async_wdb = AsyncShieldnetDefendDBConnection(loop='test_loop')
...
def test_async_close():
    ...
    async_wdb = AsyncShieldnetDefendDBConnection()
...
async def test_async_send_ko():
    ...
    with pytest.raises(exception.ShieldnetDefendInternalError, match=".* 2005 .*"):
    ...
    with pytest.raises(exception.ShieldnetDefendInternalError, match=r'\b2010\b'):
        await async_wdb._send('test')

    # Shieldnet-Defend-db error response.
    encoded_response = 'err Error message'.encode(encoding='utf-8')
    async_wdb._reader.readexactly.side_effect = [struct.pack('<I', len(encoded_response)), encoded_response]
    with pytest.raises(exception.ShieldnetDefendError, match=r'\b2003\b'):
        await async_wdb._send('test')


@pytest.mark.asyncio
async def test_run_wdb_command():
    """Test `ShieldnetDefendDBConnection.run_wdb_command` method."""
    send_result = ('status', '["data"]')
    command = "any wdb command"

    wdb_con = AsyncShieldnetDefendDBConnection()
    with patch('shieldnetdefend.core.wdb.AsyncShieldnetDefendDBConnection._send', return_value=send_result) as wdb_send_mock:
...
async def test_run_wdb_command_ko(wdb_response):
    """Test `ShieldnetDefendDBConnection.run_wdb_command` method expected exceptions."""
    with patch('shieldnetdefend.core.wdb.AsyncShieldnetDefendDBConnection._send', return_value=wdb_response):
        wdb_con = AsyncShieldnetDefendDBConnection()
        with pytest.raises(exception.ShieldnetDefendInternalError, match=".* 2007 .*") as expected_exc:
...
def test_failed_connection():
    ...
    with patch('shieldnetdefend.core.common.WDB_SOCKET', '/this/path/doesnt/exist'):
        with pytest.raises(exception.ShieldnetDefendException, match=".* 2005 .*"):
            ShieldnetDefendDBConnection()
...
def test_wrong_character_encodings_wdb(send_mock, connect_mock):
    """
    Tests receiving a text with a bad character encoding from shieldnetdefend db
    """
    ...
    with patch('socket.socket.recv', side_effect=recv_mock):
        mywdb = ShieldnetDefendDBConnection()
...
def test_failed_send_private(send_mock, connect_mock):
    ...
    with patch('socket.socket.recv', side_effect=recv_mock):
        mywdb = ShieldnetDefendDBConnection()
        with pytest.raises(exception.ShieldnetDefendException, match=".* 2003 .*"):
            mywdb._send('test_msg')

    with patch('socket.socket.recv', return_value=b'a' * (MAX_SOCKET_BUFFER_SIZE + 1)):
        mywdb = ShieldnetDefendDBConnection()
        with pytest.raises(exception.ShieldnetDefendException, match=".* 2009 .*"):
            mywdb._send('test_msg')
...
def test_query_input_validation_private(send_mock, connect_mock, error_query):
    mywdb = ShieldnetDefendDBConnection()
    with pytest.raises(exception.ShieldnetDefendException, match=".* 2004 .*"):
...
@patch("socket.socket.connect")
@patch("socket.socket.send")
@patch("shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send")
def test_execute(send_mock, socket_send_mock, connect_mock):
    ...
    with patch("shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send", new=send_mock):
...
def test_execute_pagination(socket_send_mock, connect_mock):
    ...
    # Test pagination
    with patch("shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send",
               side_effect=[[{'total': 5}], exception.ShieldnetDefendInternalError(2009), ['ok', '{"total": 5}'],
                            ['ok', '{"total": 5}']]):
        mywdb.execute("agent 000 sql select test from test offset 1 limit 500")

    # Test pagination error
    with patch("shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send",
               side_effect=[[{'total': 5}], exception.ShieldnetDefendInternalError(2009)]):
        with pytest.raises(exception.ShieldnetDefendInternalError, match=".* 2009 .*"):
            mywdb.execute("agent 000 sql select test from test offset 1 limit 1")
...
def test_failed_execute(send_mock, connect_mock, error_query, error_type, expected_exception, delete, update):
    ...
    if not error_type:
        ...
        with patch("shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send", return_value=[{'total': 5}]):
            with patch("shieldnetdefend.core.wdb.min", side_effect=error_type):
                with pytest.raises(exception.ShieldnetDefendException, match=f'.* {expected_exception} .*'):
                    mywdb.execute(error_query, delete=delete, update=update)
...
def test_Shieldnet_DefendDBConnection_loads(string):
    """Test that the `loads` method from the class `ShieldnetDefendDBConnection` cleans empty objects from the result."""
    result = ShieldnetDefendDBConnection.loads(string)
    assert len(result) == 1
