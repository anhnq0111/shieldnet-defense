...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        from shieldnetdefend.core.manager import *
...
@patch('shieldnetdefend.core.cluster.utils.glob')
...
def test_get_ossec_log_fields():
    ...
    assert result[1] == 'shieldnet-defend-rootcheck'
...
def test_get_ossec_logs(log_format):
    ...
    with patch("shieldnetdefend.core.manager.get_shieldnet_defend_active_logging_format", return_value=log_format):
        with pytest.raises(ShieldnetDefendInternalError, match=".*1020.*"):
            get_ossec_logs()

        with patch('shieldnetdefend.core.manager.exists', return_value=True):
...
def test_get_logs_summary(mock_exists, mock_active_logging_format):
    ...
    with patch('shieldnetdefend.core.manager.tail', return_value=logs):
        ...
        assert result['shieldnet-defend-modulesd:database'] == {'all': 2, 'info': 0, 'error': 0, 'critical': 0, 'warning': 0,
                                                     'debug': 2}


@patch('shieldnetdefend.core.manager.exists', return_value=True)
@patch('shieldnetdefend.core.manager.ShieldnetDefendSocket')
def test_validate_ossec_conf(mock_shieldnet_defendsocket, mock_exists):
    with patch('socket.socket') as sock:
        # Mock sock response
        json_response = json.dumps({'error': 0, 'message': ""}).encode()
        mock_shieldnet_defendsocket.return_value.receive.return_value = json_response
        result = validate_ossec_conf()

        assert result == {'status': 'OK'}
        mock_exists.assert_called_with(os.path.join(common.SHIELDNET_DEFEND_PATH, 'queue', 'sockets', 'com'))
...
def test_validation_ko(mock_exists):
    ...
    with patch('socket.socket.bind'):
        ...
        with patch("shieldnetdefend.core.manager.exists", return_value=False):
            with pytest.raises(ShieldnetDefendInternalError, match='.* 1901 .*'):
                validate_ossec_conf()

        with patch('socket.socket.connect'):
            # Socket send raise socket.error
            with patch('shieldnetdefend.core.manager.ShieldnetDefendSocket.send', side_effect=socket.error):
                with pytest.raises(ShieldnetDefendInternalError, match='.* 1014 .*'):
                    validate_ossec_conf()

            with patch('socket.socket.send'):
                # Socket recv raise socket.error
                with patch('shieldnetdefend.core.manager.ShieldnetDefendSocket.receive', side_effect=socket.timeout):
                    with pytest.raises(ShieldnetDefendInternalError, match='.* 1014 .*'):
                        validate_ossec_conf()

                # _parse_execd_output raise KeyError
                with patch('shieldnetdefend.core.manager.ShieldnetDefendSocket'):
                    with patch('shieldnetdefend.core.manager.parse_execd_output', side_effect=KeyError):
                        with pytest.raises(ShieldnetDefendInternalError, match='.* 1904 .*'):
                            validate_ossec_conf()
...
def test_parse_execd_output(error_flag, error_msg):
    ...
    if not error_flag:
        ...
        with pytest.raises(ShieldnetDefendException, match=f'.* 1908 .*'):
            parse_execd_output(json_response)


@patch('shieldnetdefend.core.manager.configuration.api_conf', new={'experimental_features': True})
...
def test_get_update_information_template(last_check_date, update_check, installation_uid):
    ...
    assert template['current_version'] == f"v{shieldnetdefend.__version__}"
            ...
            release_info.append(
                {
                    ...
                    'title': f'ShieldnetDefend {semver}',
...
async def test_query_update_check_service_request(
    ...
    version = '4.8.0'
    with patch('framework.shieldnetdefend.core.manager.shieldnetdefend.__version__', version):
        ...
        client_session_get_mock.assert_called_with(
            ...
            headers={
                ...
                SHIELDNET_DEFEND_TAG_KEY: f'v{version}',
                USER_AGENT_KEY: f'ShieldnetDefend UpdateCheckService/v{version}'
            },
