...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        from shieldnetdefend.core.agent import *
        from shieldnetdefend.core.exception import ShieldnetDefendException
        from api.util import remove_nones_to_dict
        from shieldnetdefend.rbac.utils import RESOURCES_CACHE
...
def get_Shieldnet_DefendDBQuery_params(wdb_class):
    """Get default parameters for the specified ShieldnetDefendDBQuery class.

    Parameters
    ----------
    wdb_class : str
        Suffix of the ShieldnetDefendDBQuery class. Example: 'Agents' to get default parameters from `ShieldnetDefendDBQueryAgents` class.
    ...
    """
    with patch('shieldnetdefend.core.agent.ShieldnetDefendDBQuery.__init__') as wdbquery_mock:
        getattr(sys.modules[__name__], f'ShieldnetDefendDBQuery{wdb_class}')()
        return wdbquery_mock.call_args.kwargs


# list with ShieldnetDefend packages availables with their hash
...
@patch("shieldnetdefend.core.agent.ShieldnetDefendDBBackend")
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents__init__(socket_mock, send_mock, backend_mock, value):
    """Tests if method __init__ of ShieldnetDefendDBQueryAgents works properly.
    ...
    if value:
        ShieldnetDefendDBQueryAgents()
        backend_mock.assert_called_once()
    else:
        with pytest.raises(ShieldnetDefendException, match=".* 2005 .*"):
            ShieldnetDefendDBQueryAgents()


@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents_filter_date(mock_socket_conn):
    """Tests _filter_date of ShieldnetDefendDBQueryAgents returns expected query"""
    query_agent = ShieldnetDefendDBQueryAgents()
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents_sort_query(mock_socket_conn, field, expected_query):
    """Tests _sort_query of ShieldnetDefendDBQueryAgents returns expected result
    ...
    """
    query_agent = ShieldnetDefendDBQueryAgents(sort={'order': 'asc'})
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents_add_search_to_query(mock_socket_conn):
    """Tests _add_search_to_query of ShieldnetDefendDBQueryAgents returns expected query"""
    query_agent = ShieldnetDefendDBQueryAgents(search={'value': 'test', 'negation': True})
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents_format_data_into_dictionary(mock_socket_conn, data):
    """Tests _format_data_into_dictionary of ShieldnetDefendDBQueryAgents returns expected data"""
    query_agent = ShieldnetDefendDBQueryAgents(offset=0, limit=1, sort=None,
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents_parse_legacy_filters(mock_socket_conn):
    """Tests _parse_legacy_filters of ShieldnetDefendDBQueryAgents returns expected query"""
    query_agent = ShieldnetDefendDBQueryAgents(filters={'older_than': 'test'})
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryAgents_process_filter(mock_socket_conn, field_name, field_filter, q_filter):
    """Tests _process_filter of ShieldnetDefendDBQueryAgents returns expected query
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryGroup__init__(socket_mock, send_mock, backend_mock, value):
    """Test if method __init__ of ShieldnetDefendDBQueryGroup works properly.
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryGroup_filters(socket_mock, send_mock, filters):
    """Test if parameter filters of ShieldnetDefendDBQueryGroup works properly.
        ...
        """
    query_group = ShieldnetDefendDBQueryGroup(filters=filters)
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryGroupByAgents__init__(mock_socket_conn):
    """Tests if method __init__ of ShieldnetDefendDBQueryGroupByAgents works properly."""
    query_group = ShieldnetDefendDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryGroupByAgents_format_data_into_dictionary(mock_socket_conn):
    """Tests if method _format_data_into_dictionary of ShieldnetDefendDBQueryGroupByAgents works properly."""
    query_group = ShieldnetDefendDBQueryGroupByAgents(filter_fields=['name', 'os.name'], offset=0, limit=1, sort={'order': 'asc'},
                                            search={'value': 'test', 'negation': True},
                                            select={'os.name'}, query=None, count=5, get_data=None)

    query_group.filter_fields = {'fields': set(query_group.filter_fields)}
    query_group._data = [{'count': 1, 'name': 'shieldnet-defend-master'},
                         {'count': 1, 'name': 'shieldnet-defend-agent1'}]
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryGroupByAgents(mock_socket_conn, send_mock, filter_fields, expected_response):
    """Tests if ShieldnetDefendDBQueryGroupByAgents works properly."""
    query_group = ShieldnetDefendDBQueryGroupByAgents(filter_fields=filter_fields, offset=0, limit=None, sort=None,
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryGroup__add_sort_to_query(mock_socket_conn, send_mock):
    """Tests if _add_sort_to_query method of ShieldnetDefendDBQueryGroup works properly"""
    query_group = ShieldnetDefendDBQueryGroup()
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryMultigroups__init__(mock_socket_conn):
    """Tests if method __init__ of ShieldnetDefendDBQueryMultigroups works properly."""
    query_multigroups = ShieldnetDefendDBQueryMultigroups(group_id='test')
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryMultigroups_default_query(mock_socket_conn, group_id):
    """Tests if method _default_query of ShieldnetDefendDBQueryMultigroups works properly.
    ...
    """
    query_multigroups = ShieldnetDefendDBQueryMultigroups(group_id=group_id)
...
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryMultigroups_default_count_query(mock_socket_conn):
    """Tests if method _default_count_query of ShieldnetDefendDBQueryMultigroups works properly."""
    query_multigroups = ShieldnetDefendDBQueryMultigroups(group_id='test')
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_Shieldnet_DefendDBQueryMultigroups_get_total_items(mock_socket_conn, send_mock):
    """Tests if method _get_total_items of ShieldnetDefendDBQueryMultigroups works properly."""
    query_multigroups = ShieldnetDefendDBQueryMultigroups(group_id='test')
...
@patch('shieldnetdefend.core.agent.Agent._add')
...
def test_agent_load_info_from_db_ko(socket_mock, send_mock):
    ...
    with pytest.raises(ShieldnetDefendResourceNotFound, match='.* 1701 .*'):
...
def test_agent_get_key_ko(socket_mock, send_mock):
    ...
    with pytest.raises(ShieldnetDefendError, match='.* 1703 .*'):
        agent = Agent('000')
        agent.get_key()


@patch('shieldnetdefend.core.agent.ShieldnetDefendQueue.send_msg_to_agent')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect(socket_mock, send_mock, mock_send_msg):
    ...
    agent.reconnect(ShieldnetDefendQueue(common.AR_SOCKET))

    # Assert send_msg method is called with correct params
    mock_send_msg.assert_called_with(ShieldnetDefendQueue.HC_FORCE_RECONNECT, agent_id)


@patch('shieldnetdefend.core.agent.ShieldnetDefendQueue')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_reconnect_ko(socket_mock, send_mock, mock_queue):
    ...
    with pytest.raises(ShieldnetDefendError, match='.* 1707 .*'):
        agent = Agent('003')
        agent.reconnect(mock_queue)


@patch('shieldnetdefend.core.agent.Agent._remove_authd', return_value='Agent was successfully deleted')
def test_agent_remove(mock_remove_authd):
    """Tests if method remove() works as expected."""

    with patch('shieldnetdefend.core.agent.get_manager_status', return_value={'shieldnet-defend-authd': 'running'}):
        agent = Agent('000')
        result = agent.remove()
...
def test_agent_remove_ko(mock_remove_authd):
    ...
    with pytest.raises(ShieldnetDefendError, match='.* 1726 .*'):
        agent = Agent('000')
        agent.remove()


@patch('shieldnetdefend.core.agent.ShieldnetDefendSocketJSON')
def test_agent_remove_authd(mock_shieldnet_defend_socket):
    """Tests if method remove_authd() works as expected"""
    agent = Agent('000')
    agent._remove_authd(purge=True)
    mock_shieldnet_defend_socket.return_value.send.assert_called_once_with(
        {"function": "remove", "arguments": {"id": str(0).zfill(3), "purge": True}})
    mock_shieldnet_defend_socket.return_value.receive.assert_called_once()
    mock_shieldnet_defend_socket.return_value.close.assert_called_once()
...
@patch('shieldnetdefend.core.agent.Agent._add_authd')
...
@patch('shieldnetdefend.core.agent.get_manager_status', return_value={'shieldnet-defend-authd': 'stopped'})
def test_agent_add_ko(mock_maganer_status):
    """Test if _add() method raises expected exception."""
    agent = Agent('001')

    with pytest.raises(ShieldnetDefendError, match='.* 1706 .*'):
...
@patch('shieldnetdefend.core.agent.ShieldnetDefendSocketJSON')
def test_agent_add_authd(mock_shieldnet_defend_socket, name, ip, id, key, force):
    ...
    mock_shieldnet_defend_socket.return_value.send.assert_called_once_with(socket_msg)


@pytest.mark.parametrize("mocked_exception, expected_exception", [
    (None, None),
    (ShieldnetDefendError(9008, cmd_error=True), ".* 1705 .*"),
    (ShieldnetDefendError(9007, cmd_error=True), ".* 1706 .*"),
    (ShieldnetDefendError(9012, cmd_error=True), ".* 1708 .*"),
    (ShieldnetDefendError(9000, cmd_error=True), ".* None")
])
@patch('shieldnetdefend.core.agent.ShieldnetDefendSocketJSON')
def test_agent_add_authd_ko(mock_shieldnet_defend_socket, mocked_exception, expected_exception):
    """Tests if method _add_authd() raises expected exception"""
    agent = Agent('001')

    if not mocked_exception:
        with pytest.raises(ShieldnetDefendError, match=".* 1709 .*"):
            agent._add_authd('test_add', '192.168.0.1', '2', 'adsiojew')
    else:
        mock_shieldnet_defend_socket.return_value.receive.side_effect = mocked_exception
        with pytest.raises(ShieldnetDefendError, match=expected_exception):
...
@patch('shieldnetdefend.core.agent.rmtree')
@patch('shieldnetdefend.core.agent.path.exists', return_value=True)
@patch('shieldnetdefend.core.common.SHARED_PATH', new=os.path.join(test_data_path, 'etc', 'shared'))
...
@patch('shieldnetdefend.core.agent.Agent.get_agent_groups', new_callable=AsyncMock)
@patch('shieldnetdefend.core.agent.Agent.set_agent_group_relationship')
...
@patch('shieldnetdefend.core.agent.Agent.get_agent_groups', return_value=['default'])
async def test_agent_add_group_to_agent_ko(agent_groups_mock):
    ...
    with patch('shieldnetdefend.core.agent.Agent.get_agent_groups', side_effect=ShieldnetDefendError(2003)):
        with pytest.raises(ShieldnetDefendInternalError, match='.* 2007 .*'):
...
def test_agent_group_exists_ko():
    ...
    with pytest.raises(ShieldnetDefendError, match='.* 1722 .*'):
...
@patch('shieldnetdefend.core.agent.ShieldnetDefendDBConnection.send')
@patch('socket.socket.connect')
...
def test_agent_set_agent_group_relationship_ko(socket_connect_mock):
    ...
    with pytest.raises(ShieldnetDefendInternalError, match='.* 2005 .*'):
...
@patch('shieldnetdefend.core.agent.Agent.set_agent_group_relationship')
@patch('shieldnetdefend.core.agent.Agent.group_exists', return_value=True)
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
async def test_agent_unset_single_group_agent(agent_info_mock, group_exists_mock, set_agent_group_mock, agent_id, group_id,
    ...
    """
    with patch('shieldnetdefend.core.agent.Agent.get_agent_groups', return_value=previous_groups):
...
async def test_agent_unset_single_group_agent_ko(socket_mock, agent_information_mock):
    ...
    with patch('shieldnetdefend.core.agent.Agent.group_exists', return_value=False):
        with pytest.raises(ShieldnetDefendResourceNotFound, match='.* 1710 .*'):
            await Agent.unset_single_group_agent('002', 'test_group')

    # Agent does not belong to group
    with patch('shieldnetdefend.core.agent.Agent.get_agent_groups', return_value=['new_group', 'new_group2']):
        # Group_id is not within group_list
        with pytest.raises(ShieldnetDefendError, match='.* 1734 .*'):
            await Agent.unset_single_group_agent('002', 'test_group', force=True)

    # Group ID is 'default' and it is the last only one remaining
    with patch('shieldnetdefend.core.agent.Agent.get_agent_groups', return_value=['default']):
        # Agent file does not exists
        with pytest.raises(ShieldnetDefendError, match='.* 1745 .*'):
            await Agent.unset_single_group_agent('002', 'default', force=True)


@patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config(socket_mock, send_mock, mock_shieldnet_defend_socket):
    """Test getconfig method returns expected message."""
    agent = Agent('001')
    mock_shieldnet_defend_socket.return_value.receive.return_value = b'ok {"test": "conf"}'
    result = agent.get_config('com', 'active-response', 'ShieldnetDefend v4.0.0')
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_config_ko(socket_mock, send_mock, mock_shieldnet_defend_socket):
    """Test getconfig method raises expected exceptions."""
    # Invalid component
    agent = Agent('003')
    with pytest.raises(ShieldnetDefendError, match=".* 1101 .*"):
        agent.get_config('invalid_component', 'active-response', 'ShieldnetDefend v4.0.0')

    # Component or config is none
    agent = Agent('003')
    with pytest.raises(ShieldnetDefendError, match=".* 1307 .*"):
        agent.get_config('com', None, 'ShieldnetDefend v4.0.0')
        agent.get_config(None, 'active-response', 'ShieldnetDefend v4.0.0')

    # Agent ShieldnetDefend version is lower than ACTIVE_CONFIG_VERSION
    agent = Agent('002')
    with pytest.raises(ShieldnetDefendInternalError, match=".* 1735 .*"):
        agent.get_config('com', 'active-response', 'ShieldnetDefend v3.6.0')


@patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket')
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats(socket_mock, send_mock, mock_shieldnet_defend_socket):
    """Test get_stats method returns expected message."""
    agent = Agent('001')
    mock_shieldnet_defend_socket.return_value.receive.return_value = b'{"error":0, "data":{"global":{}, "interval":{}}}'
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection._send', side_effect=send_msg_to_wdb)
@patch('socket.socket.connect')
def test_agent_get_stats_ko(socket_mock, send_mock, mock_shieldnet_defend_socket):
    """Test get_stats method raises expected exception when the agent's version is lower than required."""
    agent = Agent('002')
    with pytest.raises(ShieldnetDefendInternalError, match=r'\b1735\b'):
        agent.get_stats('logcollector')


@pytest.mark.parametrize('agents_list, versions_list', [
    (['001', '002', '003', '004'],
     [{'version': ver} for ver in ['ShieldnetDefend v4.2.0', 'ShieldnetDefend v4.0.0', 'ShieldnetDefend v4.2.1', 'ShieldnetDefend v3.13.2']])
])
@patch('shieldnetdefend.core.agent.ShieldnetDefendQueue.send_msg_to_agent')
@patch('shieldnetdefend.core.agent.ShieldnetDefendQueue.__init__', return_value=None)
def test_send_restart_command(wq_mock, wq_send_msg, agents_list, versions_list):
    ...
    """
    with patch('shieldnetdefend.core.agent.Agent.get_basic_information', side_effect=versions_list):
        for agent_id, agent_version in zip(agents_list, versions_list):
            wq = ShieldnetDefendQueue(common.AR_SOCKET)
            send_restart_command(agent_id, agent_version['version'], wq)
            expected_msg = ShieldnetDefendQueue.RESTART_AGENTS_JSON if ShieldnetDefendVersion(
                agent_version['version']) >= ShieldnetDefendVersion(common.AR_LEGACY_VERSION) else ShieldnetDefendQueue.RESTART_AGENTS
...
def test_get_agents_info():
    ...
    with patch('shieldnetdefend.core.agent.open', mock_open(read_data=client_keys)) as m:
...
def test_get_groups():
    ...
    with patch('shieldnetdefend.core.common.SHARED_PATH', new=shared):
        try:
...
def test_expand_group(socket_mock, group, wdb_response, expected_agents):
    ...
    wdb_response: list
        Mock return values for the `ShieldnetDefendDBConnection.send` method.
    expected_agents : set
    ...
    with patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection.send', side_effect=wdb_response):
...
@patch('shieldnetdefend.core.agent.core_upgrade_agents')
