...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.core import decoder
...
@pytest.mark.parametrize("detail, value, details, expected_result", [
    ("parent", "shieldnetdefend", {'regex': ['regex00']}, {'parent': 'shieldnetdefend', 'regex': ['regex00']}),
...
@pytest.mark.parametrize("status, expected_result", [
    ...
    ("enabled", "enabled"),
    ("disabled", "disabled"),
    ("wrong", ShieldnetDefendError(1202))
])
...
@pytest.mark.parametrize("filename, relative_dirname, status, permissions, exception", [
    ...
    ('wrong_decoders.xml', 'decoders', "all", 777, ShieldnetDefendInternalError(1501)),
    ('non_existing.xml', 'decoders', "disabled", 777, ShieldnetDefendError(1502)),
    ('test1_decoders.xml', 'decoders', "all", 000, ShieldnetDefendError(1502)),
])
@patch('shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH', new=test_data_path)
