...
from shieldnetdefend.core.analysis import (
    is_ruleset_file,
    log_ruleset_reload_response,
    RulesetReloadResponse,
    send_reload_ruleset_and_get_results
)
from shieldnetdefend.core.common import SHIELDNET_DEFEND_PATH
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (os.path.join(SHIELDNET_DEFEND_PATH, "etc/lists/file.txt"), True),
        (os.path.join(SHIELDNET_DEFEND_PATH, "etc/rules/file.txt"), True),
        (os.path.join(SHIELDNET_DEFEND_PATH, "etc/decoders/file.txt"), True),
...
def test_send_reload_ruleset_and_get_results(socket_response_dict, expected_affected, expected_failed, expected_msg):
    ...
    node_id = "test-node"
    results = AffectedItemsShieldnetDefendResult(all_msg="test")
    with patch("shieldnetdefend.core.analysis.send_reload_ruleset_msg") as mock_send:
        ...
        if expected_failed:
            ...
            for error_obj, node_ids in updated_results.failed_items.items():
                if isinstance(error_obj, ShieldnetDefendError) and node_id in node_ids:
