...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.core import rule
...
@pytest.mark.parametrize('status, expected_result', [
    ...
    (None, 'all'),
    ('unexistent', ShieldnetDefendError(1202))
])
...
@pytest.mark.parametrize('rule_file, rule_path, rule_status, exception', [
    ...
    ('noexists.xml', 'tests/data/rules', 'enabled', ShieldnetDefendError(1201))
])
@patch("shieldnetdefend.core.common.SHIELDNET_DEFEND_PATH", new=parent_directory)
@patch("shieldnetdefend.core.common.RULES_PATH", new=data_path)
...
@patch("shieldnetdefend.core.rule.load_shieldnet_defend_xml", side_effect=OSError(13, 'Error', 'Permissions'))
def test_load_rules_from_file_permissions(mock_load):
    """Test set_groups rule core function."""
    with pytest.raises(ShieldnetDefendError, match='.* 1207 .*'):
        rule.load_rules_from_file('nopermissions.xml', 'tests/data/rules', 'disabled')


@patch("shieldnetdefend.core.rule.load_shieldnet_defend_xml", side_effect=OSError(8, 'Error', 'Unknown'))
