...
from shieldnetdefend.tests.test_security import db_setup  # noqa
...
def test_revoke_tokens(db_setup):
    """Checks that the return value of revoke_tokens is a ShieldnetDefendResult.
    ...
    db_setup: callable
        This function creates the rbac.db file.
    """
    with patch('shieldnetdefend.core.security.change_keypair', side_effect=None):
        security, ShieldnetDefendResult, _ = db_setup
        mock_current_user = ContextVar('current_user', default='shieldnetdefend')
        with patch("shieldnetdefend.sca.common.current_user", new=mock_current_user):
...
def test_invalid_roles_tokens(db_setup, role_list, expected_roles):
    ...
    expected_roles : set
        Expected roles.
    """
    with patch('shieldnetdefend.core.security.TokenManager.add_user_roles_rules') as TM_mock:
...
@patch('shieldnetdefend.core.security.TokenManager.add_user_roles_rules')
...
@patch("shieldnetdefend.core.security.revoke_tokens")
@patch("shieldnetdefend.core.security.check_database_integrity")
@patch("shieldnetdefend.core.security.os.remove")
