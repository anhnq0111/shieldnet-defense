...
from shieldnetdefend.core.cluster.control import get_health, get_nodes, get_node_ruleset_integrity
from shieldnetdefend.core.cluster.utils import get_cluster_status, read_cluster_config, read_config
...
def read_config_wrapper() -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg='All selected information was returned',
...
@expose_resources(actions=['cluster:read'], resources=[f'node:id:{node_id}'])
def get_node_wrapper() -> AffectedItemsShieldnetDefendResult:
...
def get_status_json() -> ShieldnetDefendResult:
    """Return the cluster status.

    Returns
    -------
    ShieldnetDefendResult
        ShieldnetDefendResult object with the cluster status.
    """
    return ShieldnetDefendResult({'data': get_cluster_status()})
...
async def get_health_nodes(lc: local_client.LocalClient,
                           filter_node: Union[str, list] = None) -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg='All selected nodes healthcheck information was returned',
...
async def get_nodes_info(lc: local_client.LocalClient, filter_node: Union[str, list] = None,
                         **kwargs: dict) -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg='All selected nodes information was returned',
    ...
    for node in non_existent_nodes:
        result.add_failed_item(id_=node, error=ShieldnetDefendResourceNotFound(1730))
...
async def get_ruleset_sync_status(master_md5: dict = None):
    ...
    """
    result = AffectedItemsShieldnetDefendResult(all_msg="Nodes ruleset synchronization status was successfully read",
