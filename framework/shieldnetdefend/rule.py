...
from shieldnetdefend.core.rule import check_status, load_rules_from_file, format_rule_decoder_file, REQUIRED_FIELDS, \
    RULE_REQUIREMENTS, SORT_FIELDS, RULE_FIELDS, RULE_FILES_FIELDS, RULE_FILES_REQUIRED_FIELDS
from shieldnetdefend.core.utils import process_array, safe_move, \
    validate_shieldnet_defend_xml, upload_file, full_copy, to_relative_path
...
def get_rules(rule_ids: list = None, status: str = None, group: str = None, pci_dss: str = None, gpg13: str = None,
    ...
    """
    result = AffectedItemsShieldnetDefendResult(none_msg='No rule was returned',
    ...
    if level:
        ...
        if len(levels) < 0 or len(levels) > 2:
            raise ShieldnetDefendError(1203)
    ...
    for rule_id in no_existent_ids:
        result.add_failed_item(id_=rule_id, error=ShieldnetDefendError(1208))
...
def get_rules_files(status: str = None, relative_dirname: str = None, filename: list = None, offset: int = 0,
    ...
    Raises
    ------
    ShieldnetDefendError(1200)
        Error reading rules from `SHIELDNET_DEFEND_HOME/etc/ossec.conf`.
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(none_msg='No rule files were returned',
    ...
    if not ruleset_conf:
        raise ShieldnetDefendError(1200)
    rules_files = list()
...
def get_groups(offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None, sort_ascending: bool = True,
               search_text: str = None, complementary_search: bool = False,
               search_in_fields: list = None) -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Dictionary: {'items': array of items, 'totalItems': Number of items (without applying the limit)}
    """
    result = AffectedItemsShieldnetDefendResult(none_msg='No groups in rules were returned',
...
def get_requirement(requirement: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT, sort_by: dict = None,
    ...
    if requirement not in RULE_REQUIREMENTS:
        result.add_failed_item(id_=requirement,
                               error=ShieldnetDefendError(1205, extra_message=requirement,
...
def get_rule_file_path(filename: str = None, relative_dirname: str = None) -> str:
    ...
    if len(rules) == 0:
        ...
        return join(common.SHIELDNET_DEFEND_PATH, rule['relative_dirname'], filename)
    else:
        return normpath(join(common.SHIELDNET_DEFEND_PATH, rules[0]['relative_dirname'], filename))
...
def get_rule_file(filename: str = None, raw: bool = False,
    ...
    if not full_path:
        result.add_failed_item(id_=filename,
                               error=ShieldnetDefendError(1415, extra_message=f"{filename}"))
        return result
    ...
    try:
        ...
        result.add_failed_item(id_=filename,
                               error=ShieldnetDefendError(1413, extra_message=f"{filename}: {str(exc)}"))
    except OSError:
        result.add_failed_item(id_=filename,
                               error=ShieldnetDefendError(1414, extra_message=f"{filename}"))
...
def validate_upload_delete_dir(relative_dirname: Union[str, None]) -> Tuple[str, ShieldnetDefendError]:
    ...
    Tuple (str, ShieldnetDefendError)
        ...
        The second element of the tuple is a ShieldnetDefendError exception
            If relative_dirname has no 'rule_dir' tag in ruleset return ShieldnetDefendError(1505).
            If relative_dirname is inside the default RULES_PATH return ShieldnetDefendError(1506).
            If relative_dirname has a 'rule_dir' tag in ruleset but it doesn't exists return ShieldnetDefendError(1507).
    ...
    if relative_dirname not in ruleset_conf['rule_dir']:
        shieldnet_defend_error = ShieldnetDefendError(1209)
    elif commonpath([join(common.SHIELDNET_DEFEND_PATH, relative_dirname), common.RULES_PATH]) == common.RULES_PATH:
        shieldnet_defend_error = ShieldnetDefendError(1210)
    elif not exists(join(common.SHIELDNET_DEFEND_PATH, relative_dirname)):
        shieldnet_defend_error = ShieldnetDefendError(1211)
...
def upload_rule_file(filename: str, content: str, relative_dirname: str = None,
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg='Rule was successfully uploaded',
                                      none_msg='Could not upload rule')

    backup_file = ''
    try:
        relative_dirname, shieldnet_defend_error = validate_upload_delete_dir(
...
def delete_rule_file(filename: Union[str, list], relative_dirname: str = None) -> AffectedItemsShieldnetDefendResult:
    """Delete a rule file.
    ...
    result = AffectedItemsShieldnetDefendResult(all_msg='Rule was successfully deleted',
