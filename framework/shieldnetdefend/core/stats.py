...
from shieldnetdefend.core import common, utils
from shieldnetdefend.core import shieldnet_defend_socket
from shieldnetdefend.core.exception import ShieldnetDefendError, ShieldnetDefendInternalError, ShieldnetDefendException
...
def totals_(date: datetime.datetime = utils.get_utc_now()) -> list:
    ...
    try:
        ...
        raise ShieldnetDefendError(1308, extra_message=stat_filename)
        ...
        if len(data) == 4:
            ...
            if len(data) != 5:
                if len(data) in (0, 1):
                    ...
                    raise ShieldnetDefendInternalError(1309)
...
def get_daemons_stats_socket(socket: str, agents_list: Union[list[int], str] = None, last_id: int = None) -> dict:
    """Send message to ShieldnetDefend socket to get statistical information.
    ...
    """
    # Create message
    full_message = shieldnet_defend_socket.create_shieldnet_defend_socket_message(
    ...
    # Connect to socket
    try:
        s = shieldnet_defend_socket.ShieldnetDefendSocketJSON(socket)
    except Exception:
        raise ShieldnetDefendInternalError(1121, extra_message=socket)
...
def get_daemons_stats_(filename: str) -> list:
    ...
    try:
        ...
        try:
            ...
            raise ShieldnetDefendInternalError(1104, extra_message=str(e))
    except IOError:
        raise ShieldnetDefendError(1308, extra_message=filename)
...
def get_stats_socket_path(agent_id: Union[str, int], daemon: str) -> str:
    ...
    if is_agent_a_manager(agent_id):
        return os.path.join(common.SHIELDNET_DEFEND_PATH, "queue", "sockets", daemon)
    else:
...
def send_command_to_socket(dest_socket: str, command: str) -> dict:
    ...
    except ShieldnetDefendException:
...
def get_daemons_stats_from_socket(agent_id: str, daemon: str) -> dict:
    ...
    if not check_if_daemon_exists_in_agent(agent_id=agent_id, daemon=daemon):
        raise ShieldnetDefendError(1310)
    ...
    if socket_response.get('error', 0) != 0:
        ...
        raise ShieldnetDefendError(1117, extra_message=rec_msg)
