...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        ...
        from shieldnetdefend.core import common, stats
        from shieldnetdefend.core.exception import ShieldnetDefendError, ShieldnetDefendException, ShieldnetDefendInternalError
...
def test_totals_ko_():
    ...
    with patch('shieldnetdefend.core.stats.open', side_effect=IOError):
        with pytest.raises(ShieldnetDefendException, match=".* 1308 .*"):
            stats.totals_(date(1996, 8, 13))

    with patch("builtins.open", mock_open(read_data='15-571-3-2\n15--107--1483')):
        with pytest.raises(ShieldnetDefendInternalError, match=".* 1309 .*"):
            stats.totals_(date(1996, 8, 13))


@patch('shieldnetdefend.core.common.STATS_PATH', new=test_data_path)
def test_weekly_():
...
@patch('shieldnetdefend.core.common.STATS_PATH', new='')
...
@pytest.mark.parametrize('agent, daemon, expected_value', [
    (0, 'logcollector', os.path.join(common.SHIELDNET_DEFEND_PATH, "queue", "sockets", 'logcollector')),
...
@patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocketJSON.close')
@patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocketJSON.send')
@patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocketJSON.__init__', return_value=None)
def test_get_daemons_stats_socket(mock__init__, mock_send, mock_close, agents_list, expected_socket_response,
    ...
    with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocketJSON.receive',
...
def test_get_daemons_stats_socket_ko(agents_list):
    ...
    socket = '/test_path/socket'
    with pytest.raises(ShieldnetDefendInternalError, match=f".* 1121 .*: {socket}"):
...
def test_get_daemons_stats_ko():
    ...
    with patch('shieldnetdefend.core.stats.open'):
        with pytest.raises(ShieldnetDefendInternalError, match=".* 1104 .*"):
            stats.get_daemons_stats_('filename')
...
def test_get_daemons_stats_from_socket(agent_id, daemon, response):
    ...
    with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.__init__', return_value=None) as mock_socket:
        with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.send', side_effect=None) as mock_send:
            with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.receive', return_value=response.encode()):
                with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.close', side_effect=None):
                    stats.get_daemons_stats_from_socket(agent_id, daemon)

        if agent_id == '000':
            mock_socket.assert_called_once_with(os.path.join(common.SHIELDNET_DEFEND_PATH, "queue", "sockets", "logcollector"))
            mock_send.assert_called_once_with(b'getstate')
        else:
            mock_socket.assert_called_once_with(os.path.join(common.SHIELDNET_DEFEND_PATH, "queue", "sockets", "remote"))
...
def test_get_daemons_stats_from_socket(agent_id, daemon, responses, expected, expected_socket_calls,
    ...
    with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.__init__', return_value=None) as mock_socket:
        with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.send', side_effect=None) as mock_send:
            with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.receive', side_effect=responses):
                with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.close', side_effect=None):
...
def test_get_daemons_stats_from_socket_ko():
    ...
    with pytest.raises(ShieldnetDefendError, match=r'\b1307\b'):
        stats.get_daemons_stats_from_socket(None, None)

    with pytest.raises(ShieldnetDefendError, match=r'\b1310\b'):
        stats.get_daemons_stats_from_socket('000', 'agent')

    with pytest.raises(ShieldnetDefendInternalError, match=r'\b1121\b'):
        stats.get_daemons_stats_from_socket('000', 'logcollector')

    with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.__init__', return_value=None):
        with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.close', side_effect=None):
            with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.send', side_effect=None):
                with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.receive', side_effect=ValueError):
                    with pytest.raises(ShieldnetDefendInternalError, match=r'\b1118\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')

                with patch('shieldnetdefend.core.shieldnet_defend_socket.ShieldnetDefendSocket.receive',
                           return_value=json.dumps({'error': 1}).encode()):
                    with pytest.raises(ShieldnetDefendError, match=r'\b1117\b'):
                        stats.get_daemons_stats_from_socket('000', 'logcollector')
