...
@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBBackend")
@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBQuery.__init__")
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
def test_Shieldnet_DefendDBQueryRootcheck_init(mock_info, mock_shieldnet_defendDBQuery, mock_backend):
    """Test if ShieldnetDefendDBQuery and ShieldnetDefendDBBackend are called with expected parameters"""
    test = rootcheck.ShieldnetDefendDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='test', count=True, get_data=True, distinct=False, filters=None,
                                           fields={})
    mock_backend.assert_called_with('100')
    mock_shieldnet_defendDBQuery.assert_called_with(ANY, offset=1, limit=1, table='pm_event', sort=None, search='test',
...
@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBBackend")
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
def test_Shieldnet_DefendDBQueryRootcheck_default_query(mock_info, mock_backend, distinct):
...
@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBBackend")
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
def test_Shieldnet_DefendDBQueryRootcheck_parse_filters(mock_info, mock_backend):
...
@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBBackend")
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
def test_Shieldnet_DefendDBQueryRootcheck_filter_status(mock_info, mock_backend, status, expected_items):
...
@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBBackend")
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
def test_Shieldnet_DefendDBQueryRootcheck_filter_status_ko(mock_info, mock_backend):
    """Test if expected exception is raised when status does not exist"""
    test = rootcheck.ShieldnetDefendDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test', select=['log'],
                                           query='', count=True, get_data=True, distinct=False, fields={},
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})

    with pytest.raises(ShieldnetDefendException, match=".* 1603 .*"):
        test._filter_status({'value': 'test'})


@patch("shieldnetdefend.core.rootcheck.ShieldnetDefendDBBackend")
@patch('shieldnetdefend.core.agent.Agent.get_basic_information')
def test_Shieldnet_DefendDBQueryRootcheck_format_data_into_dictionary(mock_info, mock_backend):
    """Test if format_data_into_dictionary() returns expected element"""
    test = rootcheck.ShieldnetDefendDBQueryRootcheck(agent_id='100', offset=1, limit=1, sort=None, search='test',
                                           select=['log', 'date_first', 'status', 'date_last', 'cis', 'pci_dss'],
                                           query='', count=True, get_data=True, distinct=False,
                                           fields=rootcheck.ShieldnetDefendDBQueryRootcheck.fields,
                                           filters={'status': 'all', 'pci_dss': None, 'cis': None})
...
@patch('shieldnetdefend.core.wdb.ShieldnetDefendDBConnection')
def test_rootcheck_delete_agent(mock_db_conn, agent):
    """Test if proper parameters are being sent to the shieldnetdefenddb socket.

    Parameters
    ----------
    agent : str
        Agent whose information is being deleted from the db.
    mock_db_conn : ShieldnetDefendDBConnection
        Object used to send the delete message to the shieldnetdefenddb socket.
    """
