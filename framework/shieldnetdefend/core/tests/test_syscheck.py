...
with patch('shieldnetdefend.core.common.shieldnet_defend_uid'):
    with patch('shieldnetdefend.core.common.shieldnet_defend_gid'):
        from shieldnetdefend.core import syscheck
        from shieldnetdefend.core.utils import get_date_from_timestamp


@pytest.mark.parametrize('agent', ['002', '080'])
@patch("shieldnetdefend.core.syscheck.ShieldnetDefendDBBackend")
@patch("shieldnetdefend.core.syscheck.ShieldnetDefendDBQuery.__init__")
def test_shieldnet_defend_db_query_syscheck__init__(mock_wdbquery, mock_backend, agent):
    """Test if ShieldnetDefendDBQuery and ShieldnetDefendDBBackend are called with the expected parameters.
    ...
    agent: str
        The agent to test.
    """
    syscheck.ShieldnetDefendDBQuerySyscheck(agent)
...
@patch("shieldnetdefend.core.syscheck.ShieldnetDefendDBBackend")
def test_shieldnet_defend_db_syscheck_format_data_into_dictionary(mock_backend, data, is_json):
    """Test if _format_data_into_dictionary() returns the expected element."""
    test = syscheck.ShieldnetDefendDBQuerySyscheck('002', offset=0, limit=1000, sort=None, search='test',
