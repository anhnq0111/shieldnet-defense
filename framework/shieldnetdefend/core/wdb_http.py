...
class ShieldnetDefendDBHTTPClient:
    ...
    def __init__(self, retries: int = 5, timeout: float = 10):
        ...
        try:
            ...
            raise ShieldnetDefendInternalError(2011, e)

    async def close(self) -> None:
        """Close the shieldnet-defend-db HTTP client."""
    ...
    async def _get(self, endpoint: str) -> Any:
        ...
        try:
            ...
            if response.is_error:
                raise ShieldnetDefendError(2012, extra_message=response.text)

        except RequestError as exc:
            raise ShieldnetDefendError(2013, extra_message=str(exc))
...
@asynccontextmanager
async def get_wdb_http_client() -> AsyncIterator[ShieldnetDefendDBHTTPClient]:
    """Create and return the engine client.

    Returns
    -------
    AsyncIterator[ShieldnetDefendDBHTTP]
        ShieldnetDefend DB HTTP client iterator.
    """
    ...
    try:
        ...
        raise ShieldnetDefendInternalError(2014)
    except UnsupportedProtocol:
        raise ShieldnetDefendInternalError(2015)
    except ConnectError:
        raise ShieldnetDefendInternalError(2016)
    finally:
        await client.close()
