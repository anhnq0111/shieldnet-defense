...
from shieldnetdefend.core.exception import ShieldnetDefendException, ShieldnetDefendInternalError
...
class ShieldnetDefendSocket:
    ...
    def _connect(self):
        try:
            ...
            raise ShieldnetDefendInternalError(1013, extra_message=os.path.basename(self.path))
        except ConnectionRefusedError:
            raise ShieldnetDefendInternalError(1121, extra_message=f"Socket '{os.path.basename(self.path)}' cannot receive "
                                                         "connections")
        except Exception as e:
            raise ShieldnetDefendException(1013, str(e))
    ...
    def send(self, msg_bytes, header_format="<I"):
        if not isinstance(msg_bytes, bytes):
            raise ShieldnetDefendException(1105, "Type must be bytes")

        try:
            sent = self.s.send(pack(header_format, len(msg_bytes)) + msg_bytes)
            if sent == 0:
                raise ShieldnetDefendException(1014, "Number of sent bytes is 0")
            return sent
        except Exception as e:
            raise ShieldnetDefendException(1014, str(e))
...
class ShieldnetDefendSocketJSON(ShieldnetDefendSocket):
    MAX_SIZE = 65536

    def __init__(self, path):
        ShieldnetDefendSocket.__init__(self, path)

    def send(self, msg, header_format="<I"):
        return ShieldnetDefendSocket.send(self, msg_bytes=dumps(msg).encode(), header_format=header_format)

    def receive(self, header_format="<I", header_size=4, raw=False):
        response = loads(ShieldnetDefendSocket.receive(self, header_format=header_format, header_size=header_size).decode())
        if not raw:
            if 'error' in response.keys():
                if response['error'] != 0:
                    raise ShieldnetDefendException(response['error'], response['message'], cmd_error=True)
            return response['data']
        else:
            return response


class ShieldnetDefendAsyncProtocol(asyncio.Protocol):
    """ShieldnetDefend implementation of asyncio.Protocol class."""
...
class ShieldnetDefendAsyncSocket:
    ...
    async def connect(self, path_to_socket: str):
        ...
        try:
            ...
            self.transport, self.protocol = await self.loop.create_connection(
                lambda: ShieldnetDefendAsyncProtocol(self.loop), sock=self.s)
    ...
    async def send(self, msg_bytes: bytes, header_format: str = None) -> bytes:
        ...
        Raises
        ------
        ShieldnetDefendException(1105)
            If the `msg_bytes` type is not bytes.
        ShieldnetDefendException(1014)
        ...
        if self.is_connection_lost():
            await self.close()
            raise ShieldnetDefendException(1014, "Socket connection was closed")
...
class ShieldnetDefendAsyncSocketJSON(ShieldnetDefendAsyncSocket):
    """Handler class to connect and operate asynchronously with a socket using messages in JSON format."""

    def __init__(self):
        ShieldnetDefendAsyncSocket.__init__(self)
    ...
    async def send(self, msg: str, header_format: str = None) -> bytes:
        ...
        bytes
            Bytes sent.
        """
        return await ShieldnetDefendAsyncSocket.send(self, dumps(msg).encode(), header_format)
    ...
    async def receive(self, header_size: int = None) -> dict:
        ...
        dict
            Data received.
        """
        response = await ShieldnetDefendAsyncSocket.receive(self, header_size)
...
daemons = {
    ...
    "shieldnet-defend-db": {"protocol": "TCP", "path": common.WDB_SOCKET, "header_format": "<I", "size": 4},
    "remoted": {"protocol": "TCP", "path": common.REMOTED_SOCKET, "header_format": "<I", "size": 4}
}


async def shieldnet_defend_sendasync(daemon_name: str, message: str = None) -> dict:
    ...
    """
    try:
        sock = ShieldnetDefendAsyncSocket()
        ...
        raise ShieldnetDefendInternalError(1014, extra_message=e)

    return data


async def shieldnet_defend_sendsync(daemon_name: str = None, message: str = None) -> dict:
    """Send a message to the specified daemon's socket and wait for its response.
    ...
    """
    try:
        sock = ShieldnetDefendSocket(daemons[daemon_name]['path'])
        if isinstance(message, dict):
            message = dumps(message)
...
def create_shieldnet_defend_socket_message(origin=None, command=None, parameters=None):
