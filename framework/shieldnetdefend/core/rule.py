...
from shieldnetdefend.core.utils import load_shieldnet_defend_xml, add_dynamic_detail
...
def check_status(status: str) -> str:
    ...
    if status is None:
        ...
        raise ShieldnetDefendError(1202)
...
def load_rules_from_file(rule_filename: str, rule_relative_path: str, rule_status: str) -> list:
    ...
    Raises
    ------
    ShieldnetDefendError(1201)
        Error reading rule files.
    ShieldnetDefendError(1207)
    ...
    try:
        rules = list()
        root = load_shieldnet_defend_xml(os.path.join(common.SHIELDNET_DEFEND_PATH, rule_relative_path, rule_filename))
        ...
        if e.errno == 2:
            raise ShieldnetDefendError(1201)
        elif e.errno == 13:
            raise ShieldnetDefendError(1207)
        else:
            raise e
...
def item_format(data, all_items, exclude_filenames):
    for item in glob(all_items):
        ...
        item_dir = os.path.relpath(os.path.dirname(item), start=common.SHIELDNET_DEFEND_PATH)
...
def _create_rule_decoder_dir_dict(ruleset_conf, tag, exclude_filenames, data):
    ...
    for item_dir in items:
        all_rules = f"{common.SHIELDNET_DEFEND_PATH}/{item_dir}/*.xml"
...
def _create_dict(ruleset_conf, tag, exclude_filenames, data):
    ...
    for item in items:
        ...
        item_dir = os.path.relpath(full_dir if full_dir else common.RULES_PATH, start=common.SHIELDNET_DEFEND_PATH)
