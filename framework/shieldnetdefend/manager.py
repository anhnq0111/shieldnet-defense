...
from shieldnetdefend.core.cluster.utils import manager_restart, read_cluster_config, running_in_master_node
from shieldnetdefend.core.configuration import get_ossec_conf, write_ossec_conf
from shieldnetdefend.core.exception import ShieldnetDefendError, ShieldnetDefendInternalError
from shieldnetdefend.core.manager import status, get_api_conf, get_update_information_template, get_ossec_logs, \
...
from shieldnetdefend.core.utils import process_array, safe_move, validate_shieldnet_defend_xml, full_copy
...
def get_status() -> AffectedItemsShieldnetDefendResult:
    """Wrapper for status().
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Processes status was successfully read"
...
def ossec_log(level: str = None, tag: str = None, offset: int = 0, limit: int = common.DATABASE_LIMIT,
              ...
              q: str = '', select: str = None, distinct: bool = False) -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Logs were successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def ossec_log_summary() -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Log was successfully summarized"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def get_api_config() -> AffectedItemsShieldnetDefendResult:
    ...
    """
    result = AffectedItemsShieldnetDefendResult(**_get_config_default_result_kwargs)
...
def restart() -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(**_restart_default_result_kwargs)
    try:
        manager_restart()
...
def validation() -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(**_validation_default_result_kwargs)
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_config(component: str = None, config: str = None) -> AffectedItemsShieldnetDefendResult:
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Active configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
def read_ossec_conf(section: str = None, field: str = None, raw: bool = False,
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Configuration was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:read"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def get_basic_info() -> AffectedItemsShieldnetDefendResult:
    """Wrapper for ShieldnetDefend().to_dict
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Basic information was successfully read"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
                                      some_msg='Could not read basic information in some nodes',
    ...
    try:
        result.affected_items.append(ShieldnetDefend().to_dict())
...
@expose_resources(actions=[f"{'cluster' if cluster_enabled else 'manager'}:update_config"],
                  resources=[f'node:id:{node_id}' if cluster_enabled else '*:*:*'])
def update_ossec_conf(new_conf: str = None) -> AffectedItemsShieldnetDefendResult:
    """Replace shieldnetdefend configuration (ossec.conf) with the provided configuration.
    ...
    AffectedItemsShieldnetDefendResult
        Affected items.
    """
    result = AffectedItemsShieldnetDefendResult(all_msg=f"Configuration was successfully updated"
                                              f"{' in specified node' if node_id != 'manager' else ''}",
    ...
    try:
        ...
        if not new_conf:
            raise ShieldnetDefendError(1125)

        # Check if the configuration is valid
        validate_shieldnet_defend_xml(new_conf, config_file=True)
...
def get_update_information(installation_uid: str, update_information: dict) -> ShieldnetDefendResult:
    """Process update information into a shieldnetdefend result.
    ...
    if not update_information:
        ...
        return ShieldnetDefendResult({'data': get_update_information_template(uuid=installation_uid, update_check=False)})
    ...
    if status_code != 200:
        ...
        raise ShieldnetDefendInternalError(2100, extra_message=extra_message)

    update_information.pop('message', None)

    return ShieldnetDefendResult({'data': update_information})
