...
from shieldnetdefend.core.utils import clean_pid_files
...
def set_logging(foreground_mode=False, debug_mode=0) -> ShieldnetDefendLogger:
    """Set cluster logger.
...
def print_version():
    """Print ShieldnetDefend metadata."""
    from shieldnetdefend.core.cluster import __author__, __licence__, __version__, __shieldnet_defend_name__
    print(f"\n{__shieldnet_defend_name__} {__version__} - {__author__}\n\n{__licence__}")
...
def exit_handler(signum, frame):
    ...
    pyDaemonModule.delete_child_pids('shieldnet-defend-clusterd', cluster_pid, main_logger)

    # Remove cluster's pidfile
    pyDaemonModule.delete_pid('shieldnet-defend-clusterd', cluster_pid)
...
# Master main
#
async def master_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: ShieldnetDefendLogger):
    ...
    logger : ShieldnetDefendLogger
        Cluster logger.
    """
    from shieldnetdefend.core.cluster import local_server, master
    from shieldnetdefend.core.cluster.hap_helper.hap_helper import HAPHelper
...
# Worker main
#
async def worker_main(args: argparse.Namespace, cluster_config: dict, cluster_items: dict, logger: ShieldnetDefendLogger):
    ...
    from shieldnetdefend.core.cluster import local_server, worker
...
def main():
    """Main function of the shieldnet-defend-clusterd script in charge of starting the cluster process."""
    import shieldnetdefend.core.cluster.cluster

    # Set correct permissions on cluster.log file
    if os.path.exists(f'{common.SHIELDNET_DEFEND_PATH}/logs/cluster.log'):
        os.chown(f'{common.SHIELDNET_DEFEND_PATH}/logs/cluster.log', common.shieldnet_defend_uid(), common.shieldnet_defend_gid())
        os.chmod(f'{common.SHIELDNET_DEFEND_PATH}/logs/cluster.log', 0o660)
    ...
    if cluster_configuration['disabled']:
        sys.exit(0)
    try:
        shieldnetdefend.core.cluster.cluster.check_cluster_config(cluster_configuration)
    except Exception as e:
        main_logger.error(e)
        sys.exit(1)
    ...
    shieldnetdefend.core.cluster.cluster.clean_up()

    # Check for unused PID files
    clean_pid_files('shieldnet-defend-clusterd')

    # Foreground/Daemon
    if not args.foreground:
        pyDaemonModule.pyDaemon()

    # Drop privileges to shieldnetdefend
    if not args.root:
        os.setgid(common.shieldnet_defend_gid())
        os.setuid(common.shieldnet_defend_uid())

    pid = os.getpid()
    pyDaemonModule.create_pid('shieldnet-defend-clusterd', pid)
    if args.foreground:
    ...
    try:
        ...
        pyDaemonModule.delete_child_pids('shieldnet-defend-clusterd', pid, main_logger)
        pyDaemonModule.delete_pid('shieldnet-defend-clusterd', pid)


if __name__ == '__main__':
    import shieldnetdefend.core.cluster.utils as cluster_utils
    from shieldnetdefend.core import common, configuration, pyDaemonModule
    ...
    # Set logger
    try:
        debug_mode_ = configuration.get_internal_options_value('shieldnet_defend_clusterd', 'debug', 2, 0) or args.debug_level
    except Exception:
        debug_mode_ = 0
