...
def main():
    """shieldnet-defend-logtest main function."""
    # Parse cmdline args
    ...
    if args.version:
        logging.info('%s', ShieldnetDefend.get_description())
        logging.info('%s', ShieldnetDefend.get_license())
        sys.exit(0)
    ...
    # Initialize shieldnet-defend-logtest component
    w_logtest = ShieldnetDefendLogtest(location=args.location)
    logging.info('Starting shieldnet-defend-logtest %s', ShieldnetDefend.get_version_str())
    ...
    while True:
        ...
        try:
            ...
            logging.error('** Shieldnet-Defend-logtest error ' + str(error))
            continue
        except ConnectionError:
            logging.error('** Shieldnet-Defend-logtest error when connecting with shieldnet-defend-analysisd')
            continue
        ...
        if 'messages' in output.keys():
            ...
            for message in output['messages']:
                if message.startswith("WARNING"):
                    logging.warning('** Shieldnet-Defend-Logtest: %s', message)
        ...
        # Show shieldnet-defend-logtest output
        ShieldnetDefendLogtest.show_output(output)
...
class ShieldnetDefendDeamonProtocol:
    """Encapsulate logic communication aspects between ShieldnetDefend daemons."""

    def __init__(self, version: int = 1, origin_module: str = "shieldnet-defend-logtest", module_name: str = "shieldnet-defend-logtest"):
        """Class constructor.
        ...
        origin_module : str
            Origin source module. Default: "shieldnet-defend-logtest"
        module_name : str
            Source module name. Default: "shieldnet-defend-logtest"
        """
        self.protocol = dict()
    ...
    def wrap(self, command: str, parameters: dict) -> str:
        """Wrap data with ShieldnetDefend daemon protocol information.
    ...
    def unwrap(self, msg: dict) -> dict:
        """Unwrap data from ShieldnetDefend daemon protocol information.
...
class ShieldnetDefendSocket:
    """Encapsulate shieldnet-defend-socket communication (header with message size)."""
    ...
    def send(self, msg: str) -> bytes:
        """Send and receive data to shieldnet-defend-socket (header with message size).
...
class ShieldnetDefendLogtest:
    """Top level class to interact with shieldnet-defend-logtest feature, part of shieldnet-defend-analysisd."""
    ...
    def __init__(self, location: str = "stdin", log_format: str = "syslog"):
        ...
        """
        self.protocol = ShieldnetDefendDeamonProtocol()
        self.socket = ShieldnetDefendSocket(LOGTEST_SOCKET)
    ...
    def process_log(self, log, token: str = None, options: str = None) -> dict:
        """Send log event to shieldnet-defend-logtest and receive the outcome.
    ...
    def show_output(output: dict):
        ...
        ShieldnetDefendLogtest.show_ossec_logtest_like(output)

    def show_ossec_logtest_like(output: dict):
        """Show shieldnet-defend-logtest output like ossec-logtest.

        Parameters
        ----------
        output : dict
            Shieldnet-Defend-logtest outcome.
        """
        ...
        if 'predecoder' in output_data:
            ShieldnetDefendLogtest.show_phase_info(output_data['predecoder'], ['timestamp', 'hostname', 'program_name'])
        # Decoding phase
        logging.info('')
        logging.info('**Phase 2: Completed decoding.')
        if 'decoder' in output_data and output_data['decoder']:
            ShieldnetDefendLogtest.show_phase_info(output_data['decoder'], ['name', 'parent'])
            if 'data' in output_data:
                ShieldnetDefendLogtest.show_phase_info(output_data['data'])
        else:
        ...
        if 'rule' in output_data:
            ...
            ShieldnetDefendLogtest.show_phase_info(output_data['rule'], ['id', 'level', 'description', 'groups', 'firedtimes'])
        if output['alert']:
            logging.info('**Alert to be generated.')

    def show_phase_info(phase_data: dict, show_first: list = None, prefix: str = ""):
        """Show shieldnet-defend-logtest processing phase information.
        ...
        for field in sorted(phase_data.keys()):
            if isinstance(phase_data.get(field), dict):
                ShieldnetDefendLogtest.show_phase_info(phase_data.pop(field), [], prefix + field + '.')
            else:
...
class ShieldnetDefend:
    def get_install_path() -> str:
        """Get ShieldnetDefend installation path, obtained relative to the path of this file.

        Returns
        -------
        str
            ShieldnetDefend installation path.
        """
        return common.find_shieldnet_defend_path()

    def get_info(field: str) -> str:
        """Get ShieldnetDefend information from shieldnet-defend-control.
        ...
        str
            Field value.
        """
        shieldnet_defend_control = os.path.join(ShieldnetDefend.get_install_path(), "bin", "shieldnet-defend-control")
    ...
    def get_version_str() -> str:
        """Get ShieldnetDefend version string.

        Returns
        -------
        str
            ShieldnetDefend version.
        """
        return ShieldnetDefend.get_info('SHIELDNET_DEFEND_VERSION')

    def get_description() -> str:
        """Get ShieldnetDefend description, contact info and version.

        Returns
        -------
        str
            ShieldnetDefend description.
        """
        return f"ShieldnetDefend {ShieldnetDefend.get_version_str()} - ShieldnetDefend Inc."

    def get_license() -> str:
        """Get ShieldnetDefend License statement.

        Returns
        -------
        str
            ShieldnetDefend License.
        """
...
def init_logger(args: argparse.Namespace):
    """Initialize shieldnet-defend-logtest logger.
