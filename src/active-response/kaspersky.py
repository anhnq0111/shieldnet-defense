#!/usr/bin/env python
 ...
 ###
 # Integration of ShieldnetDefend agent with Kaspersky endpoint security for Linux
...
# Sets the file paths
##################################################################################################################
shieldnet_defend_path = os.path.abspath(os.path.join(__file__, "../../.."))
shieldnet_defend_queue = '{0}/queue/sockets/queue'.format(shieldnet_defend_path)
now = time.strftime("%a %b %d %H:%M:%S %Z %Y")
ar_log_file = '{0}/logs/active-responses.log'.format(shieldnet_defend_path)
...
def logger(msg, mode, foreground=None):
	...
	send_msg(shieldnet_defend_queue, msg)
...
def send_msg(shieldnet_defend_queue, msg):
	...
	try:
		s.connect(shieldnet_defend_queue)
	except:
		print('Error: ShieldnetDefend must be running.')
		sys.exit(1)
...
def main():
    ar_log()
    set_logger('shieldnet-defend-kaspersky', foreground=args.verbose)
    run_kaspersky()
