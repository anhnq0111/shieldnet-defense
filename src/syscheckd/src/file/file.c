    ...
    switch (resultType) {
        case INSERTED:
            ...
            if (syscheck.file_limit > 0) {
                sync_flag = (synced_docs < syscheck.file_limit) ? 1 : 0;
                minfo("INSERTED: synced_docs=%d, file_limit=%d, sync_flag=%d, path=%s",
                        synced_docs, syscheck.file_limit, sync_flag, path);
            } else {
                sync_flag = 1;
                minfo("INSERTED: file_limit=0 (unlimited), sync_flag=1, path=%s", path);
            }
        ...
        case MODIFIED:
            ...
            if (sync_flag == 0 && syscheck.file_limit > 0) { // Promote
                if(synced_docs < syscheck.file_limit){
                    synced_docs++;
                    cJSON* sync_item = cJSON_CreateObject();
                    if (sync_item != NULL) {
                        cJSON_AddStringToObject(sync_item, "path", path);
