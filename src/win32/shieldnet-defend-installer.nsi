...
!define REVISION "rc1"
!define NAME "ShieldnetDefend"
!define SERVICE "ShieldnetDefendSvc"

; output file
!ifndef OutFile
    !define OutFile "shieldnet-defend-agent-${VERSION}.exe"
!endif

Var is_upgrade

Name "${NAME} Windows Agent v${VERSION}"
BrandingText "Copyright (C) 2015, ShieldnetDefend Inc."
OutFile "${OutFile}"

VIProductVersion "4.13.1.0"
VIAddVersionKey ProductName "${NAME}"
VIAddVersionKey CompanyName "ShieldnetDefend Inc."
VIAddVersionKey LegalCopyright "2023 - ShieldnetDefend Inc."
VIAddVersionKey FileDescription "ShieldnetDefend Agent installer"
VIAddVersionKey FileVersion "${VERSION}"
VIAddVersionKey ProductVersion "${VERSION}"
VIAddVersionKey InternalName "ShieldnetDefend Agent"
...
Section "ShieldnetDefend Agent (required)" MainSec
    ...
    ; install files
    File shieldnet-defend-agent.exe
    File shieldnet-defend-agent-eventchannel.exe
    ...
    File /oname=active-response\bin\restart-shieldnet-defend.exe restart-shieldnet-defend.exe
    ...
    File /oname=libshieldnetdefendext.dll ..\libshieldnetdefendext.dll
    File /oname=libshieldnetdefendshared.dll ..\libshieldnetdefendshared.dll
    ...
    File /oname=syscollector.dll ..\shieldnet_defend_modules\syscollector\build\bin\syscollector.dll
    File /oname=libfimdb.dll ..\syscheckd/build/bin/libfimdb.dll
    File /oname=queue\syscollector\norm_config.json ..\shieldnet_defend_modules\syscollector\norm_config.json
    File VERSION.json
    ...
    ; use appropriate version of "shieldnet-defend-agent.exe"
    ${If} ${AtLeastWinVista}
        Delete "$INSTDIR\shieldnet-defend-agent.exe"
        Rename "$INSTDIR\shieldnet-defend-agent-eventchannel.exe" "$INSTDIR\shieldnet-defend-agent.exe"
    ${Else}
        Delete "$INSTDIR\shieldnet-defend-agent-eventchannel.exe"
    ${Endif}
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "Publisher" "ShieldnetDefend"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "DisplayIcon" '"$INSTDIR\favicon.ico"'
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "HelpLink" "https://shieldnetdefend.com"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\OSSEC" "URLInfoAbout" "https://shieldnetdefend.com"
    ...
    ServiceInstall:
        nsExec::ExecToLog '"$INSTDIR\shieldnet-defend-agent.exe" install-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
...
Section "Uninstall"
    ...
    ServiceUninstall:
        nsExec::ExecToLog '"$INSTDIR\shieldnet-defend-agent.exe" uninstall-service'
        Pop $0
        ${If} $0 <> 1
            MessageBox MB_ABORTRETRYIGNORE|MB_ICONSTOP "$\r$\n\
