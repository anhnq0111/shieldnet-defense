# Integration Tests
## Index
1. [Compile ShieldnetDefend](#compile-shieldnet-defend)
...
Navigate into `shieldnetdefend/src/dbsync/` and run the following commands:
```
mkdir build
cd build
cmake -DEXTERNAL_LIB=~/path/to/shieldnetdefend/src/external/ -DCMAKE_BUILD_TYPE=Debug -DUNIT_TEST=ON ..
cmake --build .
```
