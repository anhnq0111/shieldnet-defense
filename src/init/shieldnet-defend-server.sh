...
# shieldnet-defend-control        This shell script takes care of starting
...
AUTHOR="ShieldnetDefend Inc."
USE_JSON=false
DAEMONS="shieldnet-defend-clusterd shieldnet-defend-modulesd shieldnet-defend-monitord shieldnet-defend-logcollector shieldnet-defend-remoted shieldnet-defend-syscheckd shieldnet-defend-analysisd shieldnet-defend-maild shieldnet-defend-execd shieldnet-defend-db shieldnet-defend-authd shieldnet-defend-agentlessd shieldnet-defend-integratord shieldnet-defend-dbd shieldnet-defend-csyslogd shieldnet-defend-apid"
OP_DAEMONS="shieldnet-defend-clusterd shieldnet-defend-maild shieldnet-defend-agentlessd shieldnet-defend-integratord shieldnet-defend-dbd shieldnet-defend-csyslogd"
...
start_service()
{
    ...
    if [ $USE_JSON = false ]; then
        echo "Starting ShieldnetDefend $VERSION..."
    fi

    TEST=$(${DIR}/bin/shieldnet-defend-logtest-legacy -t  2>&1 | grep "ERROR")
    if [ ! -z "$TEST" ]; then
        if [ $USE_JSON = true ]; then
        ...
        touch ${DIR}/var/run/shieldnet-defend-analysisd.failed
        exit 1;
    fi
    ...
    for i in ${SDAEMONS}; do
        ## If shieldnet-defend-maild is disabled, don't try to start it.
        if [ X"$i" = "Xshieldnet-defend-maild" ]; then
             grep "<email_notification>no<" ${DIR}/etc/ossec.conf >/dev/null 2>&1
             if [ $? = 0 ]; then
                 continue
             fi
        fi
        ## If shieldnet-defend-clusterd is disabled, don't try to start it.
        if [ X"$i" = "Xshieldnet-defend-clusterd" ]; then
        ...
        ## If shieldnet-defend-authd is disabled, don't try to start it.
        if [ X"$i" = "Xshieldnet-defend-authd" ]; then
...
pstatus()
{
    ...
    if [ $? = 0 ]; then
        for pid in `cat ${DIR}/var/run/${pfile}-*.pid 2>/dev/null`; do
            ...
            if [ ! $? = 0 ]; then
                if [ $USE_JSON = false ]; then
                    echo "${pfile}: Process ${pid} not used by ShieldnetDefend, removing..."
                fi
...
stop_service()
{
    ...
    if [ $USE_JSON = true ]; then
        ...
        echo "ShieldnetDefend $VERSION Stopped"
    fi
}
...
info()
{
    if [ "X${1}" = "X" ]; then
        if [ $USE_JSON = true ]; then
            echo -n '{"error":0,"data":['
            echo -n '{"SHIELDNET_DEFEND_VERSION":"'${VERSION}'"},'
            echo -n '{"SHIELDNET_DEFEND_REVISION":"'${REVISION}'"},'
            echo -n '{"SHIELDNET_DEFEND_TYPE":"'${TYPE}'"}'
            echo -n ']}'
        else
            echo "SHIELDNET_DEFEND_VERSION=\"${VERSION}\""
            echo "SHIELDNET_DEFEND_REVISION=\"${REVISION}\""
            echo "SHIELDNET_DEFEND_TYPE=\"${TYPE}\""
        fi
    else
        case "${1}" in
            -v) echo "${VERSION}" ;;
...
restart)
    ...
    ;;
reload)
    DAEMONS=$(echo $DAEMONS | sed 's/shieldnet-defend-execd//')
    restart_service
    ;;
status)
    lock
