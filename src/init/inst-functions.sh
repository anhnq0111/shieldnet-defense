...
InstallCommon()
{
  ...
  ${INSTALL} -d -m 0750 -o root -g ${SHIELDNET_DEFEND_GROUP} ${INSTALLDIR}/lib
    ...
    elif [ -f libshieldnetdefendext.so ]
    then
        ${INSTALL} -m 0750 -o root -g ${SHIELDNET_DEFEND_GROUP} libshieldnetdefendext.so ${INSTALLDIR}/lib

        if ([ "X${DIST_NAME}" = "Xrhel" ] || [ "X${DIST_NAME}" = "Xcentos" ] || [ "X${DIST_NAME}" = "XCentOS" ]) && [ ${DIST_VER} -le 5 ]; then
            chcon -t textrel_shlib_t ${INSTALLDIR}/lib/libshieldnetdefendext.so
        fi
    fi
