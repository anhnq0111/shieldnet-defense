...
# Install functions for ShieldnetDefend
# ShieldnetDefend.com (https://github.com/shieldnetdefend)
...
InstallSELinuxPolicyPackage(){
    ...
    if command -v semodule > /dev/null && command -v getenforce > /dev/null; then
        if [ -f selinux/shieldnetdefend.pp ]; then
            if [ $(getenforce) != "Disabled" ]; then
                cp selinux/shieldnetdefend.pp /tmp && semodule -i /tmp/shieldnetdefend.pp
                rm -f /tmp/shieldnetdefend.pp
...
ShieldnetDefendUpgrade()
{
    ...
    if [ -f "$PREINSTALLEDDIR/var/db/global.db" ]; then
        ...
        if [ -f "$PREINSTALLEDDIR/queue/db/global.db" ]; then
            ...
            chown shieldnetdefend:shieldnetdefend $PREINSTALLEDDIR/queue/db/global.db
    ...
    # Remove existing SQLite databases for ShieldnetDefend DB, only if upgrading from 3.2..3.6
    ...
    if [ -d $PREINSTALLEDDIR/logs/ossec ]; then
        if [ "$(ls -A $PREINSTALLEDDIR/logs/ossec)" ]; then
            mv -f $PREINSTALLEDDIR/logs/ossec/* $PREINSTALLEDDIR/logs/shieldnetdefend
        fi
        rm -rf $PREINSTALLEDDIR/logs/ossec
    fi

    # Remove deprecated ShieldnetDefend tools
    ...
    # Remove old ShieldnetDefend daemons
    ...
    if (grep "^ossec:" /etc/group > /dev/null 2>&1) || (dscl . -read /Groups/ossec > /dev/null 2>&1)  ; then
        if [ "X$1" = "Xserver" ]; then
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -print0 | xargs -0 chown root:shieldnetdefend
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -print0 | xargs -0 chown shieldnetdefend:shieldnetdefend
        else
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -user root -exec chown root:shieldnetdefend {} \;
            find $PREINSTALLEDDIR -group $OSSEC_GROUP -exec chown shieldnetdefend:shieldnetdefend {} \;
        fi
    fi
