/*
 * ShieldnetDefend Module for file downloads
...
wmodule * wm_download_read() {
    ...
    data->enabled = getDefine_Int("shieldnet_defend_download", "enabled", 0, 1);
...
cJSON *wm_download_dump() {
    ...
    cJSON_AddItemToObject(root,"shieldnet_defend_download",wm_wd);
    return root;
}
#endif
