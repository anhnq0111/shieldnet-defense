/*
 * ShieldnetDefend Inventory Harvester - Network Interface element
...
public:
    ...
    static DataHarvester<InventoryNetIfaceHarvester> build(TContext* data)
    {
        ...
        element.data.shieldnetdefend.cluster.name = instancePolicyManager.getClusterName();
        if (instancePolicyManager.getClusterStatus())
        {
            element.data.shieldnetdefend.cluster.node = instancePolicyManager.getClusterNodeName();
        }
