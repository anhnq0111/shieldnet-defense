...
TEST_DATA_ROOT = Path("shieldnet_defend_modules/inventory_harvester/qa/test_data")
QUEUE_DIR = Path("queue")
TEMPLATE_ROOT = Path("shieldnet_defend_modules/inventory_harvester/indexer/template")
...
def discover_templates():
    ...
    return [path for path in TEMPLATE_ROOT.rglob("shieldnet-defend-states-*.json") if valid(path)]
...
def create_index_from_template(template_path: Path):
    ...
    if pattern and "*" in pattern:
        if "fim-files" in template_name:
            index_name = "shieldnet-defend-states-fim-files-cluster01"
        elif "fim-registries" in template_name:
            index_name = "shieldnet-defend-states-fim-registries-cluster01"
        else:
...
def test_data_indexation(opensearch, test_folder):
    ...
    is_shieldnet_defend_db = "shieldnet_defend_db" in str(test_path)

    cmd = (
        Path(
            "build/shieldnet_defend_modules/inventory_harvester/testtool/inventory_harvester_testtool"
        )
        if Path(
            "build/shieldnet_defend_modules/inventory_harvester/testtool/inventory_harvester_testtool"
        ).exists()
        else Path(
            "shieldnet_defend_modules/inventory_harvester/build/testtool/inventory_harvester_testtool"
        )
    )
    ...
    try:
        ...
        if is_shieldnet_defend_db:
            LOGGER.info("Loading templates manually for 'shieldnet_defend_db' test case...")
            load_all_templates()
