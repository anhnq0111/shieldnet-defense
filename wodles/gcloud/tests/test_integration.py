...
from integration import ShieldnetDefendGCloudIntegration, ANALYSISD
import exceptions
...
def test_Shieldnet_DefendGCloudIntegration__init__():
    """Test if an instance of ShieldnetDefendGCloudIntegration is created properly."""
    integration = ShieldnetDefendGCloudIntegration(logger=MagicMock())
    for attribute in ['logger', 'socket']:
        assert hasattr(integration, attribute)


def test_Shieldnet_DefendGCloudIntegration_format_msg():
...
def test_Shieldnet_DefendGCloudIntegration_initialize_socket(mock_socket):
...
def test_Shieldnet_DefendGCloudIntegration_initialize_socket_ko(raised_exception, errcode):
    """Test initialize_socket properly handles exceptions."""
    integration = ShieldnetDefendGCloudIntegration(logger=MagicMock())
    with patch('socket.socket', side_effect=raised_exception), pytest.raises(exceptions.ShieldnetDefendIntegrationInternalError) as e:
        integration.initialize_socket()
    assert errcode == e.value.errcode


def test_Shieldnet_DefendGCloudIntegration_process_data():
...
@patch('integration.socket.socket')
def test_Shieldnet_DefendGCloudIntegration_send_message(mock_socket):
    """Test if messages are sent to ShieldnetDefend queue socket."""
    integration = ShieldnetDefendGCloudIntegration(logger=MagicMock())
    with integration.initialize_socket():
        integration.send_msg(test_message)
    mock_socket.return_value.send.assert_called_with(
        f'{ShieldnetDefendGCloudIntegration.header}{test_message}'.encode(errors='replace'))


def test_Shieldnet_DefendGCloudIntegration_send_message_ko():
    ...
    with pytest.raises(exceptions.ShieldnetDefendIntegrationInternalError) as e:
        integration.send_msg(test_message)
