...
SHIELDNET_DEFEND_VERSION = "4.5.0"

TEST_SHIELDNET_DEFEND_PATH = "/var/ossec"
TEST_DATABASE = "test"
...
def get_shieldnet_defend_integration_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
    ...
    """Return a dict containing every parameter supported by ShieldnetDefendIntegration. Used to simulate different ossec.conf
    configurations.
...
def get_shieldnet_defend_aws_database_parameters(service_name: str = TEST_SERVICE_NAME, profile: str = TEST_AWS_PROFILE,
                                      db_name: str = TEST_DATABASE, access_key: str = None, secret_key: str = None,
                                      iam_role_arn: str = None, region: str = None, discard_field: str = None,
                                      discard_regex: str = None, sts_endpoint: str = None, service_endpoint: str = None,
                                      iam_role_duration: str = None, external_id: str = None):
    """Return a dict containing every parameter supported by ShieldnetDefendAWSDatabase.
...
def get_mocked_shieldnet_defend_integration(**kwargs):
    with patch('shieldnet_defend_integration.ShieldnetDefendIntegration.get_client'), \
            patch('shieldnet_defend_integration.sqlite3.connect'), \
            patch('shieldnet_defend_integration.utils.find_shieldnet_defend_path', return_value=TEST_SHIELDNET_DEFEND_PATH), \
            patch('shieldnet_defend_integration.utils.get_shieldnet_defend_version', return_value=SHIELDNET_DEFEND_VERSION):
        return shieldnet_defend_integration.ShieldnetDefendIntegration(**get_shieldnet_defend_integration_parameters(**kwargs))


def get_mocked_shieldnet_defend_aws_database(**kwargs):
    with patch('shieldnet_defend_integration.ShieldnetDefendAWSDatabase.check_metadata_version'), \
            patch('shieldnet_defend_integration.ShieldnetDefendIntegration.get_client'), \
            patch('shieldnet_defend_integration.sqlite3.connect'), \
            patch('shieldnet_defend_integration.utils.find_shieldnet_defend_path', return_value=TEST_SHIELDNET_DEFEND_PATH), \
            patch('shieldnet_defend_integration.utils.get_shieldnet_defend_version', return_value=SHIELDNET_DEFEND_VERSION):
        return shieldnet_defend_integration.ShieldnetDefendAWSDatabase(**get_shieldnet_defend_aws_database_parameters(**kwargs))
