...
SHIELDNET_DEFEND_DEFAULT_RETRY_CONFIGURATION = {aws_tools.RETRY_ATTEMPTS_KEY: 10, aws_tools.RETRY_MODE_BOTO_KEY: 'standard'}
MESSAGE_HEADER = "1:Shieldnet-Defend-AWS:"


class ShieldnetDefendIntegration:
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, service_name=None, region=None,
        ...
        self.shieldnet_defend_path = utils.find_shieldnet_defend_path()
        self.shieldnet_defend_version = utils.get_shieldnet_defend_version()
        self.shieldnet_defend_queue = path.join(self.shieldnet_defend_path, "queue", "sockets", "queue")
        self.shieldnet_defend_wodle = path.join(self.shieldnet_defend_path, "wodles", "aws")
    ...
    def default_config(profile: str) -> dict:
        ...
        This method is called when ShieldnetDefend Integration is instantiated and sets a default config using .aws/config file
        ...
        if path.exists(aws_tools.DEFAULT_AWS_CONFIG_PATH):
            ...
            try:
                ...
                if aws_tools.RETRY_ATTEMPTS_KEY in profile_config \
                    ...
                    # Set retry config
                    retries = copy.deepcopy(SHIELDNET_DEFEND_DEFAULT_RETRY_CONFIGURATION)
                    aws_tools.debug(
            ...
            args['config'] = config.Config(retries=copy.deepcopy(SHIELDNET_DEFEND_DEFAULT_RETRY_CONFIGURATION))
            aws_tools.debug(
    ...
    def send_msg(self, msg, dump_json=True):
        """
        Sends an AWS event to the ShieldnetDefend Queue
        ...
        try:
            ...
            s.connect(self.shieldnet_defend_queue)
            ...
            if e.errno == 111:
                aws_tools.error("ShieldnetDefend must be running.")
                sys.exit(11)
            elif e.errno == 90:
                aws_tools.error("Message too long to send to ShieldnetDefend.  Skipping message...")
                aws_tools.debug('+++ ERROR: Message longer than buffer socket for ShieldnetDefend. Consider increasing rmem_max. '
                                'Skipping message...', 1)
            else:
                aws_tools.error("Error sending message to shieldnetdefend: {}".format(e))
                sys.exit(13)
        except Exception as e:
            aws_tools.error("Error sending message to shieldnetdefend: {}".format(e))
            sys.exit(13)
...
class ShieldnetDefendAWSDatabase(ShieldnetDefendIntegration):
    """
    ...
    def __init__(self, access_key, secret_key, profile, iam_role_arn, db_name,
        ...
        self.sql_update_version_metadata = """
                    UPDATE
                        metadata
                    SET
                        value=:shieldnet_defend_version
                    WHERE
                        key='version';
                    """
        ...
        ShieldnetDefendIntegration.__init__(self, service_name=service_name,
        ...
        self.db_path = "{0}/{1}.db".format(self.shieldnet_defend_wodle, db_name)
        ...
        try:
            if self.db_cursor.execute(self.sql_find_table, {'name': 'metadata'}).fetchone():
                ...
                try:
                    ...
                    if metadata_version != self.shieldnet_defend_version:
                        self.db_cursor.execute(self.sql_update_version_metadata, {'shieldnet_defend_version': self.shieldnet_defend_version})
                ...
                try:
                    ...
                    self.db_cursor.execute(self.sql_insert_version_metadata, {'shieldnet_defend_version': self.shieldnet_defend_version})
