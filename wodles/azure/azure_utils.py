...
CREDENTIALS_URL = 'https://documentation.shieldnetdefend.com/current/azure/activity-services/prerequisites/credentials.html'
DEPRECATED_MESSAGE = (
...
def send_message(message: str):
    ...
    try:
        ...
        if e.errno == 111:
            logging.error('ShieldnetDefend must be running.')
            sys.exit(1)
        elif e.errno == 90:
            logging.error(
                'Message too long to send to ShieldnetDefend.  Skipping message...'
            )
        else:
            logging.error(f'Error sending message to shieldnetdefend: {e}')
            sys.exit(1)
    finally:
        s.close()
