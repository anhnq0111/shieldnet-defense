...
def test_DockerListener__init__():
    ...
    for attribute in ['shieldnet_defend_path', 'shieldnet_defend_queue', 'msg_header', 'client', 'thread1', 'thread2']:
...
def test_DockerListener_process(mock_send):
    """Test process function sends the decoded events to the ShieldnetDefend socket."""
    event = b"test"
    dl = docker_listener.DockerListener()
    dl.process(event)
...
def test_DockerListener_send_msg(mock_socket):
    """Test send_msg sends the messages with the expected contents to the ShieldnetDefend socket."""
    msg = '{"test": "value"}'
    ...
    m.connect.assert_called_with(dl.shieldnet_defend_queue)
